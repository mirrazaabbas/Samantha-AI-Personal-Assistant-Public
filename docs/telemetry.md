...
Samantha ships **anonymous usage telemetry** by default so the team can
...
- **Self-hosted backend** on the Samantha team's PostHog instance —
...
[`src/samantha/analytics/events.py`](../src/samantha/analytics/events.py).
...
1. [`src/samantha/analytics/redaction.py`](../src/samantha/analytics/redaction.py) — value-level pattern matching (20+ regexes for PII).
2. [`src/samantha/analytics/events.py`](../src/samantha/analytics/events.py) — structural allowlist (event name + property name + type validator).
...
- The leaderboard / contest opt-in (`OptInModal.tsx`) is a separate,
  ...
  the Samantha leaderboard. It is **not** the same as analytics and
