...
Samantha follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in Samantha is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
Samantha is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
j = Samantha(engine_key="ollama")  # Local Ollama server
...
Samantha **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
This recommendation is written to `config.toml` during `samantha init` and used as the default engine:

```bash
samantha init --force
# Detects hardware, writes ~/.samantha/config.toml with:
# [engine]
...
```bash
samantha telemetry stats          # Aggregated statistics
samantha telemetry export --json  # Export all records
```
...
Samantha provides a **clean Python API** through the `Samantha` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`samantha[server]`, `samantha[memory-colbert]`, etc.)
...
The API server (`samantha serve`) implements the **OpenAI chat completions API format**, making Samantha a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to Samantha:
...
Samantha requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of Samantha.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `samantha[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `samantha[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `samantha[memory-bm25]` | `rank-bm25` |
| API server | `samantha[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `samantha[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `samantha[inference-vllm]` | `vllm` |
| PDF ingestion | `samantha[memory-pdf]` | `pdfplumber` |
| WhatsApp Baileys | `samantha[channel-whatsapp-baileys]` | Node.js 22+ |
