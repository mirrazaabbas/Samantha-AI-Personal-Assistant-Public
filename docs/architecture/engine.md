...
The Engine primitive provides the **inference runtime** -- the layer that connects Samantha to language model servers. All backends implement a uniform interface, making it straightforward to swap between local and cloud inference without changing application code.
...
```bash
samantha ask --engine afm --model afm-3-core "..."   # in-process
samantha host afm-3 --backend apple_fm               # launch the shim
```
...
!!! note "Energy"
    AFM runs predominantly on the Neural Engine, so its energy is invisible on a GPU-only rail. Install `samantha[energy-apple]` to measure CPU/GPU/DRAM/ANE separately; records are then written with `energy_basis = "soc"` and an `ane_energy_joules` column. Without it, no Apple energy is recorded at all.
...
Samantha automatically detects system hardware to recommend the best engine. Detection runs at config load time via `detect_hardware()`:
...
```python
from samantha.engine import discover_engines
...
```python
from samantha.engine import discover_engines, discover_models
...
Engine hosts and defaults are configured in `~/.samantha/config.toml` using **nested per-engine sub-sections**:
...
```python
from samantha.engine._base import messages_to_dicts
...
```python
from samantha.engine import EngineConnectionError
