...
description: The five-primitive architecture behind Samantha — Intelligence, Engine, Agents, Tools, and Learning
search:
  boost: 2
---
...
Samantha is a research framework for studying on-device AI systems. Its architecture is organized around **five core abstractions** -- Intelligence, Engine, Agentic Logic, Memory, and Learning -- that work together through trace-driven feedback.

![Samantha Architecture](../assets/Samantha_Architecture.png)
...
The sandbox module (`samantha.sandbox`) adds a `SandboxedAgent` wrapper that runs any `BaseAgent` inside a Docker or Podman container with mount-security enforcement, and a `ContainerRunner` that manages the container lifecycle.
...
All extensible components in Samantha use a **decorator-based registry** for runtime discovery. The pattern is implemented in `RegistryBase[T]`, a generic base class that provides isolated storage per typed subclass.
...
src/samantha/
    core/               Core infrastructure shared by all primitives
        ...
        config.py           SamanthaConfig, hardware detection, TOML loading
    ...
    cli/                CLI commands (Click-based)
        ask.py              samantha ask -- query the assistant
        serve.py            samantha serve -- start API server

    sdk.py              Samantha class -- high-level Python SDK
