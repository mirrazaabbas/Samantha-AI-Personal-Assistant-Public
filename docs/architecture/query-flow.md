...
This page traces the end-to-end journey of a user query through the Samantha system, from the moment it enters the CLI or SDK to the final response and telemetry recording.
...
sequenceDiagram
    ...
    User->>CLI: samantha ask "query" / j.ask("query")
    CLI->>CFG: load_config()
    CFG-->>CLI: SamanthaConfig (hardware, engine defaults)
...
Samantha supports two query processing paths, selected by the `--agent` CLI flag or the `agent` parameter in the SDK.
...
```bash
# CLI
samantha ask --agent orchestrator --tools calculator,think "What is 2^10 + 3^5?"
...
```python
config = load_config()  # Reads ~/.samantha/config.toml
```
...
- Returns a `SamanthaConfig` dataclass with all settings
...
The `TelemetryStore` subscribes to `TELEMETRY_RECORD` events on the EventBus and writes records to `~/.samantha/telemetry.db`.
...
The `Samantha` class in `sdk.py` provides the same query flow through a Python API:

```python
from samantha import Samantha

j = Samantha(model="qwen3:8b", engine_key="ollama")
