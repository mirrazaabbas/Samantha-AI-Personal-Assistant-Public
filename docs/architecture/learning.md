...
!!! note "Backward compatibility"
    The canonical locations are now `samantha.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `samantha.core.types` (for `RoutingContext`). The old `samantha.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `samantha.learning._stubs`.
...
```bash
samantha ask --router learned "What is the capital of France?"
```
...
```python
from samantha.learning.router import HeuristicRouter, build_routing_context
...
```python
from samantha.learning.router import build_routing_context
...
```python
from samantha.learning.trace_policy import TraceDrivenPolicy
from samantha.traces.analyzer import TraceAnalyzer
...
```python
from samantha.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from samantha.learning.sft_policy import SFTPolicy
```
...
```python
from samantha.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from samantha.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from samantha.learning.heuristic_reward import HeuristicRewardFunction
...
over Samantha's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `samantha-learning`
