...
- **Persistence:** Data persists across restarts in `~/.samantha/memory.db`
...
from samantha.tools.storage.faiss_backend import FAISSMemory
from samantha.tools.storage.hybrid import HybridMemory
...
!!! note "Backward compatibility"
    The old imports (e.g., `from samantha.memory.sqlite import SQLiteMemory`) still work via backward-compatibility shims in the `memory/` package, but the canonical location is now `samantha.tools.storage.*`.
...
```python
from samantha.tools.storage.chunking import chunk_text, ChunkConfig
...
```python
from samantha.tools.storage.embeddings import SentenceTransformerEmbedder
...
```python
from samantha.tools.storage.context import inject_context, ContextConfig
...
The default backend is configured in `~/.samantha/config.toml`. Storage settings live under `[tools.storage]`, and context injection is controlled by `agent.context_from_memory`:
