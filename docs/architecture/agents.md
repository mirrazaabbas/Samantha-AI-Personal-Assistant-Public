...
!!! note "Backward compatibility"
    The old `from samantha.agents.react import ReActAgent` import path still works via a backward-compat shim. The registry alias `"react"` also maps to `NativeReActAgent`.

```python
from samantha.agents.native_react import NativeReActAgent
...
```python
from samantha.agents.native_openhands import NativeOpenHandsAgent
...
```python
from samantha.agents.rlm import RLMAgent
...
```python
from samantha.agents.openhands import OpenHandsAgent
...
4. Reads stdout and extracts the JSON payload between `---SAMANTHA_OUTPUT_START---` and `---SAMANTHA_OUTPUT_END---` sentinels
...
- `cleanup_orphans()` removes any stale containers labelled `samantha-sandbox=true`
...
from samantha.agents._stubs import BaseAgent
