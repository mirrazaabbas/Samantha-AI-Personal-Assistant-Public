...
Samantha has two complementary observability systems: **telemetry** for per-inference metrics and **traces** for full interaction-level recording. Together, they provide comprehensive insight into system behavior and power the learning system's routing policy updates.
...
bus = EventBus()
store = TelemetryStore(db_path="~/.samantha/telemetry.db")
...
agg = TelemetryAggregator(db_path="~/.samantha/telemetry.db")
...
samantha telemetry stats -n 5          # Top 5 models only

# Export records
samantha telemetry export              # JSON to stdout
samantha telemetry export -f csv       # CSV to stdout
samantha telemetry export -o data.json # JSON to file
samantha telemetry export -f csv -o metrics.csv

# Clear all records
samantha telemetry clear               # With confirmation prompt
samantha telemetry clear --yes         # Without confirmation
```
