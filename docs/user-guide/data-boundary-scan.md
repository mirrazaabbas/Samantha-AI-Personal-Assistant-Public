...
`samantha scan --data-boundaries` reports application-level data boundaries in the
current Samantha configuration. It complements the existing host/environment
...
```bash
samantha scan --data-boundaries
samantha scan --data-boundaries --json
samantha scan --data-boundaries --json --show-paths
samantha scan --data-boundaries --strict
```
...
On a fresh `samantha init` configuration, common warn findings include
`server.host = "0.0.0.0"` and `telemetry.enabled = true`. Running
`samantha scan --data-boundaries --strict` after init therefore exits `1` until
...
if `SAMANTHA_CONFIG` is set, that file is audited. Otherwise the scan uses
the default Samantha config path under the resolved Samantha home. If the
