...
This writes a pre-configured `~/.samantha/config.toml` for the deep research agent.
...
Samantha chunks the content and stores it in a local SQLite/FTS5 database. Supported formats include `.txt`, `.md`, `.pdf`, `.py`, `.json`, `.csv`, and more.
...
samantha ask "What meetings did I have with Alice last month?"

# Explicitly specify the agent
samantha ask --agent deep_research "Compare the approaches described in paper-a.pdf and paper-b.pdf"

# Index more documents
samantha memory index ~/Downloads/reports/
samantha memory index ./notes.md

# Search memory directly
samantha memory search "project timeline"
samantha memory search -k 20 "budget estimates"
...
samantha ask "Summarize all emails about the Q3 budget review"

# Cross-reference sources
samantha ask "What do papers A and B agree on regarding attention mechanisms?"

# Find specific information
samantha ask "What meetings did I have with Alice last month?"

# Extract structured data
samantha ask "List all action items from the meeting notes in ~/Documents/meetings/"

# Research with web fallback
samantha ask "Compare our internal benchmarks with the latest published results"
```
...
# Single file
samantha memory index ./report.pdf

# Custom chunk size for long documents
samantha memory index ./paper.pdf --chunk-size 1024 --chunk-overlap 128
```
...
```bash
samantha memory index ~/Papers/*.pdf
```
...
```bash
curl -s https://example.com/article | samantha memory index --stdin --source "example.com"
```

### Code repositories

```bash
samantha memory index ./src/ --chunk-size 256
```
...
**"No results found"** -- Make sure you have indexed documents first with `samantha memory index`. Check indexed content with `samantha memory stats`.
