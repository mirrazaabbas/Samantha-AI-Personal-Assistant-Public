...
`configs/samantha/examples/full-system-access.toml`.

Then confirm the list actually resolved:

```bash
python -c "from samantha.core.config import load_config; print(load_config().tools.enabled)"
```
...
|-------------|-----------|
| `samantha chat` | Prompts before each call. |
| `samantha ask` | Auto-approves. |
| `samantha agent ask` | Auto-approves. Pass `--no-yes` if you want prompts. |
...
| How you run Samantha | Grant access to |
|------------------------|-----------------|
| CLI (`samantha ask`, `samantha chat`) | Your terminal (Terminal, iTerm, Warp) |
| Desktop app | `Samantha.app`, which spawns `samantha serve` beneath it |
| launchd (`deploy/launchd/com.samantha.plist`) | The `samantha` binary, as its own entry |
...
nothing for it. Add `/usr/local/bin/samantha` separately.
...
There's no computer use. Samantha can't see your screen, move the pointer or
...
policy_path = "~/.samantha/policy.yaml"
```
...
- [External MCP Servers](mcp-external-servers.md) for capabilities Samantha doesn't ship
