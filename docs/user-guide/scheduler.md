...
!!! note "Optional component"
    The scheduler is a standalone module (`samantha.scheduler`). It is not wired into the default `Samantha` / `SamanthaSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `samantha scheduler start`.
...
The `samantha scheduler` subcommand group manages tasks and the daemon from the terminal.
...
# Active tasks only
samantha scheduler list --status active

# Paused tasks
samantha scheduler list --status paused
```
...
samantha scheduler pause a3f9b12c4d8e

# Resume -- next_run is recomputed from the current time
samantha scheduler resume a3f9b12c4d8e
```
...
samantha scheduler cancel a3f9b12c4d8e
```
...
samantha scheduler logs a3f9b12c4d8e
```
...
# Set up storage
store = SchedulerStore(db_path="~/.samantha/scheduler.db")  # (1)!

# Wire in a SamanthaSystem for task execution
...
4. Starts a daemon thread named `"samantha-scheduler"`. Daemon threads exit automatically when the main process exits.
...
samantha ask --agent orchestrator \
...
Scheduler settings live in the `[scheduler]` section of `~/.samantha/config.toml`.

```toml title="~/.samantha/config.toml"
[scheduler]
enabled = false
db_path = "~/.samantha/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.samantha/scheduler.db`     | SQLite database path                       |
