...
from samantha.tools._stubs import ToolSpec
...
# Execute a tool call
from samantha.core.types import ToolCall
...
from samantha.memory.sqlite import SQLiteMemory
...
```python
from samantha.tools.file_read import FileReadTool
...
```python
from samantha.scheduler.tools import ScheduleTaskTool
from samantha.scheduler.scheduler import TaskScheduler
from samantha.scheduler.store import SchedulerStore

store = SchedulerStore(db_path="~/.samantha/scheduler.db")
scheduler = TaskScheduler(store=store, system=samantha_system)
scheduler.start()
...
```bash
samantha ask --agent orchestrator --tools my_tool "Process this data"
```
...
# Multiple tools
samantha ask --agent orchestrator --tools calculator,think "Solve: 2x + 5 = 13"

# All available tools (list them)
samantha ask --agent orchestrator --tools calculator,think,retrieval,file_read "..."
```
