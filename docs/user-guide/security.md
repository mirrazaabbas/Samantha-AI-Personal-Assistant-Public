...
Samantha includes a security layer that scans prompts and model outputs for secrets, personally identifiable information (PII), and sensitive file paths. The system is designed to be composable: scanners run as a pipeline, and the `GuardrailsEngine` wrapper drops in front of any inference backend without changing how the rest of your code works.

## Three layers of security review

Samantha separates host posture, application data boundaries, and runtime prompt guardrails:

| Layer | Command / component | What it checks |
| --- | --- | --- |
| Host scan | `samantha scan` | Disk encryption, cloud-sync agents, exposed engine ports, remote-access tools |
| Data-boundary scan | `samantha scan --data-boundaries` | Configured inference, memory, traces, channels, tools, and local stores |
...
=== "Block mode"
    ...
    from samantha.engine.ollama import OllamaEngine
    from samantha.security.guardrails import GuardrailsEngine, SecurityBlockError
    from samantha.security.types import RedactionMode
    from samantha.core.types import Message, Role
...
from samantha.security.guardrails import GuardrailsEngine
from samantha.security.scanner import SecretScanner
from samantha.security.types import RedactionMode
...
from samantha.security.scanner import PIIScanner

scanner = PIIScanner()

text = "Contact samantha@example.com or call 555-867-5309"
...
from samantha.security.file_policy import is_sensitive_file, filter_sensitive_paths
...
audit = AuditLogger(db_path="~/.samantha/audit.db", bus=bus)
...
import time
from samantha.security.audit import AuditLogger
from samantha.security.types import SecurityEvent, SecurityEventType
...
audit = AuditLogger(db_path="~/.samantha/audit.db")
...
Security settings live in the `[security]` section of `~/.samantha/config.toml`.
...
secret_scanner = true
pii_scanner = true
audit_log_path = "~/.samantha/audit.db"
...
| `audit_log_path` | `str` | `~/.samantha/audit.db` | Path to the SQLite audit log |
...
from samantha.security.types import ScanFinding, ScanResult, ThreatLevel
...
See [Data Boundary Scan](data-boundary-scan.md) for the application config diagnostic (`samantha scan --data-boundaries`).

---

## See Also

- [Data Boundary Scan](data-boundary-scan.md) — application config and local-store diagnostic (`samantha scan --data-boundaries`)
- [Architecture: Security](../architecture/security.md) — pipeline design, event flow, and file policy integration
- [API Reference: Security](../api-reference/samantha/security/index.md) — full class and function signatures
