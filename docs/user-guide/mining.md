...
Samantha can mine the Pearl Proof-of-Useful-Work chain through local LLM
...
Samantha falls back to building from the pinned Pearl source checkout. First
builds can take 30-60 minutes.

On a shared NVIDIA host, restrict the miner to idle GPUs:

```bash
uv run samantha mine init --cuda-visible-devices 0
```
...
- `samantha mine models` lists Pearl model support status.
- `samantha mine inspect-model` checks a Pearl model artifact before GPU launch.
- `samantha mine doctor` prints hardware, Docker, Pearl node, wallet, provider,
  and session checks.
- `samantha mine init` writes the local mining config and resolves the image.
- `samantha mine start` launches the Pearl miner container and writes the runtime
  sidecar.
- `samantha mine stop` stops the provider and removes the sidecar.
- `samantha mine status` reads live gateway metrics.
- `samantha mine attach` writes a sidecar for a miner you launched manually.
- `samantha mine logs` prints the Docker container log tail.
- `samantha mine validate-model` probes the active vLLM miner and gateway before
...
```bash
samantha mine models
```

Samantha only lists Pearl-compatible models published by the Pearl Research
...
pass the Samantha H100/H200 validation run.

When validating a Pearl org model on a mining host, run:

```bash
samantha mine inspect-model \
  --model pearl-ai/Gemma-4-31B-it-pearl \
  --allow-planned

samantha mine validate-model \
  --model pearl-ai/Gemma-4-31B-it-pearl \
  --allow-planned \
...
v1 is solo mining only. Samantha does not take fees, custody funds, generate
wallet keys, run pools, or operate `pearld`. Users provide their own Pearl node
and payout address.

Unsupported in this PR:

- Pool mining and the future 20% Samantha fee model
- AMD GPU mining and non-Pearl backends
- RTX 4090 or other non-Hopper NVIDIA GPUs
- Wallet generation or transaction signing inside Samantha
