...
```bash
python examples/samantha/spec_search_quickstart.py
```
...
`samantha.learning.spec_search.composite_reward.score_batch`.
...
`configs/samantha/examples/spec-search-quickstart.toml`. Copy it to
`~/.samantha/config.toml` (or set `SAMANTHA_CONFIG` to it) and the
...
| `trace_store` | `MagicMock` | `samantha.traces.store.TraceStore(home / "traces.db")` |
| `student_runner` | `MagicMock` | `samantha.learning.spec_search.student_runner.VLLMStudentRunner(host=..., model=...)` |
| `judge` | `MagicMock` | `samantha.evals.core.scorer.LLMJudgeScorer(...)` (or a deterministic scorer if your benchmark provides one) |
| `session_store` | `MagicMock` | `samantha.learning.spec_search.storage.session_store.SessionStore(home / "learning" / "sessions.db")` |
| `checkpoint_store` | `MagicMock` | `samantha.learning.spec_search.checkpoint.store.CheckpointStore(home / "learning" / "checkpoints")` |
...
1. Create `src/samantha/evals/datasets/<corpus>.py` implementing
   `DatasetProvider` (`adp.py` is a small reference). The provider's
   `load(max_samples, seed, split)` must respect `split` via
   `apply_split` from `samantha.evals.core.splits`.
2. Register: `@DatasetRegistry.register("<corpus>")`.
3. Feed it to the proposer via the trace store:

```python
from samantha.evals.datasets.adp import ADPDataset
from samantha.learning.spec_search.external_adapter import (
...
store = TraceStore("~/.samantha/traces.db")
...
`src/samantha/evals/backends/samantha_agent.py` previously hardcoded
`builder.telemetry(telemetry).traces(True).build()`, ignoring the
`telemetry` parameter. This silently caused every agent-backend
evaluation to write to `~/.samantha/traces.db` regardless of caller
...
- `examples/samantha/spec_search_quickstart.py` — runnable end-to-end demo.
- `configs/samantha/examples/spec-search-quickstart.toml` — prebuilt config.
- `src/samantha/learning/spec_search/orchestrator.py` — `SpecSearchOrchestrator` (single session).
- `src/samantha/learning/spec_search/multi_session.py` — `SpecSearchLoop` (Algorithm 1).
- `src/samantha/learning/spec_search/composite_reward.py` — paper Eq. 1.
- `src/samantha/learning/spec_search/gate/benchmark_gate.py` — `GateOK` predicate.
- `src/samantha/learning/spec_search/external_adapter.py` — corpus → trace adapter.
