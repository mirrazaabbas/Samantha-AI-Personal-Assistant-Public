...
The Samantha Python SDK provides a high-level interface for interacting with local inference engines, managing memory, and running agent workflows. The primary entry point is the `Samantha` class.
...
Samantha(
    *,
    config: SamanthaConfig | None = None,
...
|---------------|------------------|---------|----------------------------------------------------------------|
| `config`      | `SamanthaConfig`   | `None`  | Provide a pre-built configuration object                       |
...
If no `config` or `config_path` is provided, the SDK loads configuration from the default location (`~/.samantha/config.toml`), falling back to built-in defaults.
...
|-----------|----------------|-----------------------------------|
| `config`  | `SamanthaConfig` | The active configuration object   |
| `version` | `str`          | The Samantha version string     |
...
The `Samantha.memory` attribute provides a `MemoryHandle` for document indexing, search, and statistics. The memory backend is lazily initialized on first use.
...
Release all resources held by the `Samantha` instance, including the memory backend, telemetry store, and engine connection.
...
!!! tip "Context Manager Pattern"
    While `Samantha` does not implement `__enter__`/`__exit__` directly, you should always call `close()` when done to free database connections and other resources:
