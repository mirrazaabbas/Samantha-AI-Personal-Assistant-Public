...
A lightweight conversational AI with no tools and no agent overhead. This is the simplest possible Samantha setup: just Ollama and a local model. Ideal for general-purpose chat, Q&A, brainstorming, and getting started quickly.
...
### 2. Install and initialize Samantha

```bash
git clone https://github.com/mirrazaabbas/Samantha-AI-Personal-Assistant-Public.git
cd Samantha
uv sync
samantha init --preset chat-simple
```

### 3. Ask a question

```bash
samantha ask "What is quantum computing?"
```
...
```bash
# Single question
samantha ask "Explain the difference between TCP and UDP"
...
samantha ask -m qwen3.5:9b "Explain general relativity"

# Adjust temperature (0.0 = deterministic, 1.0 = creative)
samantha ask -t 0.2 "List the planets in our solar system"

# Output raw JSON
samantha ask --json "What is 2+2?"
```
...
To switch models, either edit `~/.samantha/config.toml` or override per-query:

```bash
samantha ask -m qwen3.5:35b "Write a detailed comparison of REST and GraphQL"
```
...
1. Start the backend: `samantha serve` (or `./scripts/quickstart.sh`)
2. Download and open the desktop app from the [releases page](https://github.com/mirrazaabbas/Samantha-AI-Personal-Assistant-Public/releases)
...
${EDITOR:-nano} ~/.samantha/config.toml
...
samantha ask -m deepseek-r1:14b "Hello"
```

**Use an environment variable:**

```bash
SAMANTHA_MODEL=qwen3.5:9b samantha ask "Hello"
```
...
**Browser app not loading** -- Make sure both the backend (`samantha serve`) and frontend are running. The `./scripts/quickstart.sh` script starts both automatically.
