...
Agents are the agentic logic layer of Samantha. They determine how a query is processed -- whether it goes directly to a model, through a tool-calling loop, via ReAct reasoning, CodeAct code execution, recursive decomposition, or an external agent runtime. All agents implement the `BaseAgent` ABC and are registered via the `AgentRegistry`.
...
(The config directory honors `$SAMANTHA_HOME` / `$XDG_DATA_HOME` when set.) The paths are configurable under `[memory_files]`:

```toml
[memory_files]
soul_path    = "~/.samantha/SOUL.md"
memory_path  = "~/.samantha/MEMORY.md"
user_path    = "~/.samantha/USER.md"
...
```
~/.samantha/personas/<name>/SOUL.md
~/.samantha/personas/<name>/MEMORY.md
~/.samantha/personas/<name>/USER.md
```

Select one per invocation, or opt out entirely:

```bash
samantha ask --persona work  "summarize my open PRs"
samantha ask --persona none  "what is 2 + 2?"     # inject no persona
```
...
`SOUL.md`, `MEMORY.md`, and `USER.md` are plain Markdown -- open them in any editor. `MEMORY.md` and `USER.md` can also be updated by the agent itself through the `memory_manage` and `user_profile_manage` tools when those are enabled, so the agent can record a new fact mid-conversation. These tools always target the default `MEMORY.md` and `USER.md` (under `~/.samantha/`), never a named persona's copies -- edit those by hand.
...
from samantha.agents._stubs import AgentContext, AgentResult
...
!!! note "Backward compatibility"
    The registry alias `"react"` maps to `NativeReActAgent`. The old import `from samantha.agents.react import ReActAgent` also still works.
...
```bash
# Simple agent
samantha ask --agent simple "What is the capital of France?"

# Orchestrator with tools
samantha ask --agent orchestrator --tools calculator,think "What is sqrt(256)?"

# NativeReActAgent
samantha ask --agent native_react --tools calculator "What is 2+2?"

# ReAct alias (same as native_react)
samantha ask --agent react --tools calculator,think "Solve step by step: 15% of 340"

# NativeOpenHandsAgent
samantha ask --agent native_openhands --tools calculator,web_search "Summarize example.com"

# RLMAgent
samantha ask --agent rlm "Summarize this long document"

# OpenHands SDK agent
samantha ask --agent openhands "Fix the bug in test_utils.py"
```
...
!!! warning "Requirements"
    Requires Node.js 22+ with npm on `PATH` and an `ANTHROPIC_API_KEY` environment variable (or pass `api_key=` directly). The bundled runner is auto-installed to `~/.samantha/claude_code_runner/` on first use.

**How it works:**

1. On first call, copies the bundled `claude_code_runner/` to `~/.samantha/claude_code_runner/` and installs the pinned Agent SDK if it is missing or outdated.
...
4. The Python side parses the output between `---SAMANTHA_OUTPUT_START---` and `---SAMANTHA_OUTPUT_END---` markers, extracting content, tool results, and metadata.
...
**When to use:** For software engineering tasks where the Claude Agent SDK's built-in tools (code editing, bash execution, file operations) provide capabilities beyond what Samantha tool-calling agents support.

```python
from samantha.agents.claude_code import ClaudeCodeAgent
...
```bash
# Via CLI
samantha ask --agent claude_code "Refactor the tests to use pytest fixtures"
```

!!! info "accepts_tools = False"
    `ClaudeCodeAgent` does not accept Samantha tools via `--tools`. Tool access for the Claude agent is configured separately via the `allowed_tools` constructor parameter, which passes tool names understood by the Claude Agent SDK itself.

---

## OpenCodeAgent

The `OpenCodeAgent` delegates coding tasks to [opencode](https://opencode.ai), the open-source coding agent, running it **on your local engine**. opencode handles the agentic loop, file edits, and tool use; Samantha supplies the model — keeping coding-agent work local-first.

!!! warning "Requirements"
    Requires the `opencode` binary on `PATH` (`npm i -g opencode-ai` or `brew install anomalyco/tap/opencode`). It is **not** bundled; `run()` returns a clear error if it is missing. No `ANTHROPIC_API_KEY` needed — inference goes through your Samantha engine.

**How it works:**

1. Derives an OpenAI-compatible base URL from the `engine` (e.g. Ollama/vLLM/llama.cpp at `<host>/v1`) and writes an `opencode.json` in the workspace registering it as an `@ai-sdk/openai-compatible` provider (`samantha/<model>`).
...
| `provider_id`       | `str`             | `"samantha"`   | opencode provider id to register/use                     |
...
```python
from samantha.agents.opencode import OpenCodeAgent
...
samantha ask --agent opencode "Refactor the parser to use a state machine"
```
...
```python
from samantha.agents.operative import OperativeAgent
...
```bash
# Via CLI
samantha ask --agent operative "Check system status"
```
...
```python
from samantha.agents.monitor_operative import MonitorOperativeAgent
...
```bash
# Via CLI
samantha ask --agent monitor_operative "Analyze the security audit findings"
```
...
```python
from samantha.sandbox import ContainerRunner, SandboxedAgent
from samantha.agents.simple import SimpleAgent

runner = ContainerRunner(
    image="samantha-sandbox:latest",
    timeout=60,
    mount_allowlist_path="/etc/samantha/mount_allowlist.json",
)
...
| Parameter              | Type   | Default                      | Description                                    |
|------------------------|--------|------------------------------|------------------------------------------------|
| `image`                | `str`  | `"samantha-sandbox:latest"`| Docker image to run                            |
