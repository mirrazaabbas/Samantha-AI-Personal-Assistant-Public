...
This writes a pre-configured `~/.samantha/config.toml` for the code assistant.
...
```bash
samantha ask "Write a Python script that reads a CSV file and prints the top 5 rows"
```
...
samantha ask "Write a Python script that parses JSON from stdin"

# Read and explain existing code
samantha ask "Read main.py and explain the architecture"

# Fix a bug
samantha ask "Find and fix the bug in test_utils.py"

# Run tests
samantha ask "Run the test suite and summarize any failures"

# Explicitly specify agent and tools
samantha ask --agent orchestrator --tools code_interpreter "Calculate the first 20 Fibonacci numbers"
...
The preset writes this to `~/.samantha/config.toml`:
...
```bash
# Write a new script
samantha ask "Write a Python script that converts YAML to JSON"

# Explain existing code
samantha ask "Read src/samantha/core/events.py and explain the EventBus pattern"

# Debug a failing test
samantha ask "Run pytest tests/test_memory.py -v and fix any failures"

# Refactor code
samantha ask "Read utils.py and refactor the parse_config function to use dataclasses"

# Generate tests
samantha ask "Read src/samantha/tools/calculator.py and write unit tests for it"

# Shell tasks
samantha ask "Find all Python files larger than 100KB in this repo"
```
...
- The agent asks for confirmation before executing potentially destructive commands when running in interactive mode (`samantha chat`).
- For stronger isolation, use the sandboxed agent: `samantha ask --agent sandboxed --tools code_interpreter "..."`, which runs inside a Docker/Podman container.
...
**Shell command fails** -- The `shell_exec` tool runs commands relative to where you launched `samantha`. Use `cd /path && command` in your prompt if needed, or run `samantha` from the project directory.
