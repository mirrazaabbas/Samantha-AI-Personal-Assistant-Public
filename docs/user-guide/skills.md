...
| **Overlay** | Sidecar file at `~/.samantha/learning/skills/` storing optimized descriptions and few-shot examples |
...
samantha skill install hermes:apple-notes

# Bulk install a category
samantha skill sync hermes --category research

# Run a skill directly
samantha skill run math-solver -a expression="41 + 82"

# See skill details
samantha skill info research-and-summarize
```
...
samantha skill sync hermes --category research
samantha skill sync hermes --category coding
samantha skill sync hermes  # everything (~150 skills)
```
...
samantha skill install openclaw:0xv4l3nt1n3/etherscan

# Bulk install with search filter
samantha skill sync openclaw --search "web3|crypto"
```

### From Any GitHub Repo

```bash
samantha skill install github:user/repo/path/to/skill --url https://github.com/user/repo
```
...
```bash
samantha skill install github:Xquik-dev/hermes-tweet/skills/hermes-tweet --url https://github.com/Xquik-dev/hermes-tweet
```

### Config-Driven Auto Import

Add sources to `~/.samantha/config.toml` for automatic syncing:
...
samantha skill sources

# Update all configured sources
samantha skill update
```
...
Samantha can automatically mine your trace history for recurring tool sequences and surface them as candidate skills:

```bash
# Preview discovered patterns without writing
samantha skill discover --dry-run --min-frequency 3

# Write discovered skills to ~/.samantha/skills/discovered/
samantha skill discover
```

Discovered skills land in `~/.samantha/skills/discovered/` and automatically appear in `samantha skill list` on the next session.
...
samantha optimize skills --dry-run

# Run DSPy optimization
samantha optimize skills --policy dspy --min-traces 3

# Run GEPA evolutionary optimization
samantha optimize skills --policy gepa --min-traces 3

# Inspect what optimization produced
samantha skill show-overlay research-and-summarize
```

Optimization results are stored as sidecar overlays at `~/.samantha/learning/skills/<skill-name>/optimized.toml`. They override the skill's description and add few-shot examples to the agent's system prompt. The original skill files are never modified.
...
samantha bench skills

# Smoke test: 4 conditions × 1 seed × 5 tasks
samantha bench skills --max-samples 5 --seeds 42

# Single condition
samantha bench skills --condition skills_optimized_dspy
```
...
|------|--------|-------------|---------|
| **Bundled** | Ships with Samantha | Implicit trust | Full access within declared capabilities |
...
```bash
samantha skill install hermes:arxiv --with-scripts
```
...
skills_dir = "~/.samantha/skills/"  # where skills are installed
...
overlay_dir = "~/.samantha/learning/skills/"
```
...
2. **User** `~/.samantha/skills/`
3. **Bundled** (shipped with Samantha)

## CLI Reference

| Command | Description |
|---------|-------------|
| `samantha skill list` | List installed skills |
| `samantha skill info <name>` | Show detailed skill information |
| `samantha skill run <name> [-a key=value]` | Execute a skill directly |
| `samantha skill install <source>:<name>` | Install from Hermes, OpenClaw, or GitHub |
| `samantha skill sync [<source>] [--category C]` | Bulk install + update from sources |
| `samantha skill sources` | List configured skill sources |
| `samantha skill update` | Pull latest from configured sources |
| `samantha skill remove <name>` | Remove an installed skill |
| `samantha skill search <query>` | Search the skill index |
| `samantha skill discover [--dry-run]` | Mine traces for recurring tool patterns |
| `samantha skill show-overlay <name>` | Inspect optimization output for a skill |
| `samantha optimize skills [--policy dspy\|gepa]` | Optimize skill descriptions + few-shot examples |
| `samantha bench skills [--condition C]` | Run the PinchBench skills benchmark |
