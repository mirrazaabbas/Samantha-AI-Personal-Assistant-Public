...
The channels module lets Samantha send and receive messages through external messaging platforms. Each platform has a dedicated channel implementation that connects directly to the platform's API -- there is no intermediate gateway.
...
from samantha.channels.telegram import TelegramChannel
...
from samantha.channels._stubs import ChannelMessage
from samantha.channels.discord_channel import DiscordChannel
...
from samantha.channels.slack import SlackChannel
...
from samantha.core.events import EventBus, EventType
from samantha.channels.telegram import TelegramChannel
...
The `samantha channel` subcommand group provides quick access to channel operations.
...
samantha channel send telegram "Build completed successfully"
```
...
When `samantha serve` is running, three channel endpoints are available. Channels must be configured and enabled in `[channel]` for these endpoints to return data.
...
Channel settings live in the `[channel]` section of `~/.samantha/config.toml`. Each platform has its own nested sub-section.

```toml title="~/.samantha/config.toml"
[channel]
enabled = true
default_channel = ""
...
# Send a message
if channels:
    channel.send(channels[0], "Hello from Samantha!")
...
```
Your phone  ──text──▶  SendBlue  ──webhook──▶  ngrok tunnel  ──▶  Samantha
                                                                       │
Your phone  ◀──iMessage──  SendBlue  ◀──API call──  DeepResearch agent ◀┘
```

When someone texts the SendBlue number, SendBlue POSTs the message to your webhook. Samantha sends an immediate "Message received!" acknowledgment, runs the DeepResearch agent, and sends the response back via iMessage.
...
Since Samantha runs locally, you need a tunnel so SendBlue can reach your webhook:
...
from samantha.channels.sendblue import SendBlueChannel
...
# Send a message
ok = channel.send("+15551234567", "Hello from Samantha!")
```
...
samantha channel status --channel-type sendblue

# Send a message
samantha channel send sendblue "+15551234567" "Hello from Samantha!"
```

### Server Restart Behavior

SendBlue bindings are **automatically restored on server restart**. When `samantha serve` starts:
...
!!! warning "Node.js 22+ required"
    The Baileys bridge is a compiled Node.js application bundled inside the package. It is auto-installed to `~/.samantha/whatsapp_baileys_bridge/` on first `connect()` call. If `node` is not found on `PATH`, `connect()` logs an error and sets the channel to `ChannelStatus.ERROR`.
...
from samantha.channels.whatsapp_baileys import WhatsAppBaileysChannel

channel = WhatsAppBaileysChannel(
    assistant_name="Samantha",           # (1)!
...
On first connection, the bridge will print a QR code to the terminal. Scan it with the WhatsApp app on your phone to authenticate. Authentication state is saved to `~/.samantha/whatsapp_baileys_bridge/auth/` and reused on subsequent connections.
...
| Parameter                  | Type       | Default     | Description                                            |
|----------------------------|------------|-------------|--------------------------------------------------------|
| `auth_dir`                 | `str`      | `~/.samantha/whatsapp_baileys_bridge/auth` | Baileys auth state directory |
| `assistant_name`           | `str`      | `"Samantha"`  | Display name for the assistant                         |
...
auth_dir = "/home/user/.samantha/whatsapp_baileys_bridge/auth"
...
- [API Reference: Channels](../api-reference/samantha/channels/index.md) -- full class and type signatures
