...
Samantha can extend agent capabilities by connecting to external [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) servers. This allows agents to use tools provided by services like Home Assistant, databases, custom APIs, or any MCP-compatible server -- without writing custom tool code.

## How It Works

When Samantha starts, it reads the `[tools.mcp]` section in `config.toml`. For each configured server, it:
...
If a server is unreachable or returns an error, Samantha logs a warning and continues loading the remaining servers. One broken server does not prevent other tools from being available.
...
objects or one server object, which Samantha wraps in an array.
...
Samantha starts the process automatically, communicates via JSON-RPC over stdin/stdout, and terminates it on shutdown.
...
Used when the `command` field is set. Samantha spawns the command as a subprocess and communicates via JSON-RPC lines on stdin/stdout.
...
Samantha handles MCP server failures gracefully:
...
No single server failure causes Samantha to crash or prevents other tools from working.
...
2. Verify the inline `servers` JSON is valid, or that the configured JSON file
   ...
   JSON string.
3. Check the Samantha logs for warnings like `Failed to discover external MCP tools`.

### Connection refused / timeout

1. Verify the server is running and reachable from the Samantha host: `curl -v http://host:port/`.
2. Check firewall rules between the Samantha container and the MCP server.
...
2. Check stderr output in the Samantha logs for error messages from the subprocess.
