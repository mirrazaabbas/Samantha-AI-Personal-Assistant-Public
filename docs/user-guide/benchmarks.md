...
Samantha ships with two benchmarks:
...
from samantha.bench._stubs import BenchmarkResult
...
from samantha.bench.latency import LatencyBenchmark
from samantha.bench.throughput import ThroughputBenchmark
...
samantha bench run -n 50

# Run only the latency benchmark
samantha bench run -b latency

# Run only the throughput benchmark with 20 samples
samantha bench run -b throughput -n 20

# Specify model and engine
samantha bench run -m qwen3:8b -e ollama

# Output JSON summary to stdout
samantha bench run --json

# Write JSONL results to a file
samantha bench run -o results.jsonl

# Combine options
samantha bench run -b latency -n 100 -m qwen3:8b --json -o latency.jsonl
```
...
```python
import time
from samantha.bench._stubs import BaseBenchmark, BenchmarkResult
...
```bash
samantha bench run -b context_length
```
