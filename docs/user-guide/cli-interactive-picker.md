...
Optional terminal UX for `samantha chat` and bare `samantha` on an interactive TTY.
...
- `samantha chat` without bare `samantha` (picker only auto-runs on bare `samantha` unless `--pick-model`)
- `samantha chat --pick-model` forces the list
...
| Command | Role |
|---------|------|
| `samantha model list` | Inspect models without starting chat |
| `samantha chat -m MODEL` | Skip picker, fixed model |
| `samantha chat --num-ctx N --num-gpu N` | Skip runtime panel |
