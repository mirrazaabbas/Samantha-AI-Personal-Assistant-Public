...
echo '{"api_key": "YOUR_KEY", "location": "San Francisco,CA,US"}' > ~/.samantha/connectors/weather.json

# GitHub notifications (token from https://github.com/settings/tokens)
echo '{"token": "ghp_YOUR_TOKEN"}' > ~/.samantha/connectors/github.json

# News RSS (no auth needed — configure your feeds)
cat > ~/.samantha/connectors/news_rss.json << 'EOF'
{"feeds": [
...
```bash
CARTESIA_API_KEY="sk_car_..." samantha digest --fresh
```
...
```bash
samantha digest --fresh          # Generate a new digest now
samantha digest                  # Show today's cached digest
samantha digest --text-only      # Print text without audio
samantha digest --history        # Show past digests
samantha digest --schedule "0 6 * * *"   # Set daily schedule
samantha digest --schedule off   # Disable schedule
samantha digest --schedule       # Show current schedule
```

## Saying "Good morning"

When chatting with Samantha (via CLI, desktop, or browser), saying "Good morning" or "morning digest" automatically triggers the digest — no need to use the `digest` command explicitly.
...
The `persona` field loads a prompt file from `configs/samantha/prompts/personas/{name}.md`. The default `samantha` persona delivers briefings with dry British wit, prioritizes urgent items, and interprets health data as trends rather than raw numbers.

To create a custom persona, add a new `.md` file in the personas directory.

### News Feeds

Add any RSS or Atom feed to `~/.samantha/connectors/news_rss.json`:
...
```bash
samantha serve  # Start the server
...
**"No digest for today"** — Run `samantha digest --fresh` to generate one.

**Empty sections** — Check connector status with `samantha connect --list`. Ensure tokens haven't expired (Google/Spotify tokens expire after 1 hour and are auto-refreshed on next use).
