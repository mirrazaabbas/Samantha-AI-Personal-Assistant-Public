...
Samantha has two types of integrations:
...
```json
{
    "display_information": { "name": "Samantha" },
    "features": {
        "app_home": {
        ...
        "bot_user": { "display_name": "Samantha", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "Samantha" |
...
Slack serves two purposes in Samantha:
...
8. **(Optional) Set the app icon:**
   ...
   - Upload the [Samantha icon](https://github.com/mirrazaabbas/Samantha-AI-Personal-Assistant-Public/blob/main/assets/samantha-slack-icon.jpg)
...
**DM your agent:**
- In Slack, find **Samantha** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "Samantha"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @Samantha` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @Samantha` in the channel |
...
3. **Connect in Samantha:**
   ...
   - CLI: `uv run samantha connect notion`
...
3. **Connect in Samantha:**
   ...
   - CLI: `uv run samantha connect granola`
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the Samantha desktop app

2. Apple Notes is detected automatically when Full Disk Access is granted

Samantha searches an indexed snapshot rather than querying Notes.app live.
...
3. **Connect in Samantha:**
   ...
   - CLI: `uv run samantha connect outlook`
...
2. **Connect in Samantha:**
   - Desktop/Browser: Agents → Channels tab → Obsidian → paste the vault path
   - CLI: `uv run samantha connect obsidian --path /path/to/vault`
...
4. **Connect in Samantha:**
   ...
   - CLI: `uv run samantha connect dropbox`
...
| Issue | Solution |
|-------|----------|
| "Connected — no data synced yet" | The connector authenticated but hasn't synced. Try running `uv run samantha deep-research-setup --skip-chat` to trigger a sync. |
| Data seems stale | Connectors sync on demand. Run the setup command or click "Reconnect" to re-sync. |
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.samantha/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.samantha/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.
