...
Samantha API server. It builds `deploy/docker/Dockerfile`, binds the server to
Render's default web-service port (`10000`), and uses OpenAI's `gpt-4o-mini`
through the cloud engine.

## Create the service

1. Open Render's **New Blueprint Instance** flow and select your Samantha
   fork.
...
4. Retrieve the generated `SAMANTHA_API_KEY` from the Render dashboard and
...
for all of them. To use another provider, install its corresponding Samantha
...
!!! warning "Free instances do not preserve Samantha data"
    Render's free web-service filesystem is ephemeral. Everything written to
    `/home/samantha/.samantha` — including configuration, credentials,
...
persistent disk at `/home/samantha/.samantha`, or configure the relevant
...
preserve the default Samantha home.
