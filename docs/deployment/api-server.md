...
Samantha includes an OpenAI-compatible API server built on FastAPI and uvicorn. It exposes chat completion, model listing, and health check endpoints, making it a drop-in replacement for the OpenAI API when working with local models.
...
Blueprint is intended for evaluation only because local Samantha state is
ephemeral.

The server reads defaults from `~/.samantha/config.toml` and auto-detects available engines and models. Override any option via CLI flags:
...
!!! warning "Server dependency check"
    If the `[server]` extra is not installed, `samantha serve` exits with a clear error message explaining how to install the required dependencies.
...
When `SAMANTHA_API_KEY` or `[server.auth].api_key` is configured,
...
1. `samantha.auth.v1`
2. `samantha.key.b64url.<encoded-key>`, where `<encoded-key>` is the unpadded
   base64url encoding of the API key's UTF-8 bytes

The server selects `samantha.auth.v1` in its handshake response. The built-in
...
The `[server]` section of `~/.samantha/config.toml` controls default server behavior:
...
CLI flags override config file values. For example, `samantha serve --port 9000` overrides the `port` setting in the config file.
...
For production deployments, run Samantha behind a reverse proxy like Nginx or Caddy for TLS termination, rate limiting, and authentication.
...
server {
    listen 443 ssl;
    server_name samantha.example.com;

    ssl_certificate /etc/ssl/certs/samantha.pem;
    ssl_certificate_key /etc/ssl/private/samantha.key;
...
```bash
samantha serve --host 127.0.0.1 --port 8000
```
