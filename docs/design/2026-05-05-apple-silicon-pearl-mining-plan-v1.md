...
**Architecture:** New provider `CpuPearlProvider` in `src/samantha/mining/cpu_pearl.py`, registered as `"cpu-pearl"` via `MinerRegistry`. The provider's `start()` launches two subprocesses: (1) Pearl's `pearl-gateway` Python service (talks to user's `pearld`), and (2) a small Python miner-loop that polls gateway for `getMiningInfo`, calls `pearl_mining.mine()`, and submits via `submitPlainProof`. Reuses Spec A's `MiningProvider` ABC, `MinerRegistry`, sidecar at `~/.samantha/runtime/mining.json`, telemetry adapter, and CLI surface (`samantha mine init|start|stop|status|doctor`) — all unchanged.
...
src/samantha/mining/__init__.py  # Soft-import cpu_pearl
src/samantha/mining/_constants.py  # Add PEARL_PINNED_REF and helper constants
```
...
**Files:**
- Modify: `src/samantha/mining/_constants.py:1-N` (created in Spec A)
...
Run: `cat src/samantha/mining/_constants.py`
...
Append to `src/samantha/mining/_constants.py`:
...
CPU_PEARL_LOCAL_CLONE_DIR = "~/.samantha/cache/pearl"
...
Run: `uv run ruff check src/samantha/mining/_constants.py`
Expected: no errors.

- [ ] **Step 4: Commit**

```bash
git add src/samantha/mining/_constants.py
...
**Files:**
- Create: `src/samantha/mining/_install.py`
...
```python
"""Tests for samantha.mining._install."""
...
def test_pearl_packages_available_returns_false_when_pearl_mining_missing():
    from samantha.mining import _install
...
def test_install_hint_is_actionable():
    ...
    from samantha.mining._install import install_hint
...
Expected: FAIL — `ImportError: cannot import name '_install' from 'samantha.mining'`
...
```bash
git add src/samantha/mining/_install.py tests/mining/test_install.py
...
**Files:**
- Modify: `src/samantha/mining/_install.py`
...
Run: `uv run pytest tests/mining/test_install.py -v`
Expected: FAIL — `AttributeError: module 'samantha.mining._install' has no attribute 'build_from_pin'`

- [ ] **Step 3: Implement `build_from_pin`**

Append to `src/samantha/mining/_install.py`:
...
Run: `uv run ruff check src/samantha/mining/_install.py tests/mining/test_install.py`
Expected: no errors.
...
**Files:**
- Create: `src/samantha/mining/_miner_loop_main.py`
...
```python
"""Tests for samantha.mining._miner_loop_main."""
from __future__ import annotations
...
def test_decode_mining_info_returns_header_and_target(fake_mining_info_response):
    from samantha.mining._miner_loop_main import _decode_mining_info
...
def test_encode_plain_proof_round_trips():
    ...
    from samantha.mining._miner_loop_main import _encode_plain_proof
...
def test_jsonrpc_envelope_shape():
    ...
    from samantha.mining._miner_loop_main import _make_request
...
Run with:
    python -m samantha.mining._miner_loop_main \
...
logger = logging.getLogger("samantha.mining.miner_loop")
...
Run: `uv run ruff check src/samantha/mining/_miner_loop_main.py tests/mining/test_miner_loop.py`
Expected: no errors.

- [ ] **Step 6: Commit**

```bash
git add src/samantha/mining/_miner_loop_main.py tests/mining/test_miner_loop.py
...
**Files:**
- Create: `src/samantha/mining/_pearl_subprocess.py`
...
```python
"""Tests for samantha.mining._pearl_subprocess."""
from __future__ import annotations
...
def test_launcher_start_spawns_two_processes(fake_popen, tmp_path):
    from samantha.mining._pearl_subprocess import PearlSubprocessLauncher
...
- ``samantha.mining._miner_loop_main`` (this repo), which polls the gateway
    ...
    def start(self, *, m: int, n: int, k: int, rank: int) -> None:
        ...
        miner_loop = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "samantha.mining._miner_loop_main",
                "--gateway-host",
                self.gateway_host,
                "--gateway-port",
...
Run: `uv run ruff check src/samantha/mining/_pearl_subprocess.py tests/mining/test_pearl_subprocess.py`
Expected: no errors.

- [ ] **Step 6: Commit**

```bash
git add src/samantha/mining/_pearl_subprocess.py tests/mining/test_pearl_subprocess.py
...
**Files:**
- Modify: `src/samantha/mining/_pearl_subprocess.py`
...
Run: `find ~/.samantha/cache/pearl/miner/pearl-gateway -name config.py -path '*pearl_gateway*' 2>/dev/null || find / -name config.py -path '*pearl_gateway*' 2>/dev/null | head -3`

Expected: `pearl-gateway/src/pearl_gateway/config.py` (path depends on where Pearl was cloned). If the file isn't on disk yet, run `python -m samantha.mining._install` (added in Task 4) or clone Pearl manually first.
...
Open `src/samantha/mining/_pearl_subprocess.py` and replace each guessed env name with the actual one. Keep the dict structure; only the keys change.
...
```bash
git add src/samantha/mining/_pearl_subprocess.py
...
**Files:**
- Create: `src/samantha/mining/cpu_pearl.py`
...
```python
"""Tests for samantha.mining.cpu_pearl.CpuPearlProvider."""
from __future__ import annotations
...
def darwin_apple_hw():
    ...
    from samantha.mining._stubs import HardwareInfo, GpuInfo
...
def windows_hw():
    ...
    from samantha.mining._stubs import HardwareInfo

    return HardwareInfo(platform="win32", cpu_arch="x86_64", gpu=None)


def test_detect_supported_on_apple_silicon(darwin_apple_hw):
    from samantha.mining.cpu_pearl import CpuPearlProvider

    with patch(
        "samantha.mining._install.pearl_packages_available", return_value=True
    ):
...
def test_detect_unsupported_when_pearl_not_installed(darwin_apple_hw):
    ...
    with patch(
        "samantha.mining._install.pearl_packages_available", return_value=False
    ):
...
Run: `uv run ruff check src/samantha/mining/cpu_pearl.py tests/mining/test_cpu_pearl.py`
Expected: no errors.

- [ ] **Step 6: Commit**

```bash
git add src/samantha/mining/cpu_pearl.py tests/mining/test_cpu_pearl.py
...
**Files:**
- Modify: `src/samantha/mining/cpu_pearl.py`
...
def test_start_writes_sidecar_and_returns_running(darwin_apple_hw, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "samantha.mining.cpu_pearl._sidecar_path",
    ...
    with patch(
        "samantha.mining.cpu_pearl.PearlSubprocessLauncher",
...
Replace `src/samantha/mining/cpu_pearl.py` with the full implementation:
...
def _sidecar_path() -> Path:
    return Path(os.path.expanduser("~/.samantha/runtime/mining.json"))


def _log_dir() -> Path:
    return Path(os.path.expanduser("~/.samantha/logs/mining"))
...
Run: `uv run ruff check src/samantha/mining/cpu_pearl.py`
Expected: no errors.
...
**Files:**
- Modify: `src/samantha/mining/__init__.py`
...
Run: `cat src/samantha/mining/__init__.py`
...
Append to `src/samantha/mining/__init__.py` (or modify if Spec A already defined `ensure_registered`):

```python
def _register_cpu_pearl() -> None:
    """Register CpuPearlProvider in MinerRegistry. Idempotent."""
    from samantha.core.registry import MinerRegistry  # type: ignore
...
# Call at import time so the registry is populated as soon as samantha.mining
...
```bash
git add src/samantha/mining/__init__.py
...
Samantha can mine the [Pearl](https://github.com/pearl-research-labs/pearl) chain
...
```bash
samantha mine init    # OJ will detect the missing wheels and offer to build
...
# check live status
samantha mine status

# capability matrix (great when something goes wrong)
samantha mine doctor

# stop mining
samantha mine stop

# tail logs
samantha mine logs -f
```
...
|---|---|---|
| `mine doctor` says `Pearl Python packages not installed` | Wheels not built yet | Run `samantha mine init` |
| `pearl-gateway` log shows `connection refused` to `http://localhost:44107` | `pearld` not running | Start `pearld` per Pearl's README |
| `mine status` shows `last_error: gateway metrics unreachable` | `pearl-gateway` crashed | Check `~/.samantha/logs/mining/pearl-gateway.log` |
...
For anything not on this list, capture `~/.samantha/logs/mining/` and open
an issue at https://github.com/mirrazaabbas/Samantha-AI-Personal-Assistant-Public/issues.
...
def test_provider_runs_end_to_end_on_this_host(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "samantha.mining.cpu_pearl._log_dir",
...
- `MiningProvider` ABC at `src/samantha/mining/_stubs.py`
- `MinerRegistry` class at `src/samantha/core/registry.py`
- `MiningConfig`, `MiningCapabilities`, `MiningStats`, `HardwareInfo` dataclasses
- Sidecar location convention `~/.samantha/runtime/mining.json`
