...
### In scope (v1)
- New `samantha.mining` subsystem with `MiningProvider` ABC, `MinerRegistry`, `MiningCapabilities` / `MiningConfig` / `MiningStats` dataclasses
- `vllm-pearl` provider implementation: orchestrate Pearl's published `vllm-miner` Docker container
- `[mining]` TOML section in OJ config; `MiningConfig` field in `SamanthaConfig`
- New CLI namespace: `samantha mine init|start|stop|status|doctor|attach|logs`
- Runtime sidecar at `~/.samantha/runtime/mining.json` for engine ↔ mining handoff
...
| Engine attachment | Runtime sidecar JSON at `~/.samantha/runtime/mining.json` | Existing vLLM engine class stays untouched. Sidecar is the single source of truth tying mining lifecycle to engine resolution. Inspectable via `cat`. |
...
src/samantha/cli/
    mine_cmd.py          # samantha mine init|start|stop|status|doctor|attach|logs
...
`MinerRegistry` added to `src/samantha/core/registry.py` as a peer to `EngineRegistry`, `AgentRegistry`, etc. `tests/conftest.py`'s autouse `_clean_registries` fixture is updated to include `MinerRegistry.clear()`.
...
[mining.extra]
docker_image_tag         = "samantha/pearl-miner:<pinned-ref>"
...
`~/.samantha/runtime/mining.json` (created on `mine start`, removed on `mine stop`):
...
1. `samantha mine start` → `MinerRegistry.get("vllm-pearl").start(config)`.
...
4. `samantha ask` and the SDK route to that endpoint transparently. The user's normal inference is the mining work.
...
Power users running their own Pearl container skip `samantha mine start` and write the sidecar themselves via `samantha mine attach --vllm-endpoint=... --gateway-url=...`. Decouples lifecycle from wiring.
...
| Command | Purpose |
|---|---|
| `samantha mine init` | Interactive: hardware/Docker checks, prompt for wallet + pearld credentials, write `[mining]`, pull/build image. Does NOT start mining. Pre-checks `>=200 GB` free disk. |
| `samantha mine start` | Launch container via the registered provider, write sidecar, print endpoint info. Idempotent if running. |
| `samantha mine stop` | Stop container, remove sidecar. Idempotent if not running. |
| `samantha mine status` | Read sidecar + query gateway `:8339/metrics`. Print `MiningStats`. |
| `samantha mine doctor` | Capability matrix; every check ✓/✗ with reason. Works in any state. |
| `samantha mine attach` | Manual mode: write sidecar without launching. |
| `samantha mine logs [-f]` | Tail container logs through Docker SDK. |
...
Image
  samantha/pearl-miner:<ref>   present (built 2026-04-30)   ✓
Pearl node
...
- Existing `com.samantha.gateway` daemon is unchanged.
...
- launchd/systemd installation surface (`samantha daemon install`) untouched.

### 6.5 Concurrency

POSIX `flock` on `~/.samantha/runtime/mining.lock` prevents racing `mine start` invocations.

### 6.6 `samantha ask` UX hint

When `[mining]` is configured but no sidecar exists, `cli/hints.py` emits one line: `"mining configured but not running — start it with \`samantha mine start\`"`. One-line UX nudge, no new infrastructure.
...
| **Build-from-pin** | OJ git-clones Pearl at a pinned ref into `~/.samantha/cache/pearl/`, then `docker buildx build` | v1 default (Pearl publishes nothing today) |
...
- **`auto_remove=False`** so a crashed container stays around for `samantha mine logs` post-mortem.
...
- `samantha mine init` triggers `ensure_image()`, streams build/pull output through the CLI with a clear time estimate (`"Building Pearl miner image — first run takes ~45 min on a fast machine"`).
- `samantha mine doctor` reports `image: present (tag, age, sha)` or `image: missing (run mine init)`.
- `samantha mine prune` (v1.x) cleans old `samantha/pearl-miner:*` tags. Manual `docker image rm` works in v1.
...
- **v1: on-demand only.** `samantha mine status` makes one HTTP GET per call (~10 ms). No background polling.
...
- `samantha telemetry stats --mining` (v1.x) joins to the latest `MiningStats` snapshot and reports `tokens / share`, `joules / share`, `est. PRL / kWh`.
...
**1. `submit_target` parsed into a tagged union; only one variant works.** `SoloTarget` accepted at runtime in v1; `PoolTarget` raises `NotImplementedError("pool support is v2 — track samantha#XYZ")`. Reachable only by users who edit their config to opt in.
...
| Concurrent `mine start` | POSIX `flock` on `~/.samantha/runtime/mining.lock`; second invocation errors clearly | clear message |
