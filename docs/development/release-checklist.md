...
2. **`samantha` (no args)** drops into a chat session within 2 s.
...
6. **`samantha doctor`** exits 0 once all bg work completes; shows the Background tasks table.
7. **Re-run `curl … | bash`** on the same machine. It completes ≤ 30 s, says `[ok] step already done` for every step.
8. **`samantha-uninstall`** removes `~/.samantha/` and `~/.local/bin/samantha*`. Verify with `ls`.
...
- Delete `~/.samantha/config.toml` — verify bare `samantha` re-runs init.
- Delete `~/.samantha/.venv` — verify re-running curl heals it.
