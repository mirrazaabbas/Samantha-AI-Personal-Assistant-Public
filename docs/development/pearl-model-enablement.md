...
through Pearl's vLLM miner and Samantha.

Samantha can point `vllm-pearl` at a model id, but a raw Hugging Face model is
...
Samantha only supports Pearl models published by the `pearl-ai` Hugging Face
organization. Private staging artifacts and Samantha-specific conversion
...
`samantha mine start`, vLLM `/v1/models`, Samantha inference routing, Pearl
gateway template refresh, and `samantha mine validate-model`.

`pearl-ai/Gemma-4-31B-it-pearl` and
`pearl-ai/Llama-3.1-8B-Instruct-pearl` are listed because they are public Pearl
org artifacts. They remain `planned` in Samantha until we have clean
...
2. Convert the target model.
   ...
   Samantha includes an experimental local converter for this work:
   ...
   its output as a staging artifact until `samantha mine inspect-model` and
   `samantha mine validate-model` pass on H100/H200 hardware.
...
3. Validate the Pearl vLLM plugin path.
   - Run `samantha mine inspect-model --model <pearl-model-id>
...
4. Validate chain integration.
   ...
   - `samantha mine status` parses those metrics.

5. Promote the model in Samantha.
...
```text
src/samantha/mining/_models.py
```

`samantha mine models` renders that registry. Planned models are visible to users
...
- `samantha mine inspect-model --model <pearl-model-id> --allow-planned`
- `samantha mine init --model <pearl-model-id>`
- `samantha mine start`
- `curl http://127.0.0.1:8000/v1/models`
- `samantha ask "Say hello in one sentence."`
- `samantha mine status`
- `samantha mine validate-model --model <pearl-model-id> --allow-planned --prompt
...
Attach the JSON artifact from `samantha mine validate-model --output` to the issue.
