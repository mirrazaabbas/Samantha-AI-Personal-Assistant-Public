...
prove Samantha wiring; this validates that a real H100/H200 host can mine
through Pearl and serve inference through Samantha.
...
- Docker container `samantha-pearl-miner` starts.
- `~/.samantha/runtime/mining.json` is written.
...
```bash
docker ps --filter name=samantha-pearl-miner
cat ~/.samantha/runtime/mining.json
uv run samantha mine logs --tail 200
...
## Verify Samantha Inference Uses Mining Endpoint

Run:

```bash
uv run samantha mine doctor
uv run samantha ask "Say hello in one sentence."
```
...
- If metric names differ, attach `/tmp/pearl-gateway-metrics.txt` to the PR and
  update `src/samantha/mining/_metrics.py`.
...
```bash
uv run samantha mine stop
docker ps --filter name=samantha-pearl-miner
test ! -e ~/.samantha/runtime/mining.json
```
...
- Container stops.
- Sidecar is removed.
- `samantha ask` no longer routes through `vllm-pearl-mining` unless another
...
- Samantha inference succeeds through `vllm-pearl-mining`.
...
```bash
uv run samantha mine doctor
uv run samantha mine status || true
uv run samantha mine logs --tail 300 || true
docker inspect samantha-pearl-miner || true
