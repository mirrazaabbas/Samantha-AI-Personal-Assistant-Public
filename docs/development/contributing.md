...
```bash
uv run samantha --version   # Should print 0.1.0
uv run samantha --help      # Show all subcommands
```
...
Samantha uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=samantha --cov-report=html
```
...
Samantha uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/samantha/`:
...
src/samantha/
    ...
    core/                       # Shared infrastructure
        config.py               # SamanthaConfig, hardware detection, TOML loader
    ...
    cli/                        # Click CLI commands
        ...
        ask.py                  # samantha ask
        init_cmd.py             # samantha init
        model.py                # samantha model list/info
        memory_cmd.py           # samantha memory index/search/stats
        telemetry_cmd.py        # samantha telemetry stats/export/clear
        bench_cmd.py            # samantha bench run
        serve.py                # samantha serve
```
...
try:
    import samantha.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```
