---
title: Configuration
description: Complete reference for Samantha configuration
---

# Configuration

Samantha uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
```
~/.samantha/config.toml
```

Samantha creates the `~/.samantha/` directory and populates it with a default config when you run `samantha init`.

## Relocating the Samantha directory

Samantha keeps **all** of its state — config, databases, caches, logs,
...
`~/.samantha`, but you can move it.

The root is resolved in priority order:

1. **`$SAMANTHA_HOME`** — explicit override. Honored by both the installer
   and the Python runtime.
2. **`$XDG_DATA_HOME/samantha`** — used when `$XDG_DATA_HOME` is set (a single
   `samantha` directory nested under it, per the XDG Base Directory spec).
3. **`~/.samantha`** — the default. With no environment variables set, the
...
```bash
samantha config path
```

!!! note "Migration"
    Because the default is unchanged, **no data migration is required** for
    existing installs. If you set `SAMANTHA_HOME` (or `XDG_DATA_HOME`) on a
    machine that already has data in `~/.samantha`, Samantha will look in
    the new location and not see your old data — move it yourself if you want
    to keep it: `mv ~/.samantha "$SAMANTHA_HOME"`.

`$SAMANTHA_CONFIG` still points at an explicit `config.toml` file
...
3. Writes `~/.samantha/config.toml` with sensible defaults
...
|-------|------|---------|-------------|
| `default` | string | Auto-detected | Default engine backend. One of: `ollama`, `vllm`, `llamacpp`, `sglang`, `cloud`. Set automatically by `samantha init` based on hardware detection. |
...
!!! tip "Engine fallback"
    If the configured default engine is unreachable, Samantha automatically probes all registered engines and falls back to any healthy one.
...
When both `default_model` and `fallback_model` are empty, Samantha uses the configured router policy (see `[learning]`) to select a model from those available on the active engine.
...
```bash
samantha ask --router heuristic "Hello"
```
...
| `db_path` | string | `~/.samantha/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend. |
...
Controls the OpenAI-compatible API server started by `samantha serve`.
...
```bash
samantha serve --host 127.0.0.1 --port 9000 --model qwen3:8b --agent simple
```
...
[traces]
enabled = false
db_path = "~/.samantha/traces.db"
```

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `false` | Whether to record traces for each agent interaction. |
| `db_path` | string | `~/.samantha/traces.db` | Path to the SQLite trace database. |
...
| `skills_dir` | string | `~/.samantha/skills/` | Directory where skills are installed. |
...
| `overlay_dir` | string | `~/.samantha/learning/skills/` | Where optimized skill overlays are stored. |
...
When you run `samantha init`, Samantha probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.samantha/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure Samantha entirely from Python without a TOML file:

```python
from samantha import Samantha
from samantha.core.config import (
    AgentConfig,
    EngineConfig,
    IntelligenceConfig,
    SamanthaConfig,
...
```python
j = Samantha(config_path="/path/to/my-config.toml")
```
...
Samantha respects the following environment variables:
