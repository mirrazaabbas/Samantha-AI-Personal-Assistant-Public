...
struct BootPlan {
    ...
    /// ~/.samantha/config.toml so `samantha serve` picks it up.
    engine_host: Option<(String, String)>,
    /// Args appended after `uv run samantha serve --port <port>`.
...
fn boot_plan(cfg: &InferenceConfig, ram_gb: f64) -> BootPlan {
    match cfg.kind {
        ...
        SourceKind::Custom => {
            ...
            // `model` may be empty if the config is malformed; `samantha serve`
...
/// Find the Samantha project root (contains pyproject.toml).
/// Checks SAMANTHA_ROOT env var, walks up from the executable, then
/// probes common clone locations.
fn find_project_root() -> Option<std::path::PathBuf> {
    // 1. Explicit env var override
    if let Ok(root) = std::env::var("SAMANTHA_ROOT") {
    ...
    let home = home_dir();
    let direct = [
        format!("{home}/Samantha"),
        format!("{home}/projects/hazy/Samantha"),
        format!("{home}/projects/Samantha"),
        format!("{home}/src/Samantha"),
        format!("{home}/Documents/Samantha"),
        format!("{home}/Desktop/Samantha"),
        format!("{home}/Developer/Samantha"),
        format!("{home}/dev/Samantha"),
        format!("{home}/Code/Samantha"),
        format!("{home}/code/Samantha"),
        format!("{home}/repos/Samantha"),
        format!("{home}/github/Samantha"),
    ];
    for p in &direct {
    ...
    // 4. Shallow scan: look for Samantha one level inside common parent dirs.
    //    This catches clones like ~/Documents/my-stuff/Samantha without
    ...
    for parent in &scan_parents {
        ...
        if let Ok(entries) = std::fs::read_dir(&parent_path) {
            for entry in entries.flatten() {
                let candidate = entry.path().join("Samantha");
                if candidate.join("pyproject.toml").exists() {
                    return Some(candidate);
                }
                // Also check if the entry itself is Samantha (case-insensitive match)
                if let Some(name) = entry.file_name().to_str() {
                    if name.eq_ignore_ascii_case("samantha")
...
// ---------------------------------------------------------------------------
// BackendManager — owns the Ollama + Samantha server child processes
// ---------------------------------------------------------------------------
...
/// Rolling buffer holding the most recent ~16 KB of samantha stderr.
///
/// Populated by a background drainer task spawned at boot so the pipe
/// never fills and back-pressures `samantha serve`; consumed by the boot
...
struct BackendManager {
    ...
    samantha: Option<ChildHandle>,
    samantha_stderr_tail: StderrTail,
}
...
impl Default for BackendManager {
    fn default() -> Self {
        Self {
            ollama: None,
            samantha: None,
            samantha_stderr_tail: Arc::new(Mutex::new(Vec::new())),
        }
    }
}
...
impl BackendManager {
    async fn stop_all(&mut self) {
        if let Some(ref mut h) = self.samantha {
            h.kill().await;
        }
        self.samantha = None;
...
/// Outcome of waiting for `samantha serve` to become healthy.
///
...
fn spawn_samantha_stderr_drainer(mut stderr: tokio::process::ChildStderr, tail: StderrTail) {
...
async fn read_samantha_stderr_tail(backend: &SharedBackend) -> String {
    let tail = backend.lock().await.samantha_stderr_tail.clone();
    let bytes = tail.lock().await.clone();
    String::from_utf8_lossy(&bytes).trim().to_string()
}

/// Poll `samantha serve` health, watching the child process state so we
/// never wait 10 minutes for a process that crashed in the first second.
async fn wait_for_samantha_health(
    url: &str,
    timeout: Duration,
    ...
    {
        Ok(c) => c,
        Err(_) => return SamanthaStartResult::Timeout,
    };
    let deadline = tokio::time::Instant::now() + timeout;
    loop {
        ...
        let exit_status = {
            ...
            match mgr.samantha.as_mut() {
                Some(h) => h.child.try_wait().ok().flatten(),
                None => None,
            }
        };
        if let Some(status) = exit_status {
            let stderr = read_samantha_stderr_tail(backend).await;
            return SamanthaStartResult::EarlyExit {
                code: status.code(),
                stderr,
            };
        }
        ...
        match client.get(url).send().await {
            Ok(resp) => {
                ...
                if status.is_success() {
                    return SamanthaStartResult::Ready;
                }
                if status == reqwest::StatusCode::SERVICE_UNAVAILABLE {
                    ...
                    return SamanthaStartResult::ServiceUnavailable(body);
                }
        ...
        if tokio::time::Instant::now() >= deadline {
            return SamanthaStartResult::Timeout;
        }
        tokio::time::sleep(Duration::from_millis(500)).await;
    }
}
...
///
/// When the Samantha desktop binary is shipped as an AppImage, the AppImage
...
fn rust_toolchain_install_hint() -> &'static str {
    "The desktop app needs the Rust toolchain to build `samantha_rust`. \
...
fn format_extension_import_failure(root: &std::path::Path, stderr: &str) -> String {
    ...
    format!(
        "`samantha_rust` is still not importable after building. Last output:\n\n{}\n\n\
         Run these manually for the full build log:\n\n\
           cd {}\n\
           {}\n\
           uv run python -c \"import samantha_rust\"",
        if tail.is_empty() {
            "(no stderr output)"
        } else {
            &tail
...
async fn verify_samantha_rust_extension(
    root: &std::path::Path,
    uv_bin: &str,
) -> Result<(), String> {
    let mut cmd = tokio::process::Command::new(uv_bin);
    cmd.args(["run", "python", "-c", "import samantha_rust"])
    ...
    match cmd.output().await {
        ...
        Err(e) => Err(format!(
            "Could not verify `samantha_rust`: {}. Verify uv is installed at `{}`.",
            e, uv_bin
        )),
    }
}
...
fn port_owner_hint() -> String {
    if cfg!(target_os = "windows") {
        format!("netstat -ano | findstr :{}", SAMANTHA_PORT)
    } else {
        format!("lsof -i :{}", SAMANTHA_PORT)
    }
}
...
fn format_port_unavailable(port: u16, reason: &str) -> String {
    format!(
        "Port {} is not available: {}. Stop the process using that port or \
         change the Samantha port, then relaunch.\n\nTo identify it:\n  {}",
        port,
        reason,
        port_owner_hint(),
    )
...
fn check_samantha_port_available() -> Result<(), String> {
    match std::net::TcpListener::bind(("127.0.0.1", SAMANTHA_PORT)) {
        Ok(listener) => {
            drop(listener);
            Ok(())
        }
        Err(err) => Err(format_port_unavailable(SAMANTHA_PORT, &err.to_string())),
    }
}
...
async fn boot_backend(backend: SharedBackend, status: SharedStatus) {
    ...
    if plan.launch_ollama {
        ...
        // Point `samantha serve` at the user's endpoint by writing the engine
        // host into ~/.samantha/config.toml (the env var alone is shadowed by
    ...
    if project_root.is_none() {
        ...
        let target_path = std::path::PathBuf::from(home_dir()).join("Samantha");
        ...
        if target_path.exists() && !target_path.join("pyproject.toml").exists() {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "{} exists but is not a valid Samantha project. \
                 Remove it and relaunch, or set SAMANTHA_ROOT to the correct path.",
                clone_target,
            ));
            return;
        }
        ...
        {
            let mut s = status.lock().await;
            s.detail = "Downloading Samantha (first launch)...".into();
        }
        ...
        let clone_result = tokio::process::Command::new(&git_bin)
            .args([
                ...
                "--depth",
                "1",
                "https://github.com/mirrazaabbas/Samantha-AI-Personal-Assistant-Public.git",
                &clone_target,
            ])
            .stdout(std::process::Stdio::null())
            .stderr(std::process::Stdio::piped())
        ...
        match clone_result {
            Ok(child) => match child.wait_with_output().await {
                ...
                Ok(output) => {
                    ...
                    let mut s = status.lock().await;
                    s.error = Some(format!(
                        "Failed to download Samantha: {}. \
                         Clone manually: git clone https://github.com/mirrazaabbas/Samantha-AI-Personal-Assistant-Public.git {}",
                        stderr.trim(),
                        clone_target,
                    ));
                    return;
    ...
    // already started `samantha serve` in a terminal and then launched the
    ...
    // override from config instead of hard-coding SAMANTHA_PORT.
    {
        let client = reqwest::Client::builder()
            .timeout(Duration::from_secs(2))
            .build()
            .unwrap();
        match client
            .get(format!("http://127.0.0.1:{}/health", SAMANTHA_PORT))
            .send()
            .await
        {
            ...
            Ok(resp) if resp.status() == reqwest::StatusCode::SERVICE_UNAVAILABLE => {
                ...
                s.error = Some(format!(
                    "An API server is already running on port {} but its \
                     ...
                     `samantha serve`, wait for it to finish loading and relaunch. \
    ...
    if let Err(err) = check_samantha_port_available() {
        let mut s = status.lock().await;
        s.error = Some(err);
        return;
    }
    ...
    // the user saw no error, the boot continued, `uv run samantha serve`
    // then ran in an under-provisioned venv, and the user waited the full
    // 600s health-check window before getting "Samantha server did not
    ...
    sync_cmd
        .args([
            ...
            // samantha_rust lives in a uv dependency group (not the published
    ...
    {
        let mut s = status.lock().await;
        s.detail = "Verifying Rust extension (samantha_rust)...".into();
    }
    if let Err(err) = verify_samantha_rust_extension(root, &uv_bin).await {
        let mut s = status.lock().await;
        s.error = Some(err);
        return;
    }
    ...
    let samantha_child = cmd.spawn();

    match samantha_child {
        Ok(mut child) => {
            ...
            let mut mgr = backend.lock().await;
            let tail = mgr.samantha_stderr_tail.clone();
            mgr.samantha = Some(ChildHandle { child });
            drop(mgr);
            if let Some(stderr) = stderr_handle {
                spawn_samantha_stderr_drainer(stderr, tail);
            }
        }
        Err(e) => {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "Could not start samantha server: {}. \
                 Make sure uv is installed (https://astral.sh/uv) and the Samantha repo is cloned at {}",
                e,
                root.display(),
            ));
            return;
    ...
    let server_url = format!("http://127.0.0.1:{}/health", SAMANTHA_PORT);
    match wait_for_samantha_health(&server_url, Duration::from_secs(600), &backend).await {
        SamanthaStartResult::Ready => {}
        SamanthaStartResult::ServiceUnavailable(body) => {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "Samantha server is running but the inference engine is not available \
                 (HTTP 503). This usually means the configured model couldn't be loaded.\n\n\
                 Check the server logs, or run 'uv run samantha serve --port {}{}' \
        ...
        SamanthaStartResult::EarlyExit { code, stderr } => {
            ...
            s.error = Some(if stderr.is_empty() {
                format!(
                    "Samantha server exited (code {}) before becoming ready.\n\n\
                     No stderr output. Check that:\n\
                     1. uv is installed ({})\n\
                     2. The Samantha repo is at {}\n\
        ...
        SamanthaStartResult::Timeout => {
            let stderr = read_samantha_stderr_tail(&backend).await;
            let mut s = status.lock().await;
            s.error = Some(if stderr.is_empty() {
                format!(
                    "Samantha server did not become ready within 10 minutes. Check that:\n\
                     1. uv is installed ({})\n\
                     2. The Samantha repo is at {}\n\
                ...
                format!(
                    "Samantha server did not become ready within 10 minutes.\n\nStderr:\n{}",
                    stderr,
                )
            });
            return;
...
fn api_base() -> String {
    format!("http://127.0.0.1:{}", SAMANTHA_PORT)
}
...
#[tauri::command]
async fn run_samantha_command(args: Vec<String>) -> Result<String, String> {
    let uv_bin = resolve_bin("uv");

    let mut cmd_args = vec!["run".to_string(), "samantha".to_string()];
    cmd_args.extend(args.iter().cloned());

    let mut cmd = tokio::process::Command::new(&uv_bin);
    cmd.args(&cmd_args);
    // Run from the project root so `uv run samantha` resolves the Samantha
    // project regardless of the app's launch cwd. In a packaged install the
    // cwd isn't the checkout, so without this `samantha` isn't found and the
    ...
    if !is_serve {
        ...
        let output = cmd
            .output()
            .await
            .map_err(|e| format!("Failed to launch samantha: {}", e))?;
    ...
    // `samantha serve` is a long-running server that never exits. The old code
    ...
    let mut child = cmd
        .spawn()
        .map_err(|e| format!("Failed to launch samantha serve: {}", e))?;

    let tail: StderrTail = Arc::new(Mutex::new(Vec::new()));
    if let Some(stderr) = child.stderr.take() {
        spawn_samantha_stderr_drainer(stderr, tail.clone());
    }
    ...
    let url = format!("http://127.0.0.1:{}/health", SAMANTHA_PORT);
    ...
    loop {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            ...
            return Err(format!(
                "samantha serve exited (code {:?}) before becoming healthy:\n{}",
                status.code(),
                stderr.trim()
            ));
        }
        if let Ok(resp) = client.get(&url).send().await {
            if resp.status().is_success() {
                ...
                return Ok(format!(
                    "samantha serve is ready on http://127.0.0.1:{}",
                    SAMANTHA_PORT
                ));
            }
        }
        if tokio::time::Instant::now() >= deadline {
            return Err(format!(
                "samantha serve did not become healthy on port {} within 120s.",
                SAMANTHA_PORT
            ));
        }
        tokio::time::sleep(Duration::from_millis(500)).await;
...
const SECURE_KEY_SERVICE: &str = "Samantha Cloud Keys";
...
async fn reload_cloud_keys(keys: Vec<(String, String)>) {
    let reload_url = format!("http://127.0.0.1:{}/v1/cloud/reload", SAMANTHA_PORT);
...
// ---------------------------------------------------------------------------
// Inference-source selection (~/.samantha/inference.json)
// ---------------------------------------------------------------------------
...
/// Path to the inference-source config (~/.samantha/inference.json).
...
/// Write the custom-endpoint host into ~/.samantha/config.toml so
/// `samantha serve` (which reads that file via load_config) points at it.
...
mod native_overlay {
    ...
    pub unsafe fn create(html: &str, api_port: u16) {
        ...
        if Class::get("SamanthaOverlayPanel").is_none() {
            let sup = Class::get("NSPanel").unwrap();
            let mut decl = ClassDecl::new("SamanthaOverlayPanel", sup).unwrap();
        ...
        if Class::get("SamanthaOverlayNavDelegate").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("SamanthaOverlayNavDelegate", sup).unwrap();
        ...
        if Class::get("SamanthaOverlayMsgHandler").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("SamanthaOverlayMsgHandler", sup).unwrap();
        ...
        let cls = Class::get("SamanthaOverlayPanel").unwrap();
        ...
        let hcls = Class::get("SamanthaOverlayMsgHandler").unwrap();
        ...
        let nav_cls = Class::get("SamanthaOverlayNavDelegate").unwrap();
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let health = MenuItemBuilder::with_id("health", "Health: starting...")
                .enabled(false)
                .build(app)?;
            let quit = MenuItemBuilder::with_id("quit", "Quit Samantha").build(app)?;
            ...
            #[cfg(target_os = "macos")]
            unsafe {
                native_overlay::create(include_str!("overlay.html"), SAMANTHA_PORT);
            }
        ...
        .expect("error while building Samantha Desktop")
        .run(move |_app, event| {
...
mod tests {
    ...
    fn failure_message_includes_exit_code_and_tail_and_hint() {
        let msg = format_uv_sync_failure(
            Path::new("/home/u/.samantha/src"),
            Some(2),
            "error: failed to resolve numpy==2.1.3",
        );
        assert!(msg.contains("exit 2"));
        assert!(msg.contains("/home/u/.samantha/src"));
    ...
    fn missing_rust_toolchain_message_names_cargo_and_installer() {
        ...
        assert!(msg.contains("samantha_rust"));
    ...
    fn uv_sync_rust_failure_mentions_toolchain() {
        let msg = format_uv_sync_failure(
            Path::new("C:\\Users\\me\\Samantha"),
            Some(1),
            "maturin failed: linker `link.exe` not found while building samantha-rust",
        );
        assert!(msg.contains("exit 1"));
    ...
    fn extension_import_failure_names_verification_command() {
        let msg = format_extension_import_failure(
            Path::new("C:\\Users\\me\\Samantha"),
            "ModuleNotFoundError: No module named 'samantha_rust'",
        );
        assert!(msg.contains("samantha_rust"));
        assert!(msg.contains(DESKTOP_UV_SYNC_COMMAND));
        assert!(msg.contains("uv run python -c \"import samantha_rust\""));
