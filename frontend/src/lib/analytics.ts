...
/**
 * Mirror of the Python event catalog (src/samantha/analytics/events.py
    ...
    if (import.meta.env.DEV) {
      console.warn(
        `[analytics] Unknown event "${event}" dropped. ` +
          ...
          `REGISTRY in src/samantha/analytics/events.py.`,
      );
    }
    return;
  }
 ...
 * :func:`samantha.analytics.redaction.hash_id`.
 */
