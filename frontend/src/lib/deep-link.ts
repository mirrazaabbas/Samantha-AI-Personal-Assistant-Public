/**
 * deep-link.ts — Handler for samantha:// deep links in the Tauri desktop app.
 *
 ...
 *   samantha://research/{session_id}   → open a research session
 *   samantha://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `samantha://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `samantha://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `samantha:` scheme.
 *
 * @example
 * parseDeepLink("samantha://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("samantha://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "samantha:") return null;

    // The URL constructor treats "samantha://research/abc123" such that
