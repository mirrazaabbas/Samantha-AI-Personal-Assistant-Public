...
impl SQLiteMemory {
    pub fn new(db_path: &Path) -> Result<Self, SamanthaError> {
        ...
        let db_path = if db_path.starts_with("~") {
            let home = std::env::var("HOME").map_err(|_| {
                SamanthaError::Io(std::io::Error::other("HOME environment variable not set"))
            })?;
        ...
        if let Some(parent) = db_path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| SamanthaError::Io(std::io::Error::other(e)))?;
        }

        let conn = Connection::open(db_path)
            .map_err(|e| SamanthaError::Io(std::io::Error::other(e.to_string())))?;
    ...
    ) -> Result<Vec<String>, SamanthaError> {
...
impl MemoryBackend for SQLiteMemory {
    ...
    fn retrieve(&self, query: &str, top_k: usize) -> Result<Vec<RetrievalResult>, SamanthaError> {
        let conn = self.conn.lock();
