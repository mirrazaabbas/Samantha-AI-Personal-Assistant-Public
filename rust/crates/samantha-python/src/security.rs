...
pub struct PySecretScanner {
    inner: samantha_security::SecretScanner,
}
...
impl PySecretScanner {
    ...
    fn new() -> Self {
        Self {
            inner: samantha_security::SecretScanner::new(),
        }
    }
...
pub struct PyPIIScanner {
    inner: samantha_security::PIIScanner,
}
...
impl PyPIIScanner {
    ...
    fn new() -> Self {
        Self {
            inner: samantha_security::PIIScanner::new(),
        }
    }
...
pub struct PyGuardrailsEngine {
    inner: samantha_security::GuardrailsEngine<samantha_engine::Engine>,
}
...
impl PyGuardrailsEngine {
    ...
    fn new(
        ...
        let redaction_mode = match mode {
            "redact" => samantha_security::RedactionMode::Redact,
            "block" => samantha_security::RedactionMode::Block,
            _ => samantha_security::RedactionMode::Warn,
        };
        Ok(Self {
            inner: samantha_security::GuardrailsEngine::new(
                engine,
                redaction_mode,
                scan_input,
                scan_output,
...
pub struct PyAuditLogger {
    inner: parking_lot::Mutex<samantha_security::AuditLogger>,
}
...
impl PyAuditLogger {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        ...
        let inner = samantha_security::AuditLogger::new(&db_path)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self {
...
pub struct PyCapabilityPolicy {
    inner: samantha_security::CapabilityPolicy,
}
...
impl PyCapabilityPolicy {
    ...
    fn new(default_deny: bool) -> Self {
        Self {
            inner: samantha_security::CapabilityPolicy::new(default_deny),
        }
    }
...
pub struct PyInjectionScanner {
    inner: samantha_security::InjectionScanner,
}
...
impl PyInjectionScanner {
    ...
    fn new() -> Self {
        Self {
            inner: samantha_security::InjectionScanner::new(),
        }
    }
...
pub struct PyRateLimiter {
    inner: samantha_security::RateLimiter,
}
...
impl PyRateLimiter {
    ...
    fn new(requests_per_minute: u32, burst_size: u32) -> Self {
        Self {
            inner: samantha_security::RateLimiter::new(
                samantha_security::RateLimitConfig {
                    requests_per_minute,
                    burst_size,
                    enabled: true,
                },
...
pub struct PyTaintSet {
    inner: samantha_security::TaintSet,
}
...
impl PyTaintSet {
    ...
    fn new() -> Self {
        Self {
            inner: samantha_security::TaintSet::new(),
        }
    }

    fn add(&mut self, label: &str) {
        let taint_label = match label {
            "pii" => samantha_security::TaintLabel::Pii,
            "secret" => samantha_security::TaintLabel::Secret,
            "user_private" => samantha_security::TaintLabel::UserPrivate,
            "external" => samantha_security::TaintLabel::External,
            _ => samantha_security::TaintLabel::External,
        };
        // TaintSet is immutable-style; union with a single-label set.
        self.inner = self.inner.union(
            &samantha_security::TaintSet::from_labels(&[taint_label]),
        );
    }
