...
use crate::RUNTIME;
use samantha_agents::OjAgent;
use samantha_engine::rig_adapter::RigModelAdapter;
use samantha_engine::Engine;
use pyo3::prelude::*;
use std::sync::Arc;
...
enum AgentEnum {
    Simple(samantha_agents::SimpleAgent<DefaultAdapter>),
    Orchestrator(samantha_agents::OrchestratorAgent<DefaultAdapter>),
    NativeReAct(samantha_agents::NativeReActAgent<DefaultAdapter>),
}
...
impl AgentEnum {
    ...
    async fn run(
        &self,
        input: &str,
        context: Option<&samantha_core::AgentContext>,
    ) -> Result<samantha_core::AgentResult, samantha_core::SamanthaError> {
        match self {
...
fn make_adapter(engine_key: &str, model: &str) -> PyResult<DefaultAdapter> {
    let config = samantha_core::SamanthaConfig::default();
    let engine = samantha_engine::get_engine_static(&config, Some(engine_key))
...
impl PySimpleAgent {
    ...
    fn new(
        ...
        let agent = samantha_agents::SimpleAgent::new(adapter, system_prompt, temperature);
...
impl PyOrchestratorAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(samantha_tools::ToolExecutor::new(None, None));
        let agent = samantha_agents::OrchestratorAgent::new(
...
impl PyNativeReActAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(samantha_tools::ToolExecutor::new(None, None));
        let agent = samantha_agents::NativeReActAgent::new(
...
impl PyNativeOpenHandsAgent {
    ...
    fn new(
        ...
        let engine = samantha_engine::get_engine_static(&config, Some(engine_key))
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        let adapter = samantha_engine::rig_adapter::RigModelAdapter::new(
            Arc::new(engine),
            model.to_string(),
        );
        let executor = Arc::new(samantha_tools::ToolExecutor::new(None, None));
        let agent = samantha_agents::NativeOpenHandsAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
impl PyMonitorOperativeAgent {
    ...
    fn new(
        ...
        let mem_ext = match memory_extraction {
            "scratchpad" => samantha_agents::MemoryExtraction::Scratchpad,
            "structured_json" => samantha_agents::MemoryExtraction::StructuredJson,
            "none" => samantha_agents::MemoryExtraction::None,
            _ => samantha_agents::MemoryExtraction::CausalityGraph,
        };
        let obs_comp = match observation_compression {
            "truncate" => samantha_agents::ObservationCompression::Truncate,
            "none" => samantha_agents::ObservationCompression::None,
            _ => samantha_agents::ObservationCompression::Summarize,
        };
        let ret_strat = match retrieval_strategy {
            "keyword" => samantha_agents::RetrievalStrategy::Keyword,
            "semantic" => samantha_agents::RetrievalStrategy::Semantic,
            "none" => samantha_agents::RetrievalStrategy::None,
            _ => samantha_agents::RetrievalStrategy::HybridWithSelfEval,
        };
        let task_dec = match task_decomposition {
            "monolithic" => samantha_agents::TaskDecomposition::Monolithic,
            "hierarchical" => samantha_agents::TaskDecomposition::Hierarchical,
            _ => samantha_agents::TaskDecomposition::Phased,
        };

        let monitor_config = samantha_agents::MonitorConfig {
        ...
        let agent = samantha_agents::MonitorOperativeAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
pub struct PyLoopGuard {
    inner: samantha_agents::LoopGuard,
}
...
impl PyLoopGuard {
    ...
    fn new(max_identical: usize, max_ping_pong: usize, poll_budget: usize) -> Self {
        Self {
            inner: samantha_agents::LoopGuard::new(max_identical, max_ping_pong, poll_budget),
        }
    }
