...
pub struct PyWorkflowGraph {
    inner: samantha_workflow::WorkflowGraph,
}
...
impl PyWorkflowGraph {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: samantha_workflow::WorkflowGraph::new(name),
        }
    }
    ...
    fn add_node(
        ...
        let nt = match node_type {
            "tool" => samantha_workflow::NodeType::Tool,
            "condition" => samantha_workflow::NodeType::Condition,
            "parallel" => samantha_workflow::NodeType::Parallel,
            "loop" => samantha_workflow::NodeType::Loop,
            "transform" => samantha_workflow::NodeType::Transform,
            _ => samantha_workflow::NodeType::Agent,
        };
        let node = samantha_workflow::WorkflowNode {
            id: id.to_string(),
            node_type: nt,
    ...
    fn add_edge(&mut self, source: &str, target: &str) -> PyResult<()> {
        let edge = samantha_workflow::WorkflowEdge {
...
pub struct PyWorkflowBuilder {
    inner: Option<samantha_workflow::WorkflowBuilder>,
}
...
impl PyWorkflowBuilder {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Some(samantha_workflow::WorkflowBuilder::new(name)),
        }
    }
