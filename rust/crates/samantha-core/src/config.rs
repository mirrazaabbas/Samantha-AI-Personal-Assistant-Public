...
//!
//! Rust translation of `src/samantha/core/config.py`.
...
/// Default config directory: `~/.samantha/`
pub fn default_config_dir() -> PathBuf {
    dirs_home().join(".samantha")
}

/// Default config file path: `~/.samantha/config.toml`
...
fn default_sandbox_image() -> String { "samantha-sandbox:latest".into() }
...
fn default_operators_dir() -> String { "~/.samantha/operators".into() }
...
// ---------------------------------------------------------------------------
// Top-level SamanthaConfig
// ---------------------------------------------------------------------------

/// Top-level configuration for Samantha.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct SamanthaConfig {
    #[serde(skip)]
...
pub fn load_config(path: Option<&Path>) -> Result<SamanthaConfig, ConfigError> {
    ...
    let mut cfg = if config_path.exists() {
        ...
        let mut cfg: SamanthaConfig = toml::from_str(&content)?;
        ...
        let mut cfg = SamanthaConfig::default();
        cfg.engine.default = recommended_engine;
        cfg
    };
...
mod tests {
    ...
    fn test_default_config() {
        let cfg = SamanthaConfig::default();
...
mode = "block"
"#;
        let cfg: SamanthaConfig = toml::from_str(toml_str).unwrap();
