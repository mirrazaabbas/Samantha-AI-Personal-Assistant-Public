...
use samantha_core::{AgentContext, AgentResult, SamanthaError, ToolResult};
use samantha_tools::executor::ToolExecutor;
use regex::Regex;
...
impl<M: CompletionModel + 'static> OjAgent for NativeOpenHandsAgent<M> {
    ...
    async fn run(
    ...
    ) -> Result<AgentResult, SamanthaError> {
            ...
            .map(|ctx| {
                ctx.conversation
                    ...
                    .filter_map(|m| match m.role {
                        samantha_core::Role::User => {
                            Some(RigMessage::user(&m.content))
                        }
                        samantha_core::Role::Assistant => {
        ...
        for turn in 1..=self.max_turns {
            let response = self
                ...
                .await
                .map_err(|e| {
                    SamanthaError::Agent(samantha_core::error::AgentError::Execution(
                        e.to_string(),
                    ))
                })?;
...
mod tests {
    ...
    type OpenHandsAgent = NativeOpenHandsAgent<RigModelAdapter<samantha_engine::Engine>>;
