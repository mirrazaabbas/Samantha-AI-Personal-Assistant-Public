...
All notable changes to Samantha are documented in this file.
...
`samantha ask --engine afm --model afm-3-core "..."`. Requires an Apple Silicon
...
**Vision input for `samantha ask`** — attach images to a query with
...
`SAMANTHA_NUM_CTX` environment variable to tune the Ollama context window
(default `16384`).
...
eval traces and `samantha bench` came from it. Renaming the import would not
...
`samantha bench --setup-energy` built a path to it and offered to run it -- so
...
Custom browser clients must migrate to the `samantha.auth.v1` subprotocol
...
install path while `samantha.ai` is down, improves desktop
first-boot diagnostics on Windows, and ships the RAM-detection fix
for Windows that missed the v1.0.1 cutoff.

### Fixed

**`samantha/traces/` missing from the v1.0.1 PyPI wheel** (#372).
...
`src/samantha/traces/` — silently dropping the whole package. Every
fresh `pip install samantha==1.0.1` then failed at import with
`ModuleNotFoundError: No module named 'samantha.traces'` on the
first `samantha ask`, learning, or server call. Anchored the pattern to
...
the v1.0.1 cutoff, so v1.0.1 users still saw `0.0 GB` from `samantha
...
`samantha.ai/install.sh` URL was failing with `sslv3 alert
...
**`samantha self-update`** subcommand. Detects how Samantha was
installed (pip, uv tool, editable git checkout) by inspecting
`samantha.__file__`, then runs the right upgrade command. Supports
...
(`SAMANTHA_NO_UPDATER=1`) documented in `docs/desktop-auto-update.md`.

**Analytics env-var opt-out** (`DO_NOT_TRACK`, `SAMANTHA_NO_ANALYTICS`).
...
upgrade command. Honors `SAMANTHA_NO_UPDATE_CHECK=1` and `CI=true` to
...
- **Editable-git users running `samantha self-update`** get the
  detected `git pull && uv sync` command pointed at their actual
  checkout, not `~/Samantha`. If you'd come to rely on the
...
Companion blog post:
[From Minions to Samantha: A Retrospective on Two Years in Local AI](https://hazyresearch.stanford.edu/blog/2026-05-19-minions-to-samantha-retrospective).
...
`SamanthaSystem` in `src/samantha/system.py`, driven by a TOML config.
...
`samantha init --preset <name>` bundle an agent with a hardware-appropriate
...
(`src/samantha/learning/routing/complexity.py`). Produces a 0.0–1.0
...
**LLM-guided spec search** (`src/samantha/learning/spec_search/`).
...
preset and a runnable tutorial at `examples/samantha/spec_search_quickstart.py`.

**Six hybrid coordination paradigms** in `src/samantha/agents/hybrid/`.
...
A runner CLI (`python -m samantha.agents.hybrid.runner --cell <name>`)
...
- **agentskills.io spec adoption** — canonical `SKILL.md` format with YAML frontmatter following the [agentskills.io](https://agentskills.io/specification) open standard.
  ...
  - Sourced subdirectory layout (`~/.samantha/skills/<source>/<name>/`)
...
- **Skills learning loop** — trace tagging, pattern discovery, DSPy/GEPA optimization.
  ...
  - `SkillOverlay` — sidecar storage at `~/.samantha/learning/skills/<name>/optimized.toml`
...
- **Skills benchmark harness** — 4-condition PinchBench evaluation.
  ...
  - `SamanthaAgentBackend` accepts `skills_enabled` and `overlay_dir` kwargs
  - Conditions: `no_skills`, `skills_on`, `skills_optimized_dspy`, `skills_optimized_gepa`

- **CLI commands:**
  - `samantha skill list` / `info` / `run` / `install` / `sync` / `sources` / `update` / `remove` / `search`
  - `samantha skill discover` — mine traces for recurring tool patterns
  - `samantha skill show-overlay` — inspect optimization output
  - `samantha optimize skills` — run DSPy/GEPA per-skill optimization
  - `samantha bench skills` — run the PinchBench skills benchmark
...
- `examples/samantha/spec_search_quickstart.py` — runnable end-to-end
...
- `src/samantha/agents/hybrid/registry/*.toml` — 35-cell registry of
...
- **`learning/distillation/` is now `learning/spec_search/`.** The
  ...
  (`from samantha.learning.distillation.*` →
  `from samantha.learning.spec_search.*`). The `samantha distillation`
...
- **Engine `generate_full` return shape extended.**
  `SamanthaAgentBackend.generate_full` and `SamanthaDirectBackend.generate_full`
...
- **OpenClaw category/name queries** — `samantha skill install openclaw:owner/slug` now correctly splits into category + name match.
