"""Tests for ``samantha chat`` interactive REPL command."""
...
from samantha.cli._voice_chat import VOICE_EXIT, VoiceSession, record_voice, speak
from samantha.cli.chat_cmd import _read_input, chat
from samantha.core.config import SamanthaConfig
from samantha.core.events import Event, EventBus, EventType
from samantha.core.registry import AgentRegistry, ToolRegistry
from samantha.core.types import Role, ToolCall, ToolResult
from samantha.memory.store import LocalFactStore
...
class TestChatCommand:
    ...
    def test_voice_mode_preserves_typed_slash_commands(self) -> None:
        ...
        with (
            patch("samantha.cli.chat_cmd.load_config", return_value=config),
            patch("samantha.engine.get_engine", return_value=("mock", engine)),
            patch("samantha.intelligence.register_builtin_models"),
            patch("samantha.cli._voice_chat.record_voice") as record_voice,
        ):
    ...
    def test_voice_mode_ctrl_c_during_recording_exits_cleanly(self) -> None:
        ...
        with (
            ...
            patch("samantha.intelligence.register_builtin_models"),
            patch(
                "samantha.speech._discovery.get_speech_backend",
                return_value=backend,
            ),
            patch(
                "samantha.speech.voice_io.record_until_silence",
...
class TestVoiceInput:
    def test_unavailable_stt_is_detected_before_recording(self) -> None:
        ...
        with (
            ...
            patch("samantha.speech.voice_io.record_until_silence") as record,
        ):
            assert record_voice(console) is VOICE_EXIT

        record.assert_not_called()
        assert "Samantha" in str(console.print.call_args)
    ...
    def test_stt_backend_is_cached_for_the_chat_session(self) -> None:
        ...
        session = VoiceSession(SamanthaConfig())
        console = MagicMock()
    ...
    def test_tts_backend_is_cached_for_the_chat_session(self) -> None:
        ...
        with (
            ...
            patch("samantha.speech.voice_io.play_wav") as play,
        ):
...
class TestChatAgents:
    def test_direct_chat_injects_auto_memory_facts(self, tmp_path) -> None:
        ...
        with (
            ...
            patch("samantha.engine.get_engine", return_value=("mock", engine)),
            patch("samantha.intelligence.register_builtin_models"),
            patch("samantha.memory.build_memory_service", return_value=None),
            patch("samantha.cli.ask._get_memory_backend", return_value=None),
        ):
            result = CliRunner().invoke(
                chat,
                ["--model", "test-model"],
    ...
    def test_chat_generation_survives_fact_store_failure(self) -> None:
        ...
        with (
            ...
            patch("samantha.intelligence.register_builtin_models"),
            patch(
                "samantha.memory.build_memory_service",
    ...
    def test_simple_agent_does_not_receive_tool_only_kwargs(self) -> None:
        ...
        with (
            ...
            patch("samantha.intelligence.register_builtin_models"),
            patch(
                "samantha.cli._model_switch.tty_wants_model_picker",
                return_value=False,
            ),
            patch(
                "samantha.cli._runtime_panel.tty_wants_runtime_panel",
                return_value=False,
            ),
        ):
            result = CliRunner().invoke(
