"""Tests for ``samantha ask --agent`` CLI integration."""
...
from samantha.core.types import ToolCall, ToolResult
...
def mock_setup():
    ...
    with (
        ...
        mock_cfg.return_value = SamanthaConfig()
...
class TestAskAgentOption:
    ...
    def test_no_agent_flag_falls_back_to_config_default_agent(self, runner, mock_setup):
        ...
        The default ``SamanthaConfig`` sets ``default_agent = "simple"``, so
        ``samantha ask "..."`` should route through SimpleAgent rather than
...
class TestPersonaFilesReachModel:
    ...
    def test_soul_md_content_reaches_engine_in_simple_agent(
        ...
        soul.write_text("PERSONA_SENTINEL_zh_samantha", encoding="utf-8")
        ...
        assert "PERSONA_SENTINEL_zh_samantha" in joined
