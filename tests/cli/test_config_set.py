"""Tests for ``samantha config set`` command."""
...
class TestConfigSet:
    def test_set_creates_config_file(self, tmp_path: Path) -> None:
        ...
        env = {"SAMANTHA_CONFIG": str(config_file)}
    ...
    def test_set_engine_ollama_host(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            mock.patch("samantha.cli.config_cmd.httpx"),
        ):
            result = CliRunner().invoke(
                cli,
    ...
    def test_set_uses_utf8_for_existing_config(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch.dict(os.environ, {"SAMANTHA_CONFIG": str(config_file)}),
    ...
    def test_set_engine_host_probes_reachable(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch.dict(os.environ, env),
            mock.patch("samantha.cli.config_cmd.httpx") as mock_httpx,
        ):
