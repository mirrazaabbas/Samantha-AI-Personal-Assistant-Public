"""Tests for SamanthaSystem.wire_channel() — channel → agent routing.

These tests exercise wire_channel() on SamanthaSystem directly. The serve.py
...
def _make_system(engine=None, agent_name="", tmp_path=None) -> SamanthaSystem:
    """Build a minimal SamanthaSystem with mock engine for testing wire_channel."""
    config = SamanthaConfig()
    if tmp_path is not None:
        config.sessions.db_path = str(tmp_path / "sessions.db")
    mock_engine = engine or MagicMock()
    return SamanthaSystem(
        config=config,
...
# ---------------------------------------------------------------------------
# Tests via SamanthaSystem.wire_channel()
# ---------------------------------------------------------------------------
...
class TestChannelToolLoading:
    """SamanthaSystem receives tools when agent accepts them."""

    def test_tool_using_agent_receives_tools(self, tmp_path):
        """SamanthaSystem built with a tool list passes tools to the agent via ask()."""
    ...
    def test_non_tool_agent_receives_empty_tools(self, tmp_path):
        """SamanthaSystem with no tools list results in empty tools — simple agent
        unaffected."""
        config = SamanthaConfig()
        config.sessions.db_path = str(tmp_path / "sessions.db")
