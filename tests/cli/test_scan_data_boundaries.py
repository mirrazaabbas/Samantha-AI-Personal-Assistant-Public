...
from samantha.cli.scan_cmd import PrivacyScanner, ScanResult, scan
...
def _low_noise_config():
    ...
    SamanthaConfig defaults include absolute store paths under the real
    SAMANTHA_HOME; clear those so tests only see artifacts under tmp_path.
    """
    ...
    # Avoid scanning the developer's real ~/.samantha store files.
...
def _patch_config(monkeypatch, tmp_path, config, config_loaded=True, error=""):
    monkeypatch.setattr(
        "samantha.cli.scan_cmd._load_data_boundary_config",
        lambda: (config, tmp_path, config_loaded, error, ""),
    )
    monkeypatch.setattr("samantha.cli.scan_cmd.get_config_dir", lambda: tmp_path)
...
def test_scan_data_boundaries_strict_passes_with_info_only(
    ...
    assert result.exit_code == 0
    assert "Samantha Data-Boundary Scan" in result.output
...
def test_top_level_scan_data_boundaries_does_not_check_for_updates(
    ...
    monkeypatch.setattr(
        "samantha.cli._version_check.check_for_updates",
        fake_check_for_updates,
    )
    monkeypatch.setattr(
        "samantha.cli.scan_cmd._load_data_boundary_config",
        lambda: (SamanthaConfig(), tmp_path, False, "", ""),
    )
    monkeypatch.setattr(
        sys,
        "argv",
        ["samantha", "scan", "--data-boundaries", "--json"],
    )
...
def test_update_check_skip_helper_is_precise():
    ...
    from samantha.cli import _should_skip_update_check

    ctx = Context(Command("samantha"))
...
def test_existing_scan_quick_text_still_works(monkeypatch):
    ...
    assert result.exit_code == 0
    assert "Samantha Security Scan" in result.output


def test_data_boundary_loader_honors_samantha_config(
    monkeypatch,
    tmp_path,
):
    from samantha.cli import scan_cmd
    ...
    monkeypatch.setenv("SAMANTHA_CONFIG", str(config_path))
    monkeypatch.setenv("SAMANTHA_HOME", str(tmp_path / "home"))
...
def test_data_boundary_loader_reports_root_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "samantha.cli.scan_cmd.get_config_dir",
...
def test_data_boundary_cli_reports_real_root_error_without_import_crash():
    ...
    invalid_home = repo_root / ".invalid-samantha-home"
    env = os.environ.copy()
    env["SAMANTHA_HOME"] = str(invalid_home)
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "from samantha.cli import main; main()",
            "scan",
            "--data-boundaries",
            "--json",
        ],
