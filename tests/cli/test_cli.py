...
import samantha
from samantha.cli import cli, main


class TestMainEntryPoint:
    """Tests for the ``samantha`` console script entry point."""
    ...
    def test_windows_reconfigures_stdout_to_utf8(self) -> None:
        ...
        with (
            ...
            mock.patch("samantha.cli.cli") as cli_mock,
        ):
            main()
...
class TestCLI:
    ...
    def test_version(self) -> None:
        ...
        assert result.exit_code == 0
        assert samantha.__version__ in result.output
...
class TestStartupResilience:
    ...
    down every `samantha` command — including `samantha serve` — because the CLI
    ...
    def test_importing_cli_does_not_import_numpy(self) -> None:
        ...
        assert result.returncode == 0, (
            "importing samantha.cli pulled in numpy (a broken numpy would then "
            f"crash `samantha serve`):\nstdout={result.stdout}\nstderr={result.stderr}"
        )
