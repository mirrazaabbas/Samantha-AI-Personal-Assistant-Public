"""Tests for ``samantha scan`` privacy scanner CLI command."""
...
from samantha.cli.scan_cmd import PrivacyScanner, ScanResult
...
class TestFileVault:
    ...
    def test_command_failure_is_not_reported_as_disabled(self) -> None:
        scanner = PrivacyScanner()
        proc = _make_proc(stderr="Unknown volume", returncode=1)
        with patch("subprocess.run", return_value=proc):
            result = scanner.check_filevault()
        assert result.status == "skip"
...
class TestJsonOutput:
    ...
    def test_json_output_structure(self) -> None:
        ...
        runner = CliRunner()
        with patch(
            "samantha.cli.scan_cmd.PrivacyScanner.run_all",
            return_value=[
