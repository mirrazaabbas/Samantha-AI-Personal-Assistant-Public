...
from samantha.cli import _version_check
from samantha.cli._version_check import (
    _check_disabled,
    _config_disabled,
...
def _clean_env(monkeypatch, tmp_path):
    for v in ("SAMANTHA_NO_UPDATE_CHECK", "CI", "SAMANTHA_CONFIG"):
        monkeypatch.delenv(v, raising=False)
    # Point config + cache at empty tmp paths so tests don't see the
    # developer's real ~/.samantha state.
    monkeypatch.setenv("SAMANTHA_CONFIG", str(tmp_path / "no-config.toml"))
...
class TestCheckDisabled:
    ...
    def test_samantha_no_update_check_disables(self, monkeypatch, value):
        monkeypatch.setenv("SAMANTHA_NO_UPDATE_CHECK", value)
...
class TestConfigDisabled:
    ...
    def test_auto_update_false_disables(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("SAMANTHA_CONFIG", str(cfg))
    ...
    def test_samantha_config_env_override(self, monkeypatch, tmp_path):
        """SAMANTHA_CONFIG should redirect the lookup, matching core.config."""
...
class TestCheckForUpdates:
    @patch("samantha.cli._version_check._do_check")
    ...
    @patch(
        "samantha.cli._version_check._do_check",
