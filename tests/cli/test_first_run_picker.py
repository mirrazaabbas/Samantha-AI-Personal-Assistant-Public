"""Tests for bare ``samantha`` routing with model picker."""
...
def test_check_and_route_skips_picker_with_env(
    ...
    monkeypatch.setenv("SAMANTHA_SKIP_MODEL_PICK", "1")
    ctx = MagicMock()
    ctx.invoked_subcommand = None
    ctx.obj = {}
    with patch("sys.stdin.isatty", return_value=True):
...
def test_check_and_route_runs_init_without_config(
    ...
    ctx.invoked_subcommand = None
    check_and_route(ctx)
    ctx.invoke.assert_called_once_with(init_cmd, from_bare_samantha=True)
