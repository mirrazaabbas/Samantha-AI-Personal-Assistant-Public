"""CLI-level regression tests for ``samantha ask`` vision input.
...
from samantha.core.types import Role
...
def _patch_ask(monkeypatch, tmp_path: Path, *, engine_name: str) -> _RecordingEngine:
    """Wire ``samantha ask`` to a recording engine reported under ``engine_name``."""
    cfg = SamanthaConfig()
