"""Tests for ``samantha start|stop|restart|status`` daemon management commands."""
...
from samantha.cli.daemon_cmd import _pid_alive, _read_pid, _write_pid
...
class TestDaemonCommands:
    ...
    def test_stop_no_server(self) -> None:
        """``samantha stop`` when no PID file shows 'not running'."""
        with patch("samantha.cli.daemon_cmd._read_pid", return_value=None):
            result = CliRunner().invoke(cli, ["stop"])
        assert result.exit_code != 0
        assert "No running server" in result.output

    def test_status_no_server(self) -> None:
        """``samantha status`` when no PID file shows 'not running'."""
        with patch("samantha.cli.daemon_cmd._read_pid", return_value=None):
    ...
    def test_read_pid_no_file(self, tmp_path: Path) -> None:
        ...
        with patch(
            "samantha.cli.daemon_cmd._PID_FILE",
    ...
    def test_write_and_read_pid(self, tmp_path: Path) -> None:
        ...
        with (
            patch("samantha.cli.daemon_cmd._PID_FILE", pid_file),
            patch("samantha.cli.daemon_cmd.DEFAULT_CONFIG_DIR", tmp_path),
            patch("samantha.cli.daemon_cmd._pid_alive", return_value=True),
        ):
            _write_pid(12345)
    ...
    def test_status_shows_running(self) -> None:
        """``samantha status`` shows running info when PID exists."""
        mock_config = MagicMock()
        mock_config.server.host = "127.0.0.1"
        mock_config.server.port = 8000

        with (
            patch("samantha.cli.daemon_cmd._read_pid", return_value=9999),
            patch(
                "samantha.cli.daemon_cmd.load_config",
    ...
    def test_start_already_running(self) -> None:
        """``samantha start`` exits with error when a server is already running."""
        with patch("samantha.cli.daemon_cmd._read_pid", return_value=42):
...
class TestPidLiveness:
    ...
    def test_read_pid_stale_pid_returns_none(self, tmp_path: Path) -> None:
        ...
        with patch("samantha.cli.daemon_cmd._PID_FILE", pid_file):
            assert _read_pid() is None
...
class TestDaemonDetachment:
    ...
    def _spawn_kwargs(platform: str) -> dict:
        ...
        """
        with (
            patch("samantha.cli.daemon_cmd._read_pid", return_value=None),
            patch("samantha.cli.daemon_cmd._write_pid"),
            patch("samantha.cli.daemon_cmd.load_config"),
            patch(
                "samantha.cli.daemon_cmd._server_healthy",
                side_effect=[False, True],
            ),
            patch("samantha.cli.daemon_cmd.sys.platform", platform),
            patch("samantha.cli.daemon_cmd.subprocess.Popen") as popen,
...
def test_start_refuses_healthy_server_without_pid() -> None:
    config = MagicMock()
    config.server.host = "127.0.0.1"
    config.server.port = 8000
    with (
        patch("samantha.cli.daemon_cmd._read_pid", return_value=None),
        patch("samantha.cli.daemon_cmd.load_config", return_value=config),
        patch("samantha.cli.daemon_cmd._server_healthy", return_value=True),
        patch("samantha.cli.daemon_cmd.subprocess.Popen") as popen,
    ):
        result = CliRunner().invoke(cli, ["start"])
    assert result.exit_code != 0
    assert "already reachable" in result.output
