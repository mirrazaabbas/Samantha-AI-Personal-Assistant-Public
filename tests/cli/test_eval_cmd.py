"""Tests for the ``samantha eval`` CLI commands."""
...
class TestEvalCLI:
    ...
    def test_eval_group_exists(self):
        """``samantha eval --help`` shows run/compare/report/list subcommands."""
    ...
    def test_eval_list_benchmarks(self):
        """``samantha eval list`` exits 0 and outputs benchmark names."""
        ...
        assert "samantha-direct" in result.output
        assert "samantha-agent" in result.output

    def test_eval_run_missing_args(self):
        """``samantha eval run`` without required args fails gracefully."""
