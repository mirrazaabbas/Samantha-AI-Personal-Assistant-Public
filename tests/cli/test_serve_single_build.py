"""Regression tests for #263 — ``samantha serve`` must build the system once.
...
1. ``SystemBuilder.build`` is never called during ``samantha serve`` startup
...
# ``samantha.cli.serve`` as an attribute resolves to the click *command*
# (re-exported on the package); grab the real module to monkeypatch its globals.
serve_mod = importlib.import_module("samantha.cli.serve")
...
def _repopulate_registries() -> None:
    ...
    if not AgentRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("samantha.agents.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not ToolRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("samantha.tools.")
    ...
    if not MemoryRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("samantha.tools.storage.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
                    importlib.reload(sys.modules[mod_name])
...
def _run_serve(tmp_path, monkeypatch, *, build_spy, set_system_spy):
    """Invoke ``samantha serve`` with all heavy/blocking pieces stubbed out.
    ...
    monkeypatch.setattr("samantha.security.setup_security", lambda *a, **k: sec)

    with (
        patch(
            "samantha.system.builder.SystemBuilder.build",
            build_spy,
        ),
        patch(
            "samantha.agents.executor.AgentExecutor.set_system",
            set_system_spy,
        ),
...
def test_serve_does_not_call_systembuilder_build(tmp_path, monkeypatch):
    ...
    build_spy = MagicMock(
        side_effect=AssertionError(
            "SystemBuilder.build() must not run during `samantha serve` startup "
