...
"""
    ...
    def test_run_once(self):
        ...
        assert call_kwargs.kwargs["max_turns"] == m.max_turns
...
class TestOperativeAgent:
    def test_init_defaults(self):
        ...
        engine = FakeEngine()
        with patch(
            "samantha.agents._stubs.load_config",
    ...
    def test_daily_briefing_plain_text_removes_requests(self):
        from samantha.agents.daily_briefing import DailyBriefingAgent

        text = DailyBriefingAgent._plain_text(
            "**Schedule clear.** Please provide your priorities. No tasks are active."
        )

        assert text == "Schedule clear. No tasks are active."
    ...
    def test_run_injects_authoritative_local_timestamp(self):
        from samantha.agents.operative import OperativeAgent

        engine = FakeEngine([{"content": "Timestamp received."}])
        agent = OperativeAgent(engine, "test-model")
        ...
        assert any("Authoritative Runtime Context" in text for text in system_messages)
        assert any("Current local date and time:" in text for text in system_messages)
    ...
    def test_max_turns_exceeded(self):
        ...
        assert result.content.startswith("Tick reached its bounded turn budget")
...
class TestSystemAskPassthrough:
    def test_system_prompt_forwarded(self):
        ...
        from samantha.system import QueryOrchestrator

        with patch.object(QueryOrchestrator, "_run_agent", patched_run_agent):
            real_system = SamanthaSystem(
...
class TestOperatorsConfig:
    def test_config_defaults(self):
        from samantha.core.config import OperatorsConfig
    ...
    def test_config_in_samantha_config(self):
        from samantha.core.config import SamanthaConfig
...
class TestLoadBundledOperators:
    ...
    def operators_dir(self):
        ...
        ops_dir = project_root / "src" / "samantha" / "operators" / "data"
    ...
    def test_daily_briefing_loads_with_privacy_safe_tools(self, operators_dir):
        from samantha.operators.loader import load_operator

        m = load_operator(operators_dir / "daily_briefing.toml")
        assert m.id == "daily_briefing"
        assert m.schedule_type == "cron"
        assert m.schedule_value == "30 2 * * *"
        assert m.agent == "daily_briefing"
        assert m.max_turns == 1
        assert {
            "calendar_upcoming",
            "user_profile_manage",
            "memory_retrieve",
        ...
        assert "Never send, upload, publish, or share" in m.system_prompt
        assert 'backend="macos"' in m.system_prompt
