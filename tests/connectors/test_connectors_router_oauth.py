...
``~/.samantha/connectors`` (a real source of spurious failures — see the
...
def hermetic_connectors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    test leaves ``~/.samantha`` untouched.
    ...
    *source* (``samantha.core.config``) — every connector re-derives
    ...
    import samantha.connectors.oauth as oauth_mod
    import samantha.core.config as config_mod
    import samantha.server.connectors_router as router_mod
    ...
    google_mods = [
        "samantha.connectors.gdrive",
        "samantha.connectors.gcalendar",
        "samantha.connectors.gcontacts",
        "samantha.connectors.gmail",
        "samantha.connectors.google_tasks",
        ...
        "samantha.connectors.spotify",
        "samantha.connectors.strava",
    ]
...
def test_connect_client_pair_returns_oauth_required_no_browser(
    ...
    """
    with patch("samantha.core.open_browser") as mock_browser:
        resp = client.post(
...
def test_oauth_callback_exchanges_and_connects(
    ...
    from samantha.connectors.gdrive import GDriveConnector
...
def test_oauth_callback_rejects_missing_access_token_without_false_success(
    ...
    from samantha.connectors.spotify import SpotifyConnector
