...
def test_exchange_google_token_calls_endpoint() -> None:
    from samantha.connectors.oauth import exchange_google_token
...
def test_generic_exchange_rejects_response_without_access_token() -> None:
    from samantha.connectors.oauth import OAUTH_PROVIDERS, _exchange_token
...
def test_gdrive_is_connected_requires_access_token(tmp_path: Path) -> None:
    from samantha.connectors.gdrive import GDriveConnector
    from samantha.connectors.oauth import save_tokens
...
def test_gcalendar_handle_callback_persists_creds_no_background_flow(
    ...
    from samantha.connectors.gcalendar import GCalendarConnector
    from samantha.connectors.oauth import load_tokens
...
def test_gcontacts_handle_callback_persists_creds_no_background_flow(
    ...
    """Sibling connector shares the fix: creds saved, no browser thread (#512)."""
    from samantha.connectors.gcontacts import GContactsConnector
    from samantha.connectors.oauth import load_tokens
