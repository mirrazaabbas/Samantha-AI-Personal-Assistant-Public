...
class TestSamanthaSystem:
    ...
    def test_build_tools_injects_memory_backend(self):
        ...
        ToolRegistry.register_value("memory_retrieve", MemoryRetrieveTool)
        ...
        tools = system._build_tools(["memory_store", "memory_retrieve"])

        assert len(tools) == 2
        assert all(tool._backend is memory_backend for tool in tools)
...
class TestSystemBuilder:
    ...
    def test_import_works(self):
        ...
        assert SamanthaSystem is not None
    ...
    def test_builder_default_config(self):
        ...
        assert isinstance(builder._config, SamanthaConfig)
...
class TestSystemBuilderEngineInstance:
    """Explicit engine injection (samantha eval --base-url path)."""
    ...
    def test_engine_instance_is_fluent(self):
        builder = SystemBuilder(SamanthaConfig())
    ...
    def test_unhealthy_injected_instance_never_consults_discovery(self):
        ...
        with patch("samantha.engine._discovery.get_engine") as mock_get_engine:
