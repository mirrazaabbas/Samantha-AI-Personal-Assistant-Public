...
from samantha.engine.ollama import _ollama_request_options
...
def test_default_num_ctx_when_omitted(monkeypatch) -> None:  # noqa: ANN001
    monkeypatch.setenv("SAMANTHA_NUM_CTX", "24576")
    monkeypatch.delenv("SAMANTHA_NUM_GPU", raising=False)
    opts = _ollama_request_options(
        temperature=0.7,
        max_tokens=1024,
        kwargs={},
...
def test_default_num_gpu_from_environment(monkeypatch) -> None:  # noqa: ANN001
    monkeypatch.setenv("SAMANTHA_NUM_GPU", "0")
    opts = _ollama_request_options(
        temperature=0.7,
        max_tokens=1024,
        kwargs={},
