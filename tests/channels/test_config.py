...
from samantha.core.config import ChannelConfig, SamanthaConfig, load_config
...
class TestChannelConfigInSamanthaConfig:
    def test_channel_config_in_samantha_config(self) -> None:
        cfg = SamanthaConfig()
        assert hasattr(cfg, "channel")
        assert isinstance(cfg.channel, ChannelConfig)

    def test_samantha_config_channel_defaults(self) -> None:
        cfg = SamanthaConfig()
