...
def _run_fake_samantha(tmp_path: Path, argv: list[str]) -> list[str]:
    """Run an argv assembled with Docker's exec-form ENTRYPOINT semantics."""
    fake_samantha = tmp_path / "samantha"
    fake_samantha.write_text(
        "#!/bin/sh\nprintf '%s\\0' \"$@\"\n",
        encoding="utf-8",
    )
    fake_samantha.chmod(0o755)
    completed = subprocess.run(
        [str(fake_samantha), *argv[1:]],
        check=True,
        capture_output=True,
    )
...
def test_render_selects_cloud_engine_through_the_serve_cli(tmp_path: Path) -> None:
    ...
    args = _run_fake_samantha(tmp_path, argv)
    ...
    assert "SAMANTHA_ENGINE" not in _env_vars(service)
    assert _env_vars(service)["PORT"] == {"key": "PORT", "value": "10000"}


def test_docker_and_compose_command_overrides_remain_samantha_subcommands(
    tmp_path: Path,
) -> None:
    ...
    assert (
        _run_fake_samantha(tmp_path, [*entrypoint, *default_command]) == default_command
    )
    assert (
        _run_fake_samantha(tmp_path, [*entrypoint, *compose_override])
        == compose_override
    )


def test_render_does_not_advertise_an_unconsumed_cors_variable() -> None:
    assert "SAMANTHA_CORS_ORIGINS" not in _env_vars(_service())


def test_render_public_bind_generates_an_api_key() -> None:
    env = _env_vars(_service())

    assert env["SAMANTHA_API_KEY"] == {
        "key": "SAMANTHA_API_KEY",
        "generateValue": True,
    }
...
def test_render_free_tier_ephemeral_storage_is_explicitly_documented() -> None:
    ...
    assert env["SAMANTHA_HOME"]["value"] == "/home/samantha/.samantha"
