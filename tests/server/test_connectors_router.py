...
def test_disconnect_clears_stale_sync_checkpoint(app) -> None:
    ...
    old watermark.
    """
    from samantha.connectors.pipeline import IngestionPipeline
    from samantha.connectors.store import KnowledgeStore
    from samantha.connectors.sync_engine import SyncEngine
...
def test_disconnect_cancels_inflight_sync_before_purge(app) -> None:
    from samantha.connectors._stubs import Document, SyncStatus
    from samantha.connectors.store import KnowledgeStore
    from samantha.server.connectors_router import _instances
...
def test_disconnect_timeout_preserves_source_and_guards_reconnect(
    ...
    from samantha.connectors._stubs import SyncStatus
    from samantha.server import connectors_router
    from samantha.server.connectors_router import _instances
...
def test_connect_slack_bot_token_returns_400(app, tmp_path: Path) -> None:
    ...
    from samantha.connectors.slack_connector import SlackConnector
    from samantha.server.connectors_router import _instances
...
def test_connect_granola_invalid_key_returns_400_keeps_existing(
    ...
    from samantha.connectors.granola import GranolaConnector, GranolaKeyError
    from samantha.server.connectors_router import _instances
    ...
    try:
        with patch(
            "samantha.connectors.granola._granola_api_validate_key",
...
def test_connect_persists_generic_token_connector_credentials(
    ...
    from samantha.connectors.github_notifications import GitHubNotificationsConnector
    from samantha.connectors.oura import OuraConnector
    from samantha.connectors.weather import WeatherConnector
    from samantha.core.registry import ConnectorRegistry
    from samantha.server.connectors_router import _instances
    ...
    validators = {
        "github_notifications": (
            "samantha.connectors.github_notifications._github_api_get",
            [],
        ),
        "oura": ("samantha.connectors.oura._oura_api_get", {}),
        "weather": ("samantha.connectors.weather._weather_api_get", {}),
    }
...
def test_invalid_token_is_not_persisted_or_synced(app, tmp_path, monkeypatch) -> None:
    ...
    from samantha.connectors.github_notifications import (
...
def test_connect_news_rss_requires_and_persists_feeds(app, tmp_path: Path) -> None:
    ...
    from samantha.connectors.news_rss import NewsRSSConnector
    from samantha.server.connectors_router import _instances
