...
from samantha.core.events import EventBus, EventType  # noqa: E402
from samantha.core.types import Role  # noqa: E402
from samantha.server.app import create_app  # noqa: E402
...
def _make_agent(content="Hello from agent"):
    from samantha.agents._stubs import AgentResult
...
class TestChatCompletions:
    ...
    def test_instrumented_engine_unwrapped_to_avoid_dual_telemetry(self):
        ...
        from samantha.telemetry.instrumented_engine import InstrumentedEngine
        ...
        from samantha.core.types import TOKEN_COUNTING_VERSION
    ...
    def test_multi_engine_local_fallback_reports_actual_ollama_route(self):
        ...
        from samantha.engine.multi import MultiEngine
        ...
        with patch(
            "samantha.server.cloud_router.stream_local",
    ...
    def test_streaming_without_client_tools_uses_configured_agent(self):
        ...
        from samantha.agents.orchestrator import OrchestratorAgent
    ...
    def test_streaming_with_tools_emits_tool_calls_and_bypasses_agent(self):
        ...
        from samantha.engine._stubs import StreamChunk
...
def _identity_config():
    ...
    cfg = SamanthaConfig()
    cfg.agent.default_system_prompt = "You are Samantha."
    cfg.analytics.enabled = False
    return cfg
...
class TestIdentityPromptInjection:
    ...
    saw Samantha's identity system prompt and the model answered from its
    ...
    def test_stream_injects_identity_when_absent(self):
        ...
        assert "Samantha" in msgs[0].content
    ...
    def test_stream_uses_grounded_agent_result_without_replay(self):
        ...
        captured: list = []
        engine = _make_capturing_engine(captured)
        agent = _make_agent(content="My name is Samantha Prime.")
        agent._tools = [object()]
        ...
        assert streamed_content == "My name is Samantha Prime."
        assert captured == []
    ...
    def test_agent_path_folds_history_into_one_identity_system_message(self):
        ...
        from samantha.prompt.builder import SystemPromptBuilder
        ...
        assert messages[0].content.count("You are Samantha.") == 1
    ...
    def test_direct_merges_identity_and_auto_memory_into_one_system_message(self):
        from samantha.memory.store import Fact
        ...
        assert "Samantha" in system_messages[0].content
    ...
    def test_agent_does_not_rebuild_identity_already_merged_with_memory(self):
        ...
        system_messages = [m for m in messages if m.role == Role.SYSTEM]
        assert len(system_messages) == 1
        assert system_messages[0].content.count("You are Samantha.") == 1
        assert "favorite color is blue" in system_messages[0].content
    ...
    def test_direct_injects_soul_persona_when_present(self, tmp_path):
        ...
        persona files never applied to this path, unlike ``samantha ask`` and
        ...
        """
        from samantha.core.config import MemoryFilesConfig
        ...
        msgs = engine.generate.call_args.args[0]
        assert msgs[0].role.value == "system"
        assert "Samantha" in msgs[0].content  # identity blurb still present
    ...
    def test_memory_context_does_not_suppress_identity_injection(self):
        ...
        from samantha.server.routes import _ensure_identity_prompt
        from samantha.tools.storage.context import build_context_message
        ...
        assert len(system_msgs) == 1
        assert "Samantha" in system_msgs[0].content
        assert system_msgs[0].metadata["memory_context"] is True
        assert system_msgs[0].metadata["samantha_identity_prompt"] is True
        ...
        assert normalized_again[0].content.count("You are Samantha.") == 1
...
class TestModelsEndpoint:
    ...
    def test_configured_litellm_model_is_listed(self):
        ...
        with patch(
            "samantha.server.cloud_router.list_local_models",
    ...
    def test_litellm_provider_model_streams_through_active_engine(self):
        ...
        with patch(
            "samantha.server.cloud_router.stream_cloud",
...
def _traces_enabled_config(tmp_path):
    ...
    tests fail on any machine whose ``~/.samantha/config.toml`` disables
