...
from samantha.server.api_routes import include_all_routes  # noqa: E402
...
class TestMemoryRoutes:
    # 503 is the documented response when the native ``samantha_rust``
...
class TestMemoryRustMissing:
    """Regression for #502: when the native ``samantha_rust`` extension is
    ...
    def _client(monkeypatch):
        ...
        def _boom():
            raise ImportError("No module named 'samantha_rust'")

        import samantha._rust_bridge as bridge
    ...
    def test_store_is_not_a_silent_noop(self, monkeypatch):
        ...
        assert "samantha_rust" in detail
    ...
    def test_config_reports_unavailable(self, monkeypatch):
        ...
        assert "samantha_rust" in (data["detail"] or "")
...
class TestMetricsRoute:
    def test_metrics_endpoint(self, tmp_path, monkeypatch):
        ...
        """
        monkeypatch.setenv("SAMANTHA_HOME", str(tmp_path))
    ...
    def test_metrics_endpoint_reports_real_stats(self, tmp_path, monkeypatch):
        ...
        from samantha.core.types import TelemetryRecord
        ...
        assert "samantha_requests_total 1" in resp.text
        assert "samantha_tokens_total 15" in resp.text
        assert "samantha_latency_avg_ms 2000" in resp.text
    ...
    def test_metrics_endpoint_empty_db_no_division_by_zero(self, tmp_path, monkeypatch):
        ...
        assert resp.status_code == 200
        assert "No metrics available" not in resp.text
        assert "samantha_requests_total 0" in resp.text
        assert "samantha_latency_avg_ms 0" in resp.text
    ...
    def test_metrics_endpoint_uses_active_app_telemetry_path(self, tmp_path):
        ...
        assert resp.status_code == 200
        assert "samantha_requests_total 1" in resp.text
        assert "samantha_tokens_total 7" in resp.text
        assert "samantha_latency_avg_ms 500" in resp.text
