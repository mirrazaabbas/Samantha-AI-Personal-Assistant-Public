...
def test_returns_tools_from_mcp_server(mock_load_config: MagicMock):
    ...
    with (
        patch("samantha.mcp.transport.StreamableHTTPTransport"),
        patch("samantha.mcp.client.MCPClient"),
        patch("samantha.tools.mcp_adapter.MCPToolProvider") as MockProvider,
    ):
...
def test_returns_tools_from_external_mcp_file(
    ...
    with (
        patch("samantha.mcp.transport.StreamableHTTPTransport"),
        patch("samantha.mcp.client.MCPClient"),
        patch("samantha.tools.mcp_adapter.MCPToolProvider") as provider,
    ):
...
def test_does_not_cache_empty_results(mock_load_config: MagicMock):
    ...
    with (
        patch("samantha.mcp.transport.StreamableHTTPTransport"),
        patch("samantha.mcp.client.MCPClient") as MockClient,
        patch("samantha.tools.mcp_adapter.MCPToolProvider") as MockProvider,
    ):
...
def test_app_shutdown_stops_scheduler_before_closing_shared_mcp_clients() -> None:
    ...
    from samantha.server.agent_manager_routes import _start_managed_worker
...
def test_shutdown_interrupts_mcp_client_during_lazy_initialization() -> None:
    ...
    with (
        patch("samantha.core.config.load_config", return_value=mcp_config),
        patch("samantha.mcp.transport.StreamableHTTPTransport"),
        patch("samantha.mcp.client.MCPClient", return_value=client),
    ):
        _start_managed_worker(
            app.state,
            _discover,
...
def test_app_shutdown_closes_lazily_created_memory_backend(monkeypatch) -> None:
    ...
    from samantha.core.config import SamanthaConfig
    from samantha.server import agent_manager_routes as routes
    from samantha.server.app import create_app
...
def test_app_shutdown_keeps_owned_memory_open_for_live_worker(monkeypatch) -> None:
    ...
    from samantha.core.config import SamanthaConfig
    from samantha.server import app as app_module
    from samantha.server.agent_manager_routes import _start_managed_worker
