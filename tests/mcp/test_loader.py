"""Regression tests for samantha.mcp.loader.load_mcp_tools_from_config.

Closes the gap that #461 surfaced — MCP tools were silently dropped on
`samantha ask` and `samantha serve` because neither path read
...
def _mock_mcp_stack():
    ...
    with (
        ...
        patch("samantha.mcp.transport.StreamableHTTPTransport") as MockHttp,
        patch("samantha.mcp.transport.StdioTransport") as MockStdio,
