...
from samantha.core.types import Message, Role, TelemetryRecord
from samantha.telemetry.aggregator import AggregatedStats, TelemetryAggregator
from samantha.telemetry.energy_monitor import BASIS_GPU, EnergySample
from samantha.telemetry.instrumented_engine import InstrumentedEngine
from samantha.telemetry.store import TelemetryStore

_ask_mod = importlib.import_module("samantha.cli.ask")
_bench_mod = importlib.import_module("samantha.cli.bench_cmd")
...
def _energy_config(tmp_path, gpu_metrics=True):
    """Build a SamanthaConfig with energy monitoring enabled."""
    cfg = SamanthaConfig()
    ...
    # `samantha ask` (no --agent) now falls back to ``agent.default_agent``
...
class TestCliAskWiring:
    ...
    def test_energy_monitor_created_when_gpu_metrics_on(
        ...
        with patch(
            "samantha.telemetry.energy_monitor.create_energy_monitor",
...
class TestSdkWiring:
    ...
    def test_energy_monitor_stored(self, tmp_path):
        """Energy monitor is created and stored on Samantha instance."""
        from samantha.sdk import Samantha
...
class TestBenchWiring:
    ...
    def test_energy_monitor_passed_to_benchmarks(self):
        ...
        with (
            patch(
                "samantha.cli.bench_cmd.get_engine",
                return_value=("mock", engine),
            ),
            patch(
                "samantha.cli.bench_cmd.load_config",
                return_value=cfg,
            ),
            patch(
                "samantha.telemetry.energy_monitor.create_energy_monitor",
...
def _patch_telemetry_config(tmp_path: Path):
    ...
    return mock.patch(
        "samantha.cli.telemetry_cmd.load_config",
        return_value=cfg,
    ), db_path
