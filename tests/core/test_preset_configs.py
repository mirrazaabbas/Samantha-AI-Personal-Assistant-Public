...
Presets are installed via `samantha init --preset <name>`, which copies
`configs/samantha/examples/<name>.toml` to `~/.samantha/config.toml`.
...
PRESETS_DIR = Path(__file__).resolve().parents[2] / "configs" / "samantha" / "examples"
...
def test_preset_loads(preset_path: Path) -> None:
    ...
    # slots `samantha init` expects to be populated for a working first run.
