...
Uses OAuth 2.0 tokens stored locally (see :mod:`samantha.connectors.oauth`).
...
from samantha.connectors._stubs import BaseConnector, Document, SyncStatus
from samantha.connectors.google_auth import (
    GoogleAuthError,
)
from samantha.connectors.google_auth import (
    call_with_refresh as _call_with_refresh,
)
from samantha.connectors.oauth import (
    GOOGLE_ALL_SCOPES,
...
class GmailConnector(BaseConnector):
    ...
    credentials_path:
        ...
        ``~/.samantha/connectors/gmail.json``.
    """
    ...
    def is_connected(self) -> bool:
        ...
        token), which made `samantha connect gmail` short-circuit with
    ...
    def _refresh_token(self) -> str:
        ...
        if not new:
            raise RuntimeError(
                "Gmail token refresh failed — re-run `samantha connect gmail`"
            )
        return new
