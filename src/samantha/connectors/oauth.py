...
class OAuthProvider:
    ...
    # Filenames in ~/.samantha/connectors/ to save tokens to
...
def get_client_credentials(
    ...
    Checks credential files in ``~/.samantha/connectors/`` and falls
    back to environment variables ``SAMANTHA_{NAME}_CLIENT_ID`` and
    ``SAMANTHA_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"SAMANTHA_{provider.name.upper()}"
...
def save_tokens(path: str, tokens: Dict[str, Any]) -> None:
    ...
    """
    from samantha.security.file_utils import secure_write_json
        ...
        def do_GET(self) -> None:  # noqa: N802 — required override name
            ...
            if "code" in params:
                ...
                self.wfile.write(
                    ...
                    b"<p>You can close this tab and return to Samantha.</p>"
                    b"</body></html>"
                )
