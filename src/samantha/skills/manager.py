...
from samantha.skills.dependency import DependencyCycleError, validate_dependencies
from samantha.skills.executor import SkillExecutor, SkillResult
from samantha.skills.loader import discover_skills
...
class SkillManager:
    ...
    def __init__(
        ...
        *,
        ...
        if overlay_dir is None:
            ...
            # ~/.samantha/learning/skills/ if config can't be loaded.
            try:
    ...
    def _load_overlays(self) -> None:
        ...
        examples under ``manifest.metadata.samantha.few_shot``.

        Bad overlays are silently ignored — they should not break discovery.
        """
        from samantha.skills.overlay import SkillOverlayLoader
        ...
        for name, manifest in self._skills.items():
            ...
            if overlay.few_shot:
                ...
                oj = dict(new_metadata.get("samantha", {}) or {})
                oj["few_shot"] = list(overlay.few_shot)
                new_metadata["samantha"] = oj
    ...
    def get_few_shot_examples(self) -> List[str]:
        ...
        Pulls from ``manifest.metadata.samantha.few_shot`` for every
        registered skill.  Returns one formatted string per example.
        """
        examples: List[str] = []
        for name, manifest in self._skills.items():
            oj = manifest.metadata.get("samantha", {}) if manifest.metadata else {}
    ...
    def discover_from_traces(
        ...
        *,
        ...
        ``~/.samantha/skills/discovered/``).  Returns a list of dicts with
    ...
    def _serialize_discovered_skill(name: str, skill: Any) -> str:
        ...
        lines.append('author = "samantha (auto-discovered)"')
    ...
    def find_installed_paths(
        ...
        for root in roots:
            ...
            for candidate in root.rglob("*"):
                ...
                try:
                    from samantha.skills.loader import load_skill_directory
