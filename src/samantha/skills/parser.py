...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.samantha.* via FIELD_MAPPING.
...
from samantha.skills.types import SkillManifest
...
# dataclass attribute.  When target_kind is "samantha_meta" the value is
# stored under manifest.metadata["samantha"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "platforms": ("samantha_meta", "platforms"),
    "prerequisites": ("samantha_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.samantha.original_frontmatter,
        and merges metadata.samantha.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize samantha namespace
        oj_meta = dict(raw_metadata.get("samantha") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.samantha.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.samantha.* into canonical fields (these override
        # top-level mappings since they are explicit Samantha-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.samantha.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the samantha metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["samantha"] = oj_meta
