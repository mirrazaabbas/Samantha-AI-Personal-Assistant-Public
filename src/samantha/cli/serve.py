"""``samantha serve`` — OpenAI-compatible API server."""
...
from samantha.core.credentials import inject_credentials
...
def serve(
    ...
    # Tool credentials saved through the browser UI live in the Samantha
    ...
    if _has_cloud and engine_name != "cloud":
        try:
            from samantha.engine.cloud import CloudEngine
    ...
    # Create app
    from samantha.server.app import create_app
    ...
    api_key = _os.environ.get("SAMANTHA_API_KEY", "")
    if not api_key:
        try:
            import tomllib
    ...
    from samantha.server.auth_middleware import check_bind_safety

    check_bind_safety(bind_host, api_key=api_key)

    # Log credential status at startup
    from samantha.core.credentials import TOOL_CREDENTIALS, get_credential_status
    ...
    console.print(
        f"[green]Starting Samantha API server[/green]\n"
