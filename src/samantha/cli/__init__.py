"""Command-line interface for Samantha (Click-based)."""
...
import samantha
from samantha.cli.scan_cmd import scan
...
# A data-boundary scan must be able to diagnose an invalid SAMANTHA_HOME.
...
@click.group(
    help="Samantha — private, voice-first personal AI assistant",
    invoke_without_command=True,
)
@click.version_option(version=samantha.__version__, prog_name="samantha")
...
@click.option(
    ...
    default=False,
    help=(
        "Bare ``samantha``: force interactive model list "
        "(overrides SAMANTHA_SKIP_MODEL_PICK)."
    ),
)
@click.pass_context
def cli(ctx: click.Context, verbose: bool, quiet: bool, pick_model_bare: bool) -> None:
    """Top-level CLI group."""
    from samantha.cli.log_config import setup_logging
    ...
    # demo recordings of ``samantha ask --research``, so skip it whenever
    # the research flag is in argv (cheap argv sniff — Click hasn't
    # parsed the subcommand's args yet at this point). Also skip
    # ``samantha scan --data-boundaries`` because it is intended to be a
    ...
    if not quiet and ctx.invoked_subcommand and not skip_update_check:
        ...
        from samantha.cli._version_check import check_for_updates
        ...
        # notably `samantha serve` startup (#263). It's best-effort and never
    ...
    # First-run guard — routes bare `samantha` to chat or init.
    if ctx.invoked_subcommand is None:
        from samantha.cli._first_run import check_and_route
...
if not _DATA_BOUNDARY_BOOTSTRAP:
    from samantha.cli._bootstrap import bootstrap_cmd
    from samantha.cli.add_cmd import add
    from samantha.cli.agent_cmd import agent
    from samantha.cli.ask import ask
    from samantha.cli.bench_cmd import bench
    from samantha.cli.channel_cmd import channel
    from samantha.cli.channels_cmd import channels
    from samantha.cli.chat_cmd import chat
    from samantha.cli.compose_cmd import compose
    from samantha.cli.config_cmd import config
    from samantha.cli.connect_cmd import connect
    from samantha.cli.daemon_cmd import restart, start, status, stop
    from samantha.cli.digest_cmd import digest
    from samantha.cli.doctor_cmd import doctor
    from samantha.cli.eval_cmd import eval_group
    from samantha.cli.feedback_cmd import feedback_group
    from samantha.cli.gateway_cmd import gateway
    from samantha.cli.host_cmd import host
    from samantha.cli.init_cmd import init
    from samantha.cli.memory_cmd import memory
    from samantha.cli.mine_cmd import mine
    from samantha.cli.model import model
    from samantha.cli.operators_cmd import operators
    from samantha.cli.optimize_cmd import optimize_group
    from samantha.cli.pearl_cmd import pearl
    from samantha.cli.quickstart_cmd import quickstart
    from samantha.cli.registry_cmd import registry
    from samantha.cli.scheduler_cmd import scheduler
    from samantha.cli.self_update_cmd import self_update
    from samantha.cli.serve import serve
    from samantha.cli.skill_cmd import skill
    from samantha.cli.telemetry_cmd import telemetry
    from samantha.cli.tool_cmd import tool
    from samantha.cli.vault_cmd import vault
    from samantha.cli.workflow_cmd import workflow
    ...
    try:
        from samantha.cli.deep_research_setup_cmd import deep_research_setup
    ...
    try:
        from samantha.cli.auth_cmd import auth
    ...
    try:
        from samantha.cli.tunnel_cmd import tunnel
...
def main() -> None:
    """Entry point registered as ``samantha`` console script."""
    import sys
