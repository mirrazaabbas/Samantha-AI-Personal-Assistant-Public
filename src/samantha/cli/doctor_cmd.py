"""``samantha doctor`` — run diagnostic checks on Samantha."""
...
from samantha.core.config import DEFAULT_CONFIG_PATH, load_config
...
def _check_config_exists() -> CheckResult:
    ...
    return CheckResult(
        ...
        details="Run `samantha init` to generate a config file.",
    )
...
def _check_engines() -> List[CheckResult]:
    ...
    from samantha.engine import _discovery
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "samantha[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "samantha[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "samantha[energy-amd]", "AMD energy monitoring"),
        ("colbert", "samantha[memory-colbert]", "ColBERT memory backend"),
        (
            "zeus_apple_silicon",
            "samantha[energy-apple]",
            "Apple Silicon energy monitoring",
        ),
        ("apple_fm_sdk", "samantha[afm]", "Apple Foundation Models (AFM 3)"),
    ]
...
def doctor(as_json: bool) -> None:
    """Run diagnostic checks on your Samantha installation."""
    ...
    console = Console()
    console.print()
    console.print("[bold]Samantha Doctor[/bold]")
    console.print()
