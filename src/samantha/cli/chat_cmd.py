"""``samantha chat`` — interactive multi-turn chat REPL."""
...
from samantha.cli._runtime_panel import runtime_cli_options
from samantha.cli._tool_names import resolve_tool_names
from samantha.cli._voice_chat import VOICE_EXIT, VoiceSession, speak
...
from samantha.memory import publish_completed_exchange
...
def _safe_rich_label(value: object) -> str:
    ...
    from samantha.cli._model_switch import sanitize_model_id
...
@click.option(
    ...
    default=False,
    help=(
        "Show the model list before chat. Bare ``samantha`` already does this on a TTY "
        "unless SAMANTHA_SKIP_MODEL_PICK=1."
    ),
)
...
@click.option(
    ...
    default=None,
    help=(
        "Named persona dir under ~/.samantha/personas/<name>/ "
...
def chat(
    ...
    engine list first (bare ``samantha`` does this on a TTY unless
    ``SAMANTHA_SKIP_MODEL_PICK=1``).
    ...
    # Resolve engine
    from samantha.engine import get_engine
    from samantha.intelligence import register_builtin_models
    ...
    from samantha.cli._runtime_panel import (
        ChatRuntimeOptions,
    ...
    if agent_key and agent_key != "none":
        try:
            import samantha.agents  # noqa: F401 — trigger registration
            ...
            if AgentRegistry.contains(agent_key):
                ...
                if getattr(agent_cls, "accepts_tools", False):
                    ...
                    if tool_names_list:
                        import samantha.tools  # noqa: F401 — trigger registration
    ...
    console.print(
        f"[green bold]Samantha[/green bold]\n"
    ...
    from samantha.cli._bg_state import get_status
    from samantha.cli._chat_banner import render_startup_banner
    ...
    from samantha.cli._chat_notifications import NotificationDispatcher
    ...
    memory_service = None
    try:
        from samantha.memory import build_memory_service
    ...
    if config.agent.context_from_memory:
        from samantha.cli.ask import _get_memory_backend
    ...
    while True:
        ...
        if voice_mode:
            ...
            # Preserve typed slash commands when stdin is supplied by
            # Click, such as tests, pipes, or scripted CLI usage.
            ...
            if not stdin.isatty():
                ...
                from samantha.cli._voice_chat import record_voice
        ...
        # Fast deterministic path for safe voice commands.
        # These bypass the LLM/orchestrator for much lower latency.
        if voice_mode:
            from samantha.cli._fast_voice_commands import handle_fast_voice_command

            fast_response = handle_fast_voice_command(user_input)
            if fast_response is not None:
                console.print(f"[bold cyan]Samantha:[/bold cyan] {fast_response}")
                assert voice_session is not None
                speak(fast_response, console, voice_session)
                continue
        ...
        if config.agent.context_from_memory:
            try:
                from samantha.memory import load_configured_facts
                from samantha.tools.storage.context import (
                    ContextConfig,
                    inject_context,
                )
        ...
        try:
            if agent is not None:
                from samantha.agents._stubs import AgentContext
