...
Used by both ``install.sh`` (via ``samantha _bootstrap --write-config``)
and ``samantha init`` (so there is a single source of truth for the
...
def write_initial_config(
    *,
    ...
    Called by both ``install.sh`` (via ``samantha _bootstrap --write-config``)
    and ``samantha init`` so the TOML format has one definition.
    """
    ...
    base_toml = (
        f"# Samantha configuration\n"
        f"{hardware_line}{gpu_comment}\n"
        f"# Full reference config: samantha init --full\n"
        f"\n"
        f"{provenance}\n"
        f"\n"
        f"[engine]\n"
...
# ---------------------------------------------------------------------------
# CLI command — invoked by install.sh, hidden from `samantha --help`
# ---------------------------------------------------------------------------
