"""``samantha host`` — download and serve a model locally with auto backend setup."""
...
def _build_serve_command(backend: str, model: str, port: int) -> list[str]:
    ...
    if backend == "apple_fm":
        ...
        return [
            ...
            "-m",
            "uvicorn",
            "samantha.engine.apple_fm_shim:app",
            "--host",
            "127.0.0.1",
            "--port",
            str(port),
...
def host(
    ...
    \b
      samantha host mlx-community/Qwen2.5-7B-4bit --backend mlx
      samantha host Qwen/Qwen3-8B --backend vllm
      samantha host qwen3:8b --backend ollama
      samantha host meta-llama/Llama-3-8B -b sglang
    """
    console = Console()
    ...
    if backend != "ollama":
        ...
        console.print(
            "[dim]Samantha will auto-discover it. Press Ctrl+C to stop.[/dim]\n"
        )
