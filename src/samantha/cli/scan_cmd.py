"""``samantha scan`` — audit your environment for privacy and security risks."""
...
from samantha.core.paths import get_config_dir, get_config_path
from samantha.security.data_boundary_audit import (
    DataBoundaryReport,
...
class PrivacyScanner:
    ...
    def check_filevault(self) -> ScanResult:
        ...
        try:
            ...
            normalized = proc.stdout.strip().casefold()
            if proc.returncode == 0 and "filevault is on" in normalized:
                return ScanResult(
                    name="FileVault",
                    status="ok",
            ...
            if proc.returncode == 0 and "filevault is off" in normalized:
                return ScanResult(
                    name="FileVault",
                    status="fail",
            ...
            return ScanResult(
                name="FileVault",
                status="skip",
                message="FileVault status could not be determined in this process.",
                platform="darwin",
            )
        except Exception:
            return ScanResult(
    ...
    def check_icloud_sync(self) -> ScanResult:
        """Check whether ~/.samantha is inside iCloud Drive sync scope."""
        try:
            ...
            if str(config_path).startswith(str(icloud_path)):
                return ScanResult(
                    name="iCloud Sync",
                    status="warn",
                    message="~/.samantha may be synced to iCloud.",
                    platform="darwin",
                )
            ...
            return ScanResult(
                name="iCloud Sync",
                status="ok",
                message="~/.samantha is not inside iCloud Drive.",
                platform="darwin",
            )
        except Exception:
            return ScanResult(
...
def _resolve_data_boundary_config_path() -> Path:
    env_config = os.environ.get("SAMANTHA_CONFIG")
    if env_config:
...
def _load_data_boundary_config():
    ...
    if config_path is None or not config_path.exists():
        return SamanthaConfig(), root, False, "", root_error
    try:
        return load_config(config_path), root, True, "", root_error
    except Exception as exc:
        return SamanthaConfig(), root, False, f"{type(exc).__name__}: {exc}", root_error
...
def _render_results(results: List[ScanResult]) -> None:
    ...
    console = Console()
    console.print()
    console.print("[bold]Samantha Security Scan[/bold]")
    console.print()
    ...
    if fail_count:
        console.print(
            ...
            "before storing sensitive data with Samantha."
        )
        console.print()
    elif warn_count:
        console.print(
...
def _render_data_boundary_report(
    ...
    *,
    ...
    console = Console()
    console.print()
    console.print("[bold]Samantha Data-Boundary Scan[/bold]")
