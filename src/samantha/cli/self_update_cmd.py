"""`samantha self-update` — upgrade Samantha's framework.

Runs the right upgrade command for how the user installed Samantha:

- PyPI installs get ``pip install --upgrade samantha``.
- uv-tool installs get ``uv tool upgrade samantha``.
...
@click.command(
    "self-update",
    help=(
        "Upgrade Samantha's framework to the latest release. Detects how you "
...
def self_update(check: bool, yes: bool) -> None:
    ...
    click.echo(f"Current Samantha version: v{current}")
    ...
    click.echo("\nUpgrade complete. Re-run `samantha --version` to confirm.")
