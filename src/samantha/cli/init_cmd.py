"""``samantha init`` — detect hardware, generate config, write to disk."""
...
from samantha.cli._bootstrap import detect_cloud_keys
from samantha.cli.model import find_model_spec, hf_download, ollama_pull
...
# Engines supported by ``samantha init --engine``.
_SUPPORTED_ENGINES = [
    "ollama",
    "vllm",
    "sglang",
...
def _next_steps_text(engine: str, model: str = "") -> str:
    ...
    steps: dict[str, str] = {
        "ollama": (
            ...
            '     samantha ask "Hello"\n'
            "\n"
            "  Run `samantha doctor` to verify your setup."
        ),
        "vllm": (
            "Next steps:\n"
            "\n"
...
def _quick_privacy_check(console: Console) -> None:
    ...
    console.print()
    console.print("  Run [cyan]samantha scan[/cyan] for a full environment audit.")
...
@click.option(
    ...
    default=False,
    hidden=True,
    help="Run init non-interactively; called by the bare-samantha first-run guard.",
)
@click.pass_context
def init(
    ctx: click.Context,
    ...
    from_bare_samantha: bool = False,
) -> None:
    """Detect hardware and generate ~/.samantha/config.toml."""
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "samantha" / "examples"
        )
        ...
        console.print(
            ...
            "[bold]samantha doctor[/bold] to verify."
        )
        return
    ...
    if engine is None and config is None:
        ...
        # Bare-samantha cold path: use the recommended engine non-interactively.
        if from_bare_samantha:
            engine = recommended
        else:
            console.print()
    ...
    # the canonical location that subsequent ``samantha`` commands load.
...
"""
        ...
        console.print(
            ...
            "Run [bold]samantha connect gdrive[/bold] to connect "
            "Google services, then [bold]samantha digest --fresh[/bold]."
        )
    ...
    if not soul_path.exists():
        soul_path.write_text(
            "# Agent Persona\n\nYou are Samantha, a helpful personal AI assistant.\n",
            encoding="utf-8",
        )
    ...
    if not model:
        ...
        if not no_download and not from_bare_samantha and spec:
            ...
            if click.confirm(prompt, default=True):
                ...
                console.print(
                    ...
                    f"    [bold]samantha model pull {model}[/bold]"
                )
