"""``samantha agents`` — persistent agent lifecycle management."""
...
def list_agents() -> None:
    ...
    try:
        ...
        if not agents:
            console.print(
                "[dim]No agents found. Create one with: samantha agents create[/dim]"
            )
            return
...
def _get_system():
    """Build a SamanthaSystem for CLI commands that need scheduler/executor."""
...
def _get_scheduler_and_executor(system=None):
    """Get scheduler + executor from a SamanthaSystem instance."""
    if system is None:
        system = _get_system()
...
def launch():
    ...
    from samantha.agents.manager import AgentManager as _AM
...
def run_agent(agent_id):
    ...
    if not instruction and not pending:
        click.echo(
            ...
            f"'samantha agents ask {agent_id} <message>' to set one.",
            err=True,
        )
        raise SystemExit(1)
...
def learning(agent_id, trigger_run):
    ...
    if trigger_run:
        ...
        from samantha.core.events import EventType, get_event_bus
