"""``samantha compose`` — unified composition CLI for discrete agents and operators."""
...
@click.group()
def compose() -> None:
    """Compose, run, benchmark, and deploy Samantha configurations.
...
def compose_list(kind: Optional[str]) -> None:
    ...
    try:
        from samantha.recipes.loader import discover_recipes

        recipes = discover_recipes(kind=kind)
        if not recipes:
            console.print("[dim]No compositions found.[/dim]")
            console.print(
                "[dim]Place TOML recipes in src/samantha/recipes/data/ "
                "or ~/.samantha/recipes/[/dim]"
            )
            return
...
def compose_show(name: str) -> None:
    ...
    console = Console(stderr=True)
    try:
        from samantha.recipes.loader import resolve_recipe
...
def compose_stop(name: str) -> None:
    ...
    try:
        ...
        from samantha.recipes.loader import _PROJECT_OPERATORS_DIR
