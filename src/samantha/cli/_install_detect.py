"""Detect how Samantha was installed so we can show the right upgrade
command (and run the right upgrade command for ``samantha self-update``).

Three install paths are supported today:

- **PyPI** (``pip install samantha``). The package lives somewhere
  inside ``site-packages``. Upgrade with ``pip install --upgrade samantha``.
- **uv tool** (``uv tool install samantha``). Lives in a uv-managed
  isolated venv under ``~/.local/share/uv/tools/``. Upgrade with
  ``uv tool upgrade samantha``.
...
We detect by inspecting ``samantha.__file__``. If we can't tell with
...
def detect_install() -> InstallInfo:
    ...
    Cheap: just walks the parent directories of ``samantha.__file__``
    once and checks for marker directories. No subprocess calls.
    """
    try:
        import samantha

        pkg_file = Path(samantha.__file__).resolve()
    except Exception:
        return InstallInfo(
            kind="unknown",
            upgrade_command="pip install --upgrade samantha",
        )
