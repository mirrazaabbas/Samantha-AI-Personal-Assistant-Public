"""``samantha deep-research-setup`` — auto-detect local sources, ingest, and chat.
...
def _instantiate_connector(connector_id: str, config: Dict[str, Any]) -> Any:
    ...
    if connector_id == "apple_notes":
        from samantha.connectors.apple_notes import AppleNotesConnector

        return AppleNotesConnector(db_path=config.get("db_path", ""))
    elif connector_id == "imessage":
        from samantha.connectors.imessage import IMessageConnector

        return IMessageConnector(db_path=config.get("db_path", ""))
    elif connector_id == "obsidian":
        from samantha.connectors.obsidian import ObsidianConnector

        return ObsidianConnector(vault_path=config.get("vault_path", ""))
    elif connector_id == "gmail_imap":
        from samantha.connectors.gmail_imap import GmailIMAPConnector

        return GmailIMAPConnector()
    elif connector_id == "outlook":
        from samantha.connectors.outlook import OutlookConnector
        ...
        from samantha.connectors.notion import NotionConnector

        return NotionConnector()
    elif connector_id == "granola":
        from samantha.connectors.granola import GranolaConnector
...
def ingest_sources(
    ...
    *,
    ...
    state_db:
        ...
        ``~/.samantha/sync_state.db`` when empty.
...
def _launch_chat(store: KnowledgeStore, console: Console) -> None:
    ...
    from samantha.connectors.retriever import TwoStageRetriever
    from samantha.engine.ollama import OllamaEngine
    from samantha.tools.knowledge_search import KnowledgeSearchTool
    from samantha.tools.knowledge_sql import KnowledgeSQLTool
    from samantha.tools.scan_chunks import ScanChunksTool
