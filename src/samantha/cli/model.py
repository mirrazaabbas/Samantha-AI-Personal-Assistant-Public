"""``samantha model`` — model management subcommands."""
...
from samantha.core.registry import ModelRegistry
from samantha.engine import discover_engines, discover_models
from samantha.intelligence import merge_discovered_models, register_builtin_models
...
def _convert_mlx(hf_repo: str, output: str, mlx_4bit: bool, console: Console) -> bool:
    ...
    try:
        ...
        console.print(
            ...
            'Install it: [cyan]pip install "samantha[inference-mlx]"[/cyan]'
        )
        return False
...
def convert(
    ...
    if engine in ("vllm", "sglang"):
        console.print(
            ...
            f"[cyan]Start with:[/cyan] samantha host {shlex.quote(hf_repo)} "
            f"--backend {engine}"
        )
        return
    ...
    if engine == "mlx":
        ...
        hint = f"samantha host {shlex.quote(str(artifact))} --backend mlx"
        ...
        if engine == "ollama":
            ...
            hint = f"samantha chat --engine ollama --model {shlex.quote(ollama_name)}"
            title = "GGUF conversion and Ollama import complete"
        else:
            hint = f"samantha host {shlex.quote(str(artifact))} --backend llamacpp"
