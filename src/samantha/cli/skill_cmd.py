...
def _get_resolver(source: str, url: str = ""):
    ...
    if source == "hermes":
        from samantha.skills.sources.hermes import HermesResolver

        return HermesResolver()
    if source == "openclaw":
        from samantha.skills.sources.openclaw import OpenClawResolver
...
def install(query: str, with_scripts: bool, force: bool, url: str, yes_dangerous: bool):
    ...
    Example: ``samantha skill install hermes:apple-notes``
    """
    console = Console()
    ...
    from samantha.skills.importer import SkillImporter
    from samantha.skills.parser import SkillParser
    from samantha.skills.tool_translator import ToolTranslator
...
def remove(skill_name: str, yes: bool):
    ...
    Searches ``~/.samantha/skills/`` and ``./skills`` for a directory whose
...
def search(query: str, source: str):
    ...
    console.print(
        ...
        f"Install with: samantha skill install <source>:<name>[/dim]"
    )
...
def discover(min_frequency: int, min_outcome: float, dry_run: bool) -> None:
    ...
    discovered skill manifests under ~/.samantha/skills/discovered/."""
    console = Console()
    ...
    if dry_run:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="samantha-discover-dryrun-"))
        try:
