"""``samantha channels`` — manage messaging channels for the agent."""
...
def channels_status() -> None:
    ...
    from samantha.channels.imessage_daemon import is_running
    ...
    if is_running():
        ...
        table.add_row(
            "iMessage",
            "[dim]stopped[/dim]",
            "samantha channels imessage-start <contact>",
        )
    ...
    if background:
        ...
        proc = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "samantha.channels.imessage_daemon",
                "--chat",
                chat_identifier,
            ],
        ...
        from samantha.connectors.retriever import (
            TwoStageRetriever,
        )
        from samantha.connectors.store import KnowledgeStore
        from samantha.engine.ollama import OllamaEngine
        from samantha.tools.knowledge_search import (
            KnowledgeSearchTool,
        )
        from samantha.tools.knowledge_sql import (
            KnowledgeSQLTool,
        )
...
def imessage_stop() -> None:
    ...
    from samantha.channels.imessage_daemon import stop_daemon
