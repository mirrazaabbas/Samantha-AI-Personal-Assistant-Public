...
from samantha.engine._http_async import (
...
def _default_num_ctx() -> int:
    """Default context window (tokens). Override with ``SAMANTHA_NUM_CTX``.
    ...
    """
    try:
        return int(os.environ.get("SAMANTHA_NUM_CTX", "16384"))
    except ValueError:
        return 16384


def _default_num_gpu() -> int | None:
    """Optional default GPU offload override for constrained environments."""
    raw = os.environ.get("SAMANTHA_NUM_GPU")
    if raw is None:
        return None
    try:
        return int(raw)
...
def _ollama_request_options(
    *,
    ...
    elif (default_num_gpu := _default_num_gpu()) is not None:
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if response_format is not None:
            from samantha.engine._stubs import ResponseFormat
