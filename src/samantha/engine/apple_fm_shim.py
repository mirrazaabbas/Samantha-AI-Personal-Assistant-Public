...
Samantha's own preferred path is the in-process ``afm`` engine
(``samantha.engine.apple_fm``), which avoids an HTTP hop and a second
process whose CPU energy would otherwise land inside the same measurement
window.

Usage:
    uvicorn samantha.engine.apple_fm_shim:app \
