"""MCP Server — wraps Samantha tools as MCP-discoverable tools."""
...
from samantha.mcp.protocol import (
    INTERNAL_ERROR,
    INVALID_PARAMS,
    METHOD_NOT_FOUND,
    MCPRequest,
...
class MCPServer:
    """MCP server that exposes Samantha tools via JSON-RPC.
    ...
    def _auto_discover_tools() -> List[BaseTool]:
        ...
        try:
            ...
            pass
        try:
            from samantha.tools.web_search import WebSearchTool

            _tool_classes.append(WebSearchTool)
        except ImportError:
            pass
        try:
            from samantha.tools.code_interpreter import CodeInterpreterTool

            _tool_classes.append(CodeInterpreterTool)
        except ImportError:
            pass
        try:
            from samantha.tools.repl import ReplTool
        ...
        # Storage MCP tools
        try:
            from samantha.tools.storage_tools import (
                MemoryIndexTool,
                MemoryRetrieveTool,
                MemorySearchTool,
                MemoryStoreTool,
        ...
        # Channel MCP tools
        try:
            from samantha.tools.channel_tools import (
                ChannelListTool,
                ChannelSendTool,
                ChannelStatusTool,
            )
