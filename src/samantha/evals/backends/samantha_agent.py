"""Samantha Agent backend — agent-level inference with tool calling."""
...
from samantha.evals.backends._commit_util import samantha_commit
from samantha.evals.core.backend import InferenceBackend


class SamanthaAgentBackend(InferenceBackend):
    """Agent-level inference via SystemBuilder + SamanthaSystem.ask().

    Supports tool calling via the agent harness. Works for both local
    and cloud models.
    """

    backend_id = "samantha-agent"
    framework_name = "samantha"
    ...
    def __init__(
        ...
        if base_url:
            ...
            from samantha.evals.backends._endpoint_util import (
        ...
        # Override the agent's per-run turn budget. SamanthaConfig.agent.max_turns
    ...
    def framework_commit_value(self) -> str:
        """Samantha repo HEAD commit (for telemetry tagging)."""
        from samantha.evals.backends._commit_util import samantha_commit

        return samantha_commit()
    ...
    def generate_full(
        ...
        *,
        ...
        return {
            ...
            "framework": "samantha",
            "framework_commit": samantha_commit(),
            "error": None,
        }
