...
class TaskScheduler:
    ...
    system:
        Optional ``SamanthaSystem`` instance for executing prompts.
    poll_interval:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._poll_loop, daemon=True, name="samantha-scheduler"
        )
        self._thread.start()
    ...
    def create_task(
        ...
        **kwargs: Any,
        ...
        task = ScheduledTask(
            id=kwargs.get("task_id") or uuid.uuid4().hex[:16],
            prompt=prompt,
        ...
        try:
            if self._system is not None:
                ...
                if meta.get("operator_id"):
                    ...
                    ask_kwargs["max_turns"] = meta.get("max_turns")
