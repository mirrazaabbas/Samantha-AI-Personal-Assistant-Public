...
from samantha.agents.tool_resolver import (
...
from samantha.server.model_capabilities import is_embed_only_model
...
logger = logging.getLogger("samantha.server.agent_manager")
...
def _resolve_memory_backend(config: Any) -> Any:
    ...
    if config is None:
        return None
    try:
        import samantha.tools.storage  # noqa: F401
...
class _LightweightSystem:
    ...
    full SamanthaSystem (which picks a random model from Ollama)."""
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from samantha.core.credentials import TOOL_CREDENTIALS
    from samantha.core.registry import ChannelRegistry, ToolRegistry
...
def _replay_history_messages(
    ...
    """
    from samantha.core.types import Message, Role, ToolCall
...
def _get_mcp_tools_locked(
    ...
    from samantha.core.config import load_config, resolve_mcp_servers
...
async def _stream_managed_agent(
    *,
    ...
    if agent_type == "deep_research":
        ...
        if dr_tools:
            ...
            async def generate_deep_research():
                ...
                from samantha.agents.deep_research import DeepResearchAgent
    ...
    from samantha.tools._stubs import ToolExecutor
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from samantha.core.types import ToolCall as MsgToolCall
...
def create_agent_manager_router(
    ...
    async def bind_channel(
        ...
        if req.channel_type == "imessage":
            ...
            if identifier:
                try:
                    from samantha.channels.imessage_daemon import (
                        is_running,
                        run_daemon,
                    )
                    ...
                    if not is_running():
                        ...
                        if engine:
                            ...
                            if tools:
                                from samantha.agents.deep_research import (
                                    DeepResearchAgent,
                                )
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from samantha.channels.sendblue import (
                        SendBlueChannel,
                    )
                    ...
                    if bridge and hasattr(bridge, "_channels"):
                        ...
                        from samantha.server.channel_bridge import (
                            ChannelBridge,
                        )
                        from samantha.server.session_store import (
                            SessionStore,
                        )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from samantha.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from samantha.channels.slack_daemon import (
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from samantha.core.credentials import save_credential
    ...
    def remove_tool_credential(tool_name: str, key: str):
        from samantha.core.credentials import delete_credential
    ...
    def credential_status(tool_name: str):
        from samantha.core.credentials import get_credential_status
    ...
    async def sendblue_test(request: Request):
        ...
        try:
            payload: Dict[str, str] = {
                "number": to_number,
                "content": (
                    "Hello from your Samantha agent! "
