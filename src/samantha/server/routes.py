...
from samantha.server.models import (
    ChatCompletionChunk,
...
def _ensure_identity_prompt(messages: list[Message], app_config) -> list[Message]:
    ...
    identity_already_applied = any(
        message.metadata.get("samantha_identity_prompt") for message in system_messages
    )
    ...
    if not caller_supplied_system and not identity_already_applied:
        try:
            ...
            logging.getLogger("samantha.server").debug(
    ...
    if system_messages:
        ...
        if prompt:
            ...
            metadata["samantha_identity_prompt"] = True
    ...
    if (
        ...
        try:
            ...
            if query_text:
                ...
                from samantha.server.models import ChatMessage
...
def _uses_direct_cloud_router(engine: Any, model: str) -> bool:
    ...
    from samantha.server.cloud_router import is_cloud_model
...
def _handle_direct(
    ...
    if bus:
        from samantha.telemetry.instrumented_engine import InstrumentedEngine
        from samantha.telemetry.wrapper import instrumented_generate
...
def _handle_agent(
    ...
    *,
    ...
    if audio_path:
        ...
        from samantha.server.models import AudioMeta
    ...
    *,
    ...
    async def generate():
        ...
        try:
            ...
            logging.getLogger("samantha.server").error(
...
async def _handle_stream(
    ...
    *,
    ...
    from samantha.server.cloud_router import stream_cloud, stream_local
...
async def list_models(request: Request) -> ModelListResponse:
    ...
    """
    from samantha.server.cloud_router import is_cloud_model, list_local_models
...
async def savings(request: Request):
    ...
    from samantha.server.savings import compute_savings, savings_to_dict
...
async def security_scan():
    ...
    from samantha.cli.scan_cmd import PrivacyScanner
