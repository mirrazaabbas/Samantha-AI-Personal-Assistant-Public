...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import samantha.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from samantha.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from samantha.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from samantha.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from samantha.tools.agent_tools import AgentSendTool
...
def _get_memory_backend(request: Request):
    ...
    cannot be built because the mandatory ``samantha_rust`` extension is not
    ...
    if backend is None:
        from samantha.tools.storage._stubs import MemoryBackendUnavailable

        try:
            from samantha.tools.storage.sqlite import SQLiteMemory
...
async def memory_config(request: Request):
    ...
    ``backend_type: sqlite``) when the native ``samantha_rust`` extension is
...
async def memory_index(req: MemoryIndexRequest, request: Request):
    ...
    try:
        ...
        from samantha.security.file_policy import is_sensitive_file
        from samantha.tools.storage.ingest import ingest_path

        target = Path(req.path).expanduser().resolve()
        if not target.exists():
            raise HTTPException(status_code=404, detail=f"Path not found: {req.path}")

        # Sandbox: when workspace roots are configured via SAMANTHA_WORKSPACE
        ...
        workspace = os.environ.get("SAMANTHA_WORKSPACE", "").strip()
        if workspace:
            roots = [
...
def _telemetry_db_path(request: Request) -> Path:
    ...
    ``DEFAULT_CONFIG_DIR`` is fixed when :mod:`samantha.core.config` is
    imported, so it cannot honor a later ``SAMANTHA_HOME`` override.  The
...
async def telemetry_stats(request: Request):
    ...
    try:
        ...
        from samantha.telemetry.aggregator import TelemetryAggregator
...
async def list_skills(request: Request):
    ...
    try:
        from samantha.core.registry import SkillRegistry
...
async def install_skill(request: Request):
    ...
    return {
        ...
        "message": "Use TOML files in ~/.samantha/skills/",
    }
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from samantha.sessions.store import SessionStore
...
async def prometheus_metrics(request: Request):
    ...
    try:
        ...
        lines = [
            "# HELP samantha_requests_total Total requests processed",
            "# TYPE samantha_requests_total counter",
            f"samantha_requests_total {stats.total_calls}",
            "# HELP samantha_tokens_total Total tokens generated",
            "# TYPE samantha_tokens_total counter",
            f"samantha_tokens_total {stats.total_tokens}",
            "# HELP samantha_latency_avg_ms Average latency in milliseconds",
            "# TYPE samantha_latency_avg_ms gauge",
            f"samantha_latency_avg_ms {avg_latency_ms}",
        ]
        from starlette.responses import PlainTextResponse
...
def _record_ws_trace(
    ...
    *,
    ...
    if trace_store is None or not result:
        return
    from samantha.traces.collector import record_response_trace
...
async def websocket_chat_stream(websocket: WebSocket):
    ...
    """
    from samantha.server.auth_middleware import authenticate_websocket
...
async def learning_stats(request: Request):
    ...
    # Skill discovery
    try:
        from samantha.learning.agents.skill_discovery import SkillDiscovery
...
def include_all_routes(app) -> None:
    ...
    from samantha.server.approval_routes import (
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from samantha.server.agent_manager_routes import (  # noqa: PLC0415
    ...
    # singleton is a *different* bus that nothing in `samantha serve` ever
    # publishes to, so events silently never reached this endpoint.
    try:
        from samantha.core.events import get_event_bus
        from samantha.server.ws_bridge import create_ws_router
