"""FastAPI application factory for the Samantha API server."""
...
from samantha.server.analytics_routes import router as analytics_router
from samantha.server.api_routes import include_all_routes
from samantha.server.comparison import comparison_router
from samantha.server.connectors_router import create_connectors_router
from samantha.server.dashboard import dashboard_router
from samantha.server.digest_routes import create_digest_router
from samantha.server.research_router import router as research_router
from samantha.server.routes import router
from samantha.server.upload_router import router as upload_router
        ...
        for agent in mgr.list_agents():
            ...
            for b in bindings:
                ...
                if bridge and hasattr(bridge, "_channels"):
                    ...
                    from samantha.server.channel_bridge import ChannelBridge
                    from samantha.server.session_store import SessionStore

                    session_store = SessionStore()
                    engine = getattr(app.state, "engine", None)
                    dr_agent = None
                    if engine:
                        from samantha.server.agent_manager_routes import (
...
def create_app(
    ...
    *,
    ...
    config:
        Optional SamanthaConfig for other settings.
    """
    app = FastAPI(
        title="Samantha API",
        description="OpenAI-compatible API server for Samantha",
        version="0.1.0",
    )
    ...
    try:
        from samantha.analytics import (
            AnalyticsClient,
            EventBridge,
    ...
    try:
        from samantha.server.middleware import create_security_middleware
    ...
    if api_key:
        try:
            from samantha.server.auth_middleware import AuthMiddleware
    ...
    if webhook_config:
        try:
            from samantha.server.webhook_routes import (
