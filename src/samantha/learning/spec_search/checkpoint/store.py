...
A thin wrapper over a local git repository at ``<samantha_home>/.git``.
...
from samantha.learning.spec_search.storage.paths import (
    ConfigurationError,
    _find_source_root,
)
...
class CheckpointStore:
    ...
    root :
        ...
        ``~/.samantha/`` (or ``$SAMANTHA_HOME``); for tests it's a
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the Samantha source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"Samantha source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "spec-search@samantha.local")
            self._git("config", "user.name", "Samantha Spec Search")
        ...
        # Allow empty so init succeeds even on a brand-new samantha home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",
