...
must NEVER be inside the Samantha source tree. ``resolve_spec_search_root``
walks up from this module's ``__file__`` looking for a ``pyproject.toml`` that
identifies the Samantha source root, then refuses to operate if the resolved
root is inside it. Defense in depth — if a user accidentally points
``SAMANTHA_HOME`` at the repo, the system fails loudly instead of silently
...
from samantha.core.paths import ConfigurationError, get_config_dir
from samantha.security.file_utils import secure_mkdir

# ``ConfigurationError`` is re-exported from ``samantha.core.paths`` (it used
...
def _find_source_root() -> Path | None:
    ...
    Returns the directory containing the Samantha ``pyproject.toml``, or
...
def _resolve_samantha_home() -> Path:
    """Resolve the Samantha home directory via the unified core resolver.

    Delegates to ``get_config_dir`` so spec-search honors the same env-aware
    resolution (SAMANTHA_HOME and XDG) as the rest of the framework.
    """
...
def resolve_spec_search_root() -> Path:
    ...
    The root is ``$SAMANTHA_HOME/learning`` (or ``~/.samantha/learning``
    by default). Raises ``ConfigurationError`` if the resolved path lies
    inside the Samantha source tree, to prevent dev artifacts from leaking
    into the repo.
    """
    home = _resolve_samantha_home()
    ...
    if source_root is not None:
        try:
            ...
            raise ConfigurationError(
                f"SAMANTHA_HOME ({home}) is inside the source tree "
                f"({source_root}). Spec search refuses to write runtime "
                "artifacts inside the Samantha repo. Set SAMANTHA_HOME "
