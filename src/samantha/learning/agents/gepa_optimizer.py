...
Uses GEPA's adapter pattern to bridge Samantha traces into GEPA's
...
from samantha.core.config import GEPAOptimizerConfig
from samantha.core.registry import LearningRegistry
from samantha.learning._stubs import AgentLearningPolicy
...
class SamanthaGEPAAdapter:
    """Implements GEPA's adapter protocol for Samantha agents.
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": (
                    "gepa not installed (pip install 'samantha[learning-gepa]')"
                ),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = SamanthaGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "SamanthaGEPAAdapter"]
