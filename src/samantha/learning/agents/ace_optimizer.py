...
The wrapper adapts Samantha traces into ACE's
...
sidecar overlay under ``~/.samantha/learning/ace/<task>/`` for the
...
from samantha.core.config import ACEOptimizerConfig
...
class _TraceDataProcessor:
    """Adapter that exposes Samantha traces in ACE's three-method API.
...
def _traces_to_samples(traces: List[Any]) -> List[Dict[str, Any]]:
    """Convert Samantha trace records to ACE's sample dict shape."""
...
class ACEAgentOptimizer:
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_ACE:
            return {
                "status": "error",
                "reason": ("ace not installed (pip install 'samantha[learning-ace]')"),
            }
