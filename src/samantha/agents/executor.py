...
from samantha.agents.errors import (
    AgentTickError,
    EscalateError,
    FatalError,
    classify_error,
...
from samantha.agents.tool_resolver import resolve_agent_tools
...
class AgentExecutor:
    ...
    Constructor receives a SamanthaSystem reference for access to engine,
    ...
    def set_system(self, system: Any) -> None:
        """Deferred system injection — called after SamanthaSystem is constructed."""
        self._system = system
    ...
    def _invoke_agent_impl(self, agent: dict) -> AgentResult:
        ...
        from samantha.agents import AgentRegistry
        ...
        # Resolve engine + model from SamanthaSystem
        engine = self._system.engine if self._system else None
        if engine is None:
            raise FatalError("No engine available in SamanthaSystem")
        model = (
            config.get("model")
        ...
        if (
            ...
            if not mcp_tools:
                try:
                    from samantha.tools.mcp_adapter import MCPToolAdapter
        ...
        # agent's own ToolExecutor. Set by CLI paths like `samantha agents ask`
        ...
        if self._system is not None:
            ...
            # agents, mirroring the one-shot `samantha ask` path so they no
    ...
    def _save_trace(
        ...
        from samantha.core.types import StepType, Trace, TraceStep
