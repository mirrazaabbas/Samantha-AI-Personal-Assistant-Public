"""Load system prompt and few-shot overrides from $SAMANTHA_HOME.
...
- System prompts: ``$SAMANTHA_HOME/agents/{name}/system_prompt.md``
- Few-shot exemplars: ``$SAMANTHA_HOME/agents/{name}/few_shot.json``
...
def _samantha_home() -> Path:
    """Resolve the Samantha root, honoring SAMANTHA_HOME / XDG_DATA_HOME."""
...
def load_system_prompt_override(agent_name: str) -> str | None:
    ...
    Looks for ``$SAMANTHA_HOME/agents/<agent_name>/system_prompt.md``.
    ``SAMANTHA_HOME`` defaults to ``~/.samantha`` when unset.
    """
    home = _samantha_home()
...
def load_few_shot_exemplars(
    ...
    Looks for ``$SAMANTHA_HOME/agents/<agent_name>/few_shot.json``.
