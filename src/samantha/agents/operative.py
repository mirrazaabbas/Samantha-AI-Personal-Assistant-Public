...
class OperativeAgent(ToolUsingAgent):
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        # Scheduled agents do not receive an interactive user's timestamp. Give
        # them an authoritative, machine-generated local clock so dates cannot
        # drift to values remembered by the model or persisted session history.
        local_now = datetime.now().astimezone()
        sys_parts.append(
            "## Authoritative Runtime Context\n"
            f"Current local date and time: {local_now.isoformat(timespec='seconds')}\n"
            "This timestamp overrides conflicting dates in model knowledge, "
            "session history, memories, and tool data."
        )
        ...
        # Honor SOUL.md / MEMORY.md / USER.md persona files like `samantha ask`,
        ...
        for _turn in range(self._max_turns):
            ...
            if not content:
                completed = [
                    f"- {item.tool_name}: {item.content[:500]}"
                    for item in all_tool_results[-5:]
                ]
                content = "Tick reached its bounded turn budget."
                if completed:
                    content += "\n\nCompleted tool work:\n" + "\n".join(completed)
