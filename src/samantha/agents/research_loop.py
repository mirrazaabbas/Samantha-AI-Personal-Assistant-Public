...
from samantha.connectors.hybrid_search import HybridSearch, SearchHit
...
Strategy:
  ...
  7. You have a clarify tool. Only use it AFTER at least one search attempt. Use it when: you found multiple ambiguous matches (e.g. 3 different people named Alex), search returned zero results and the query might need reframing, or the scope is too broad to synthesize meaningfully. Never use clarify before searching — always try first.
