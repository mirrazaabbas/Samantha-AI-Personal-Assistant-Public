...
both forced by the Samantha environment and documented inline:
...
* in-tool correctness check — the original ``answer`` tool LLM-judges the
  prediction against the gold answer inside ``call_tool``. Samantha
...
def run_answer(
    ...
    *,
    ...
    in-tool LLM correctness check is dropped — Samantha scores
    downstream.
    """
...
def run_search(
    ...
    *,
    ...
    Samantha substitution for the missing FAISS wiki index).
    """
    prompt = (
        context_str.strip()
        + "\n\n"
