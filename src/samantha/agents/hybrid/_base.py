...
Why not just use Samantha's :class:`InferenceEngine` for both? Two reasons:
...
from samantha.agents._stubs import AgentContext, AgentResult, BaseAgent
from samantha.agents.hybrid._openai_retry import patch_openai_globally
from samantha.agents.hybrid._prices import (
    NO_TEMP_PREFIXES,
    is_gpt5_family,
...
def tavily_search_context(
    ...
    *,
    ...
    """Run Samantha WebSearchTool and return accounting-friendly metadata."""
...
class LocalCloudAgent(BaseAgent):
    ...
    def _call_openrouter(
        ...
        *,
        ...
        pass the Samantha-engine-style ``"openrouter/<provider>/<model>"``
        prefix (see ``src/samantha/engine/cloud.py``) — we strip it here.
