...
harness — each is registered as a standard Samantha agent so the rest
of the platform (SDK, CLI, distillation, evals) can use them like any
other agent. Results live under ``$SAMANTHA_HYBRID_EXPERIMENTS_DIR``
(defaults to ``~/.samantha/experiments/hybrid/``).
...
Items in parentheses are what the *paper* trains. These Samantha ports
...
`src/samantha/evals/scorers/swebench_harness.py` (next to the existing
structural scorer).
...
# 3. Run a smoke cell
.venv/bin/python -m samantha.agents.hybrid.runner \
    --cell minions-gaia-qwen27b-opus-3
```

Outputs land in
`$SAMANTHA_HYBRID_EXPERIMENTS_DIR/runs/<cell>/{results.jsonl,summary.json,config.json,logs/}`
(defaults to `~/.samantha-hybrid/experiments/`). The schema matches the
...
```bash
src/samantha/agents/hybrid/scripts/new_experiment.sh \
...
(`~/.samantha/experiments/hybrid/docs/results.md`) at full N —
...
these Samantha cells should reproduce the harness numbers within
