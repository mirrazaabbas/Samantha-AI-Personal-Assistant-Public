...
2. Wraps ``chat.completions.create`` with:
   ...
   - A **per-org semaphore** (``SAMANTHA_OPENAI_MAX_CONCURRENCY``,
...
- ``SAMANTHA_OPENAI_MAX_CONCURRENCY`` (default ``4``) — semaphore
  capacity. Set to e.g. ``2`` if the wall is still hit; set to ``0`` to
  disable throttling entirely (passes through to the SDK).
- ``SAMANTHA_OPENAI_MAX_RETRIES`` (default ``8``) — outer retry loop
  cap (separate from the SDK's own ``max_retries``).
- ``SAMANTHA_OPENAI_RETRY_BASE`` (default ``2.0``) — base seconds for
  exponential backoff. Schedule is ``min(60, base * 2**attempt) * jitter``.
- ``SAMANTHA_OPENAI_RETRY_CAP`` (default ``60.0``) — max single-step
  sleep in seconds.
"""
...
_MAX_CONCURRENCY = _env_int("SAMANTHA_OPENAI_MAX_CONCURRENCY", 4)
_MAX_RETRIES = _env_int("SAMANTHA_OPENAI_MAX_RETRIES", 8)
_RETRY_BASE = _env_float("SAMANTHA_OPENAI_RETRY_BASE", 2.0)
_RETRY_CAP = _env_float("SAMANTHA_OPENAI_RETRY_CAP", 60.0)
