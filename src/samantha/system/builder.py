"""Config-driven fluent builder that wires up a SamanthaSystem."""
...
from samantha.core.config import SamanthaConfig, load_config
from samantha.core.events import EventBus, get_event_bus
...
from samantha.tools._stubs import BaseTool, ToolExecutor

logger = logging.getLogger(__name__)


class SystemBuilder:
    """Config-driven fluent builder for SamanthaSystem."""

    def __init__(
        self,
        config: Optional[SamanthaConfig] = None,
        *,
    ...
    def engine_instance(
        ...
        ``samantha eval --base-url``). ``build()`` health-checks the instance
    ...
    def build(self) -> SamanthaSystem:
        """Construct a fully wired SamanthaSystem."""
    ...
    def _build(self) -> SamanthaSystem:
        ...
        if telemetry_enabled and config.telemetry.gpu_metrics:
            try:
                from samantha.telemetry.energy_monitor import (
            ...
            if energy_monitor is None:
                try:
                    from samantha.telemetry.gpu_monitor import GpuMonitor
        ...
        from samantha.security import setup_security

        sec = setup_security(config, engine, bus)
        engine = sec.engine

        if telemetry_enabled:
            from samantha.telemetry.instrumented_engine import (
                InstrumentedEngine,
            )
        ...
        if config.agent_manager.enabled:
            try:
                from samantha.agents.manager import AgentManager
        ...
        if agent_manager is not None:
            try:
                from samantha.agents.executor import AgentExecutor
                from samantha.agents.scheduler import AgentScheduler
    ...
    def _resolve_engine(self, config: SamanthaConfig):
        ...
        from samantha.engine._discovery import get_engine
    ...
    def _resolve_model(self, config: SamanthaConfig, engine: InferenceEngine) -> str:
        if self._model:
            return self._model
    ...
    def _resolve_memory(self, config):
        try:
            import samantha.tools.storage  # noqa: F401 -- trigger registration
            from samantha.core.registry import MemoryRegistry
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            import samantha.channels  # noqa: F401 -- trigger registration
            from samantha.core.registry import ChannelRegistry
            from samantha.system._channel_kwargs import build_channel_kwargs
    ...
    def _resolve_tools(
        ...
        if config.tools.mcp.enabled and config.tools.mcp.servers:
            try:
                from samantha.core.config import resolve_mcp_servers
    ...
    def _setup_sandbox(self, config):
        ...
        if not sandbox_enabled:
            return None
        try:
            from samantha.sandbox.runner import ContainerRunner
    ...
    def _setup_scheduler(self, config, bus):
        ...
        try:
            ...
            from samantha.scheduler.scheduler import TaskScheduler
    ...
    @staticmethod
    def _setup_learning_orchestrator(config: SamanthaConfig):
        if not config.learning.training_enabled:
            return None
        try:
            from samantha.core.config import DEFAULT_CONFIG_DIR
            from samantha.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from samantha.learning.training.lora import LoRATrainingConfig
            from samantha.traces.store import TraceStore
    ...
    def _discover_external_mcp(self, server_cfg) -> List[BaseTool]:
        ...
        from samantha.mcp.transport import StdioTransport, StreamableHTTPTransport
        from samantha.tools.mcp_adapter import MCPToolProvider
