...
class QueryOrchestrator:
    ...
    def _run_agent(
        ...
        *,
        ...
        if getattr(agent_cls, "accepts_tools", False):
            ...
            agent_kwargs["max_turns"] = (
                max_turns if max_turns is not None else s.config.agent.max_turns
            )
    ...
    def _build_tools(self, tool_names: List[str]) -> List[BaseTool]:
        ...
        tools: List[BaseTool] = []
        dynamic_modules = {
            "calendar_upcoming": "samantha.tools.apple_calendar",
            "calendar_search": "samantha.tools.apple_calendar",
            "long_task": "samantha.tools.creative_media",
            "memory_retrieve": "samantha.tools.storage_tools",
            "memory_search": "samantha.tools.storage_tools",
            "memory_store": "samantha.tools.storage_tools",
            "text_to_speech": "samantha.tools.text_to_speech",
            "think": "samantha.tools.think",
            "user_profile_manage": "samantha.tools.user_profile_manage",
        }
        for name in tool_names:
            try:
                if not ToolRegistry.contains(name) and name in dynamic_modules:
                    module = import_module(dynamic_modules[name])
                    # A circular startup import can leave the module cached after
                    # a registry reset. Re-register its matching tool class.
                    if not ToolRegistry.contains(name):
                        for value in vars(module).values():
                            if (
                                isinstance(value, type)
                                and getattr(value, "tool_id", None) == name
                            ):
                                ToolRegistry.register_value(name, value)
                                break
                if name == "retrieval" and s.memory_backend:
                    from samantha.tools.retrieval import RetrievalTool

                    tools.append(RetrievalTool(s.memory_backend))
                elif name == "llm":
                    from samantha.tools.llm_tool import LLMTool
