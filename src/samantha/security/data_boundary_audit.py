"""Application data-boundary diagnostics for Samantha.
...
def build_data_boundary_report(
    ...
    *,
    ...
    root_label = "<unresolved-samantha-home>"
    if root is not None:
        try:
    ...
    if root_error:
        ...
        builder.add(
            ...
            status="fail",
            title="Samantha home directory could not be resolved",
            potential_data_path="runtime state root -> local store checks",
            evidence=(
                f"{error_type} while resolving Samantha home; "
                "path details were redacted"
            ),
            recommendation=(
                "Fix SAMANTHA_HOME or XDG_DATA_HOME before relying on "
    ...
    if config_error:
        builder.add(
            ...
            status="fail",
            title="Samantha config could not be loaded",
        ...
        builder.add(
            ...
            status="info",
            title="Samantha config file was not found",
            potential_data_path="configuration defaults -> data-boundary report",
            evidence="config_loaded = false",
            recommendation=(
                "Run `samantha init` before relying on config-derived checks. "
...
def _knowledge_store_exists(root: Path) -> bool:
    """True when knowledge.db exists under the Samantha home (metadata only)."""
...
def _audit_server_exposure(config: Any, builder: _FindingBuilder) -> None:
    ...
    if not _is_loopback_host(host):
        ...
        builder.add(
            ...
            potential_data_path="Samantha HTTP server -> local network interfaces",
    ...
    if a2a_enabled and not a2a_token:
        builder.add(
            ...
            potential_data_path="inbound A2A requests -> Samantha agent runtime",
...
def _audit_channel_settings(config: Any, builder: _FindingBuilder) -> None:
    ...
    for dotted_path, label, service in configured_secrets:
        ...
        builder.add(
            ...
            potential_data_path=f"Samantha channel messages -> {service}",
...
def _derive_verdict(findings: Iterable[DataBoundaryFinding]) -> str:
    ...
    if "config-root-error" in finding_ids:
        return "Samantha home must be fixed before full data-boundary review"
...
def _redact_root(root: str) -> str:
    if root.startswith("<"):
        return root
    path = Path(root)
    if path.name == ".samantha":
        return "~/.samantha"
    if path.name == "samantha":
        return "<xdg-data-home>/samantha"
    return "<samantha-home>"
