...
from ``samantha.core`` must not pull in heavy modules at package init).
"""
...
def _posix_process_state(pid: int) -> str | None:
    ...
    # For our own child processes, waitid can identify an exited process
    # without reaping it. This also works when a sandbox blocks process-list
    # inspection through ps/sysmon.
    if hasattr(os, "waitid") and hasattr(os, "P_PID"):
        try:
            result = os.waitid(
                os.P_PID,
                pid,
