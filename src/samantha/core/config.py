...
User configuration lives at ``~/.samantha/config.toml``.  ``load_config()``
...
from samantha.core.paths import (
    ConfigurationError,
    get_cache_dir,
    get_config_dir,
    get_config_path,
...
if TYPE_CHECKING:
    # Only used by type-checkers (mypy/pyright) for the ``SamanthaConfig.mining``
...
# once at import via the env-aware resolver in ``samantha.core.paths`` (the
# install-script model: ``SAMANTHA_HOME`` / ``XDG_DATA_HOME`` are set before
...
def _ensure_config_dir() -> Path:
    ...
    from samantha.security.file_utils import secure_mkdir
...
def _total_ram_gb() -> float:
    try:
        if platform.system() == "Darwin":
            ...
            # Sandboxed macOS processes may be denied sysctl access even
            # though POSIX page counts remain available.
...
def recommend_model(hw: HardwareInfo, engine: str) -> str:
    ...
    """
    from samantha.intelligence.model_catalog import BUILTIN_MODELS
...
class ACEOptimizerConfig:
    ...
    Install via ``pip install -e samantha[learning-ace]`` once the
    ...
    # most Samantha users have credentials for.
    ...
    # Empty string defaults to ``~/.samantha/learning/ace/<task>/``.
    save_dir: str = ""
    task_name: str = "samantha"
...
class StorageConfig:
    ...
    # the background and persists them across sessions (see samantha.memory).
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        ...
        "an action and an appropriate tool is available, use that tool instead "
        "of merely explaining how the user could do it. For normal conversation, "
        "answer naturally and directly. For potentially destructive, sensitive, "
        "or externally consequential actions, ask for confirmation when required. "
        "Be concise, accurate, practical, and action-oriented. "
        "Address the user as Sir naturally in spoken and conversational replies, "
        "especially confirmations, greetings, acknowledgements, and farewells. "
...
class AnalyticsConfig:
    ...
    usage events sent to the Samantha team's PostHog instance to
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.samantha/whatsapp_auth
    assistant_name: str = "Samantha"
...
class SandboxConfig:
    ...
    enabled: bool = False
    image: str = "samantha-sandbox:latest"
    timeout: int = 300
    workspace: str = ""
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.samantha/scheduler.db
...
class SkillsConfig:
    ...
    index_repo: str = "https://github.com/samantha/skill-index.git"
    index_dir: str = field(
...
class SamanthaConfig:
    """Top-level configuration for Samantha."""
...
# Sections that users may set via ``samantha config set``.
# ``hardware`` is auto-detected and not user-settable.
_SETTABLE_SECTIONS = frozenset(SamanthaConfig.__dataclass_fields__.keys()) - {
    "hardware",
    "mining",
}
...
def validate_config_key(dotted_key: str) -> type:
    ...
    The function walks the ``SamanthaConfig`` dataclass hierarchy using
    ``dataclasses.fields()``.
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import samantha.core.config as _cfg_mod
...
def _migrate_toml_data(data: Dict[str, Any], cfg: "SamanthaConfig") -> None:
...
def load_config(path: Optional[Path] = None) -> SamanthaConfig:
    """Detect hardware, build defaults, overlay TOML overrides.

    Parameters
    ----------
    path:
        Explicit config file. If not set, uses ``SAMANTHA_CONFIG`` when set,
        otherwise ``~/.samantha/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    cfg = SamanthaConfig(hardware=hw)
    cfg.engine.default = recommend_engine(hw)

    if path is not None:
        config_path = Path(path).expanduser().resolve()
    elif os.environ.get("SAMANTHA_CONFIG"):
        config_path = Path(os.environ["SAMANTHA_CONFIG"]).expanduser().resolve()
    else:
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``samantha init``)
# ---------------------------------------------------------------------------
...
# Full reference config: samantha init --full
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.samantha/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# background and persists them. Starts/stops with `samantha serve` and
# `samantha chat`; manage stored facts with `samantha memory list` / `clear`.
[memory]
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.samantha/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.samantha/whatsapp_auth
