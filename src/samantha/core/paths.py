"""Central, env-aware resolution of Samantha's home directory.

Samantha keeps all of its runtime state (config, databases, caches, logs,
...
1. ``$SAMANTHA_HOME`` — explicit override (also honored by the shell
   installer, see ``scripts/install/install.sh``).
2. ``$XDG_DATA_HOME/samantha`` — when ``$XDG_DATA_HOME`` is set, follow the
   XDG Base Directory spec by nesting a single ``samantha`` directory under
...
3. ``~/.samantha`` — the private default runtime directory.
...
override. Modules that previously hardcoded ``Path.home() / ".samantha"``
should call :func:`get_config_dir` (or :func:`get_data_dir` /
:func:`get_cache_dir`) instead.

Defense in depth: the resolved root must never live inside the Samantha
source tree (a misconfigured ``$SAMANTHA_HOME`` pointing at the repo would
...
_DEFAULT_DIR_NAME = ".samantha"
_XDG_SUBDIR_NAME = "samantha"
...
def _find_source_root() -> Path | None:
    """Walk upward from this module to find the Samantha source root.

    Returns the directory containing the Samantha ``pyproject.toml`` (the one
    whose ``name = "samantha"``), or ``None`` when running from an installed
    ...
    for candidate in (here, *here.parents):
        ...
        if py.exists():
            try:
                ...
                continue
            if 'name = "samantha"' in content.lower():
                return candidate
    return None


def _reject_source_tree(path: Path) -> Path:
    """Raise if ``path`` resolves inside the Samantha source tree."""
    ...
    if source_root is not None:
        try:
            ...
            raise ConfigurationError(
                f"Samantha home ({path}) is inside the source tree "
                f"({source_root}). Samantha refuses to write runtime state "
                "inside its own repo. Set SAMANTHA_HOME (or XDG_DATA_HOME) "
                "to a directory outside the repo (default: ~/.samantha)."
            )
    return path


def get_config_dir() -> Path:
    """Resolve Samantha's single root directory, honoring env overrides.

    Precedence: ``$SAMANTHA_HOME`` > ``$XDG_DATA_HOME/samantha`` >
    ``~/.samantha``. The result is always absolute and is rejected if it
    falls inside the Samantha source tree.
    """
    env_home = os.environ.get("SAMANTHA_HOME")
    if env_home:
...
def get_config_path() -> Path:
    """Resolve the path to ``config.toml`` under the Samantha root."""
...
def get_cache_dir() -> Path:
    ...
    Lives at ``<root>/cache`` so caches stay inside the single Samantha
