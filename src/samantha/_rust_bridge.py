"""Single point of contact between Python and the Rust ``samantha_rust`` module.

Every Python module that wants to delegate to Rust should import helpers from
here rather than importing ``samantha_rust`` directly.  The Rust backend is
...
def get_rust_module() -> _types.ModuleType:
    """Return the ``samantha_rust`` module.
    ...
    """
    import samantha_rust  # type: ignore[import-untyped]

    return samantha_rust


def _detect_rust() -> bool:
    """Return ``True`` if the compiled ``samantha_rust`` extension is importable.
...
def scan_result_from_json(json_str: str) -> object:
    ...
    from samantha.security.types import (
        ScanFinding,
        ScanResult,
        ThreatLevel,
    )
...
def injection_result_from_json(json_str: str) -> object:
    ...
    from samantha.security.injection_scanner import (
        InjectionScanResult,
    )
    from samantha.security.types import ScanFinding, ThreatLevel
...
def retrieval_results_from_json(json_str: str) -> list:
    ...
    from samantha.tools.storage._stubs import RetrievalResult
