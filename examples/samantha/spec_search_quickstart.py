...
are local fakes, so you can ``python examples/samantha/spec_search_quickstart.py``
with no API keys and no Ollama / vLLM running. Every "swap this for production"
hookpoint is called out inline.

Configuration is read from ``configs/samantha/examples/spec-search-quickstart.toml``
via the regular ``samantha.core.config.load_config`` machinery — you can copy
that TOML to ``~/.samantha/config.toml`` and tune the gate / stagnation /
reward knobs without editing this file.

Run:

    SAMANTHA_HOME=/tmp/samantha-spec-search-demo \\
...
from samantha.core.config import SpecSearchLearningConfig
from samantha.learning.spec_search.composite_reward import (
    RewardWeights,
    TrainingSample,
    score_batch,
)
from samantha.learning.spec_search.models import (
    BenchmarkSnapshot,
    FailureCluster,
)
from samantha.learning.spec_search.multi_session import SpecSearchLoop
from samantha.learning.spec_search.orchestrator import SpecSearchOrchestrator
from samantha.learning.spec_search.triggers import OnDemandTrigger
...
def _fake_diagnosis() -> Any:
    ...
    from samantha.learning.spec_search.diagnose.runner import DiagnosisResult
...
def build_orchestrator(
    ...
    return SpecSearchOrchestrator.from_config(
        ...
        # Judge: production = samantha.evals.core.scorer.LLMJudgeScorer(...)
        judge=MagicMock(),
...
def main() -> None:
    # In a real deployment, ``load_config()`` reads from ``~/.samantha/config.toml``.
    ...
    #
    # (after copying ``configs/samantha/examples/spec-search-quickstart.toml``
    # to ``~/.samantha/config.toml``)
    ...
    home = Path(
        os.environ.get("SAMANTHA_HOME")
        or tempfile.mkdtemp(prefix="samantha-spec-search-")
    )
    print(f"SAMANTHA_HOME = {home}")
    ...
    with patch(
        "samantha.learning.spec_search.orchestrator.DiagnosisRunner"
    ) as MockDiag:
