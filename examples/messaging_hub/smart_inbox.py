...
def _run_channel(channel: str, model: str, engine_key: str) -> None:
    ...
    help_text = setup_help.get(
        ...
        f"Run: samantha channel list   to see available channels.\n",
    )
    ...
    click.echo("  from samantha import Samantha")
    click.echo(f'  j = Samantha(model="{model}", engine_key="{engine_key}")')
    click.echo("  # Listen for incoming messages on the channel")
    click.echo(f'  # See: samantha channel status  (to verify "{channel}" is connected)')
