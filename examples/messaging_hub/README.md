...
- **Channel integration** — connecting Samantha to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- Samantha installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the Samantha channel docs).
...
Samantha supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/samantha/config.toml` for the full list of channel and agent
options.
...
this via the Samantha scheduler:

```bash
samantha scheduler create "Daily inbox summary" --type cron --value "0 17 * * *"
```

Or use the operator recipe pattern to run a persistent triage agent on a
schedule. See `src/samantha/recipes/data/operators/` for examples.
