...
This tutorial demonstrates how to use Samantha to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.

## What This Demonstrates

- Using the `Samantha` SDK with different agent types (`orchestrator`, `native_react`) and tool sets
- Configuring recurring schedules via TOML and the `samantha scheduler` CLI
...
Register each script as a recurring task with `samantha scheduler create`:
...
uv run python examples/scheduled_ops/daily_digest.py | samantha channel send slack
...
See `samantha channel list` for all available channels.
...
- **Model selection**: Use `--model` to target a specific model, or let Samantha auto-select from what is available.
