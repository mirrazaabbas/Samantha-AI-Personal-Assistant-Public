...
A set of developer-focused scripts that use Samantha tool-using agents
to automate common coding tasks: code review, debugging, and test generation.

## What This Demonstrates

Each script wires up a `Samantha` instance with the `native_react` (ReAct) agent
...
1. **Install Samantha** (from the repo root):
...
```bash
python examples/code_companion/test_gen.py --module src/samantha/tools/calculator.py
```
...
python examples/code_companion/test_gen.py \
    --module src/samantha/tools/calculator.py \
    --framework unittest \
...
`memory_store`, `memory_search`, and more. Run `uv run samantha eval list` or
inspect `src/samantha/tools/` for the full registry.
