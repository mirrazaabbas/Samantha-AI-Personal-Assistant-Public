...
using Samantha. The assistant uses an orchestrator agent loop with web
...
- Samantha installed (`uv sync --extra dev` from the repo root)
...
recipe that can be loaded with `load_recipe()` or passed to the `samantha eval`
runner.
...
User query
  |
  v
Samantha SDK  (model + engine selection)
  |
  v
...
- **Add more tools** -- append tool names to the `tools` list in
  `research.toml` or pass them on the command line. See `samantha agent info
...
- **Use the recipe programmatically** -- load the TOML with
  `samantha.recipes.load_recipe("examples/deep_research/research.toml")` and
...
- [Recipes](../../src/samantha/recipes/) -- composable TOML configs that
