...
Index a directory of documents into Samantha memory and answer questions
with context-augmented retrieval and citations.

## Requirements

- Samantha installed (`git clone https://github.com/mirrazaabbas/Samantha-AI-Personal-Assistant-Public.git && cd Samantha && uv sync` or `uv sync --extra dev`)
...
1. **Index** -- Uses `Samantha.memory.index()` to chunk the documents at
...
Samantha SDK. Adjust `--chunk-size` and `--top-k` to tune the
