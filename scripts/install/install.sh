...
#   SAMANTHA_HOME        Install dir (default: $HOME/.samantha)
#   SAMANTHA_REPO_URL    git repo URL (default: https://github.com/mirrazaabbas/Samantha-AI-Personal-Assistant-Public.git)
#   SAMANTHA_FORCE_WSL   Set 1 to force WSL detection (testing)
...
# uv/git tooling installs to Windows paths the rest of Samantha can't
...
Samantha runs on Windows via WSL2. Two paths:
...
See the WSL2 install guide for the full walkthrough:
  https://github.com/mirrazaabbas/Samantha-AI-Personal-Assistant-Public/getting-started/wsl2/
EOF
        exit 1
        ;;
esac
...
Samantha installs to $HOME/.samantha, not /usr/local. Re-run as your
...
# ---- env ----
# Samantha keeps ALL of its state (install tree + runtime data, configs,
# databases, caches, logs) under a single root so it never clutters $HOME
# beyond one directory. Relocate it by exporting SAMANTHA_HOME before
# running the installer, e.g.:
#     SAMANTHA_HOME=~/apps/samantha curl ... | bash
# The Python runtime honors the same override (and, when SAMANTHA_HOME is
# unset, $XDG_DATA_HOME/samantha if XDG_DATA_HOME is set). With nothing set
# the root is ~/.samantha, so existing installs are untouched.
SAMANTHA_HOME="${SAMANTHA_HOME:-$HOME/.samantha}"
SAMANTHA_REPO_URL="${SAMANTHA_REPO_URL:-https://github.com/mirrazaabbas/Samantha-AI-Personal-Assistant-Public.git}"
SRC_DIR="$SAMANTHA_HOME/src"
VENV_DIR="$SAMANTHA_HOME/.venv"
STATE_DIR="$SAMANTHA_HOME/.state"
SCRIPTS_DIR="$SAMANTHA_HOME/.scripts"
STATE_FILE="$STATE_DIR/install-state.json"

mkdir -p "$SAMANTHA_HOME" "$STATE_DIR" "$SCRIPTS_DIR"

# ---- WSL detection ----
WSL=0
if [[ "${SAMANTHA_FORCE_WSL:-0}" == "1" ]]; then
    WSL=1
...
# Samantha team can see where users drop off during install.
...
#
ANALYTICS_HOST="${SAMANTHA_ANALYTICS_HOST:-https://34.231.106.201.sslip.io}"
ANALYTICS_KEY="${SAMANTHA_ANALYTICS_KEY:-phc_ysKu72QaxzYNmDpHFcesD2ZZAe68zkdWJEKoYYkc5e3n}"
ANON_ID_FILE="$SAMANTHA_HOME/anon_id"
...
clone_repo() {
    ...
    git clone --depth 1 "$SAMANTHA_REPO_URL" "$SRC_DIR"
}
...
write_config() {
    "$VENV_DIR/bin/samantha" _bootstrap --write-config \
...
install_symlinks() {
    ...
    ln -sf "$SCRIPTS_DIR/samantha-wrapper.sh" "$HOME/.local/bin/samantha"
    ln -sf "$SCRIPTS_DIR/samantha-uninstall.sh" "$HOME/.local/bin/samantha-uninstall"
}

# Tracks whether the user needs to source ~/.bashrc / ~/.zshrc / open a
# new terminal before `samantha` will resolve. Set only when ensure_path
...
ensure_path() {
    ...
    if grep -q "Samantha" "$rc" 2>/dev/null; then
...
from samantha.core.config import detect_hardware, recommend_model
hw = detect_hardware()
...
# ---- run ----
echo "Samantha installer"
echo "  install dir: $SAMANTHA_HOME"
echo "  WSL2:        $WSL"
echo
...
step clone_repo         "Clone Samantha repo" clone_repo
step copy_scripts       "Copy install scripts"  copy_scripts
step create_venv        "Create venv"           create_venv
step editable_install   "Install Samantha"    editable_install
...
#
#   PATH ok + model ok   -> "type samantha"
#   PATH new + model ok  -> "source rc && samantha  (or open new terminal)"
#   PATH ok + model bad  -> "model still downloading; samantha doctor"
#   PATH new + model bad -> "source rc && samantha doctor; chat works once download finishes"
#
# `samantha` and `samantha doctor` need PATH equally, so the source/restart
# guidance goes first when PATH was modified.
NEXT_CMD="samantha"
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    NEXT_CMD="samantha doctor"
fi
...
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    cat <<EOF
NOTE: the qwen3.5:2b model didn't finish downloading. 'samantha doctor'
...
Background work continues silently:
  ...
  Run 'samantha doctor' to check status anytime.
EOF
