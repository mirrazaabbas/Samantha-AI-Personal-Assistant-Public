...
#
# State files under $SAMANTHA_HOME/.state/:
...
MANIFEST="$SRC_DIR/rust/crates/samantha-python/Cargo.toml"
...
if uv run maturin develop -m "$MANIFEST" >>"$LOG" 2>&1; then
    ...
    if ! uv run python -c "import samantha_rust" >>"$LOG" 2>&1; then
        rc=$?
        {
            echo "build-extension.sh: maturin succeeded but 'import samantha_rust'"
