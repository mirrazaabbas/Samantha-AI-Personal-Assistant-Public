...
MODEL="${SAMANTHA_MODEL:-qwen3:0.6b}"
...
uv run maturin develop -m rust/crates/samantha-python/Cargo.toml --quiet 2>/dev/null \
...
if curl -sf http://localhost:8000/health &>/dev/null; then
  fail "An Samantha server is already running on port 8000. Stop it before re-running quickstart so updated environment variables are applied."
fi
uv run samantha serve --port 8000 &>/dev/null &
BACKEND_PID=$!
CLEANUP_PIDS+=("$BACKEND_PID")
sleep 3

if ! kill -0 "$BACKEND_PID" 2>/dev/null; then
  fail "Backend exited during startup. Run 'uv run samantha serve --port 8000' to see the error."
...
echo ""
echo -e "${GREEN}${BOLD}  Samantha is running!${NC}"
echo ""
