...
#
# Referenced by `samantha bench --setup-energy` (see cli/bench_cmd.py), which
...
Darwin)
    if [ "$arch" != "arm64" ]; then
        echo "Intel Macs expose no energy counters Samantha can read." >&2
        exit 1
    fi
    extra="energy-apple"
    ;;
...
echo "Installing samantha[$extra] ..."
...
if monitor is None:
    raise SystemExit(
        "Energy monitor still unavailable. Run `samantha doctor` for details."
    )
