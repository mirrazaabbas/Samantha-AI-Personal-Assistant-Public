<#
.SYNOPSIS
    Register / unregister the Samantha Windows scheduled task.

.DESCRIPTION
    The Windows equivalent of deploy/systemd/samantha.service and
    deploy/launchd/com.samantha.plist.

    Registers a per-user scheduled task named "Samantha" that starts
    `samantha serve` at logon and restarts on failure. Loopback default
    ...
    Arguments (install only):
      -InstallRoot <path>  default: %LOCALAPPDATA%\Samantha (matches
                           install.ps1's default)
      -ListenHost <addr>   default: 127.0.0.1 (loopback). Set to 0.0.0.0
                           ONLY if you also set $env:SAMANTHA_API_KEY
                           — the server refuses to start unauthenticated
                           on a non-loopback bind.
      -ListenPort <int>    default: 8000

    Usage:
      powershell -ExecutionPolicy Bypass -File samantha-service.ps1 install
      powershell -ExecutionPolicy Bypass -File samantha-service.ps1 uninstall
      powershell -ExecutionPolicy Bypass -File samantha-service.ps1 status
#>
...
function Get-DefaultInstallRoot {
    ...
    if ($env:SAMANTHA_HOME) { return $env:SAMANTHA_HOME }
    return (Join-Path $env:LOCALAPPDATA 'Samantha')
}
...
function Install-Task {
    ...
    if (-not (Test-Path $srcDir)) {
        Write-Fail "Samantha source not found at $srcDir. Run install.ps1 first."
    }
    ...
    # Mirrors deploy/systemd/samantha.service's EnvironmentFile guard.
    $isLoopback = ($ListenHost -eq '127.0.0.1' -or $ListenHost -eq 'localhost')
    if (-not $isLoopback -and -not $env:SAMANTHA_API_KEY) {
        Write-Fail @"
ListenHost is $ListenHost (non-loopback) but `$env:SAMANTHA_API_KEY is
not set. An unauthenticated non-loopback bind is refused by samantha serve
and would also create a security hole. Set the env var first:

    `$env:SAMANTHA_API_KEY = (uv run samantha auth generate-key)
...
"@
    ...
    # `samantha serve` would refuse to bind 0.0.0.0 — failing silently every
    ...
    if (-not $isLoopback) {
        Write-Info "Persisting SAMANTHA_API_KEY to User environment so the scheduled task can read it at logon."
        [System.Environment]::SetEnvironmentVariable(
            'SAMANTHA_API_KEY',
            $env:SAMANTHA_API_KEY,
            'User'
        )
    }
    ...
    $action = New-ScheduledTaskAction `
        -Execute $uvPath `
        -Argument "run samantha serve --host $ListenHost --port $ListenPort" `
    ...
    Register-ScheduledTask `
        ...
        -Settings $settings `
        -Principal $principal `
        -Description 'Samantha API server (loopback default — see deploy/windows/README.md)' | Out-Null
