...
5. Clones the Samantha repository to `%LOCALAPPDATA%\Samantha`
...
`SAMANTHA_FORCE`. If you need richer control, save the script first
...
remove the task with `samantha-service.ps1`:

```powershell
$srv = "$env:LOCALAPPDATA\Samantha\src\deploy\windows\samantha-service.ps1"
...
`samantha-service.ps1 install` refuses `-ListenHost 0.0.0.0` if
`$env:SAMANTHA_API_KEY` is unset — same guard as the systemd unit's
`EnvironmentFile=/etc/samantha/env`.

## Parity table

| Concern | systemd | launchd | Windows |
|---------|---------|---------|---------|
| Service definition | `deploy/systemd/samantha.service` | `deploy/launchd/com.samantha.plist` | `deploy/windows/samantha-service.ps1` (cmdlet-driven) |
...
```powershell
cd "$env:LOCALAPPDATA\Samantha\src"
git pull --ff-only
...
```powershell
powershell -ExecutionPolicy Bypass -File "$env:LOCALAPPDATA\Samantha\src\deploy\windows\samantha-service.ps1" uninstall
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\Samantha"
```
