<#
.SYNOPSIS
    Samantha native Windows installer.
...
.DESCRIPTION
    ...
    Steps:
      ...
      5. Clone the Samantha repository to $env:LOCALAPPDATA\Samantha
         (override with $env:SAMANTHA_HOME).
    ...
    are honored via env vars when the corresponding flag is absent:
      ...
      $env:SAMANTHA_SERVICE      = '1'
      $env:SAMANTHA_FORCE        = '1'
...
.NOTES
    ...
    pass `--host 0.0.0.0` AND set $env:SAMANTHA_API_KEY (an
...
if (-not $SkipService -and $env:SAMANTHA_SKIP_SERVICE) { $SkipService = $true }
if (-not $Service     -and $env:SAMANTHA_SERVICE)      { $Service     = $true }
if (-not $Force       -and $env:SAMANTHA_FORCE)        { $Force       = $true }
...
if ($pyMajor -ne 3 -or $pyMinor -lt 10 -or $pyMinor -gt 13) {
    Write-Fail @"
Found Python $pyMajor.$pyMinor at $pythonExe, but Samantha requires
...
$installRoot = if ($env:SAMANTHA_HOME) {
    $env:SAMANTHA_HOME
} else {
    Join-Path $env:LOCALAPPDATA 'Samantha'
}
...
$repoUrl = if ($env:SAMANTHA_REPO_URL) {
    $env:SAMANTHA_REPO_URL
} else {
...
$modelPullOk = $false
if ($ollamaReady) {
    Write-Info "Pulling qwen3.5:2b (~1.5 GB) so 'samantha' works on first run..."
...
# ---------------------------------------------------------------------------
# 9. samantha.cmd shim - so bare `samantha` works in any new PowerShell
# ---------------------------------------------------------------------------

$binDir = Join-Path $installRoot 'bin'
$shimPath = Join-Path $binDir 'samantha.cmd'
...
# so the shim is self-locating - moving %LOCALAPPDATA%\Samantha won't
...
setlocal
set "SRC=%~dp0..\src"
uv run --project "%SRC%" samantha %*
"@
Set-Content -Path $shimPath -Value $shimContent -Encoding ASCII

# Add %LOCALAPPDATA%\Samantha\bin to User PATH if it isn't already
...
# the entry as `%LOCALAPPDATA%\Samantha\bin` (unexpanded) into User
...
Write-Ok "samantha shim installed at $shimPath"

# ---------------------------------------------------------------------------
# 10. Optional: register the scheduled-task service
# ---------------------------------------------------------------------------

$serviceScript = Join-Path $srcDir 'deploy\windows\samantha-service.ps1'
...
if ($Service) {
    ...
    if ($isInteractive) {
        $reply = Read-Host "Register Samantha as a Windows scheduled task (auto-start at logon, loopback only)? [y/N]"
...
if ($shouldInstallService) {
    ...
    Write-Ok "Scheduled task 'Samantha' registered (loopback default)."
}
...
Write-Host "  |   Samantha install complete    |" -ForegroundColor Green
Write-Host "  +----------------------------------+" -ForegroundColor Green
Write-Host ""
...
$nextCmd = if ($modelPullOk) { 'samantha' } else { 'samantha doctor' }

if ($pathNeedsRefresh) {
    Write-Host ""
    Write-Host "  Run it:  open a NEW PowerShell, then: $nextCmd" -ForegroundColor Yellow
    Write-Host "           (the samantha shim was added to your User PATH; the"
...
if (-not $modelPullOk) {
    ...
    Write-Host "        'samantha doctor' shows progress."
}

if ($shouldInstallService) {
    Write-Host ""
    Write-Host "  Service: schtasks /Query /TN Samantha     (status)"
    Write-Host "           powershell -File `"$serviceScript`" uninstall    (remove)"
}
Write-Host ""
Write-Host "  Docs:    https://github.com/mirrazaabbas/Samantha-AI-Personal-Assistant-Public/"
Write-Host ""
